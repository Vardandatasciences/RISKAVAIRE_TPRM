...
class ChangeManagementService:
    ...
    def upload_to_s3(self, file_path: Path, user_id: str = "system") -> Dict:
        ...
        """
        try:
            print(f"[UPLOAD] Uploading {file_path.name} to S3...")
            ...
            if result.get('success'):
                print(f"[OK] Successfully uploaded to S3: {result['file_info']['url']}")
                return {
                    'success': True,
                ...
                print(f"[ERROR] Failed to upload to S3: {result.get('error')}")
                return {
                    'success': False,
            ...
            print(f"[ERROR] Exception during S3 upload: {str(e)}")
            return {
                'success': False,
                'error': str(e)
            }
    ...
    def identify_framework(self, file_path: Path, ai_analysis: Optional[Dict[str, Any]] = None) -> Optional[Framework]:
        ...
        for pattern, possible_names in framework_patterns.items():
            if pattern in filename:
                ...
                for framework_name in possible_names:
                    ...
                    if framework:
                        print(f"[OK] Identified framework: {framework.FrameworkName} (ID: {framework.FrameworkId})")
                        return framework
        ...
        if ai_candidates:
            ...
            for candidate in ai_candidates:
                ...
                if direct_match:
                    print(f"[OK] AI-identified framework: {direct_match.FrameworkName} (ID: {direct_match.FrameworkId})")
                    return direct_match
                ...
                if close_matches:
                    ...
                    if framework:
                        print(f"[OK] AI-suggested framework matched: {framework.FrameworkName} (ID: {framework.FrameworkId})")
                        return framework
        ...
        if normalized_filename and frameworks:
            for framework in frameworks:
                ...
                if normalized_fw and (
                    ...
                    print(f"[OK] Filename matched framework: {framework.FrameworkName} (ID: {framework.FrameworkId})")
                    return framework
            ...
            if matches:
                ...
                for framework in frameworks:
                    ...
                    if normalized_fw == matched_name:
                        print(f"[OK] Filename fuzzy matched framework: {framework.FrameworkName} (ID: {framework.FrameworkId})")
                        return framework
        
        # If no specific match, log and return None
        print(f"[WARNING] Could not identify framework for file: {file_path.name}")
        return None
    ...
    def update_framework_amendment(self, framework: Framework, amendment_data: Dict, s3_info: Dict):
        ...
        try:
            with transaction.atomic():
                ...
                print(f"[OK] Successfully updated framework {framework.FrameworkName} with new amendment")
                return True
                
        except Exception as e:
            print(f"[ERROR] Error updating framework amendment: {str(e)}")
            return False
    ...
    def process_pdf_file(self, file_path: Path, user_id: str = "system") -> Dict:
        ...
        """
        print(f"\n{'='*60}")
        print(f"[DOC] Processing file: {file_path.name}")
        print(f"{'='*60}")
        ...
        try:
            ...
            if file_hash in processed_files.get('processed', []):
                print(f"⏭[EMOJI] File already processed (hash: {file_hash[:16]}...)")
            ...
            if not framework:
                ...
                print(f"[WARNING] Skipping file - framework not identified")
                return result
            ...
            if update_success:
                ...
                print(f"[OK] Successfully processed {file_path.name}")
            else:
            ...
            print(f"[ERROR] Exception processing file: {str(e)}")
            import traceback
            traceback.print_exc()
            return result
    ...
    def scan_and_process_pdfs(self, user_id: str = "system") -> Dict:
        ...
        """
        print(f"\n{'='*60}")
        print(f"[DEBUG] Scanning for PDFs in: {self.data_dir}")
        print(f"{'='*60}\n")
        ...
        try:
            ...
            print(f"[FOLDER] Found {len(pdf_files)} PDF file(s)")
            
            # Process each PDF file
            for pdf_file in pdf_files:
                print(f"\n[EMOJI] Processing: {pdf_file.name}")
            ...
            print(f"{'='*60}")
            print(f"[OK] Successfully processed: {results['files_processed']}")
            print(f"⏭[EMOJI] Skipped (already processed): {results['files_skipped']}")
            print(f"[ERROR] Failed: {results['files_failed']}")
            print(f"{'='*60}\n")
...
if __name__ == "__main__":
    # Test the service
    print("[INFO] Testing Change Management Service")
