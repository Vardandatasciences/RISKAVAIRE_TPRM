...
def _match_compliances_with_ai(target_compliances: list, db_compliances: list, framework_name: str, use_ai: bool, threshold: float) -> dict:
    ...
    for idx, target in enumerate(target_compliances, 1):
        logger.info(f"[DEBUG] [AI Compliance Fallback] Matching structured compliance {idx}/{len(target_compliances)}")
                    ...
                    if is_pdf:
                        ...
                        if s3_url:
                            try:
                                ...
                                logger.info(f"[OK] Saved S3 URL and document info to Amendment column for framework {framework_id}. S3 URL: {s3_url}")
                            except Exception as e:
                                logger.error(f"[ERROR] Failed to save S3 URL to Amendment column: {str(e)}")
                                import traceback
                                logger.error(traceback.format_exc())
...
def match_amendments_compliances(request, framework_id):
    ...
    try:
        ...
        if use_structured_fallback:
            logger.info("[EMOJI] Using structured amendment compliances for AI-based matching (fallback mode)")
