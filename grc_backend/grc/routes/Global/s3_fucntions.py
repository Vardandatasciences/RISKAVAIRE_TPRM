...
class RenderS3Client:
    ...
    def _setup_default_mysql(self):
        ...
        try:
            ...
            if DJANGO_SETTINGS_AVAILABLE and hasattr(settings, 'DATABASES'):
                ...
                print(f"[PROC] Using Django settings for MySQL: {mysql_config['host']}:{mysql_config['port']}/{mysql_config['database']}")
            else:
                ...
                print(f"[WARNING]  Django settings not available, using environment variables")
    ...
    def _save_operation_record(self, operation_type: str, operation_data: Dict) -> Optional[int]:
        ...
        try:
            ...
            print(f"[INFO] Operation recorded in MySQL: ID {operation_id}")
            return operation_id
    ...
    def _update_operation_record(self, operation_id: int, operation_data: Dict):
        ...
        try:
            ...
            print(f"[INFO] Operation {operation_id} updated in MySQL")
    ...
    def _extract_text_from_pdf(self, pdf_content: bytes, smart_extract: bool = True) -> tuple:
        ...
        try:
                ...
                with io.BytesIO(pdf_content) as pdf_buffer:
                    with pdfplumber.open(pdf_buffer) as pdf:
                        ...
                        if total_pages <= 5:
                            ...
                            extraction_strategy = "full"
                            print(f"[DOC] Small document ({total_pages} pages) - extracting all pages")
                        elif total_pages <= 20:
                            ...
                            print(f"[DOC] Medium document ({total_pages} pages) - extracting {len(pages_to_extract)} pages")
                        else:
                            ...
                            print(f"[DOC] Large document ({total_pages} pages) - extracting {len(pages_to_extract)} key pages")
                        ...
                        for page_num in pages_to_extract:
                            try:
                                ...
                                print(f"[WARNING]  Error extracting page {page_num + 1}: {str(page_error)}")
                        
                        print(f"[OK] Extracted text from {len(pages_to_extract)} pages using pdfplumber")
            ...
            # Fallback to PyPDF2
                ...
                print(f"[OK] Extracted text from {len(pages_to_extract)} pages using PyPDF2")
            
            else:
                print("[WARNING]  No PDF library available for text extraction")
                return "", 0, "none"
            ...
            if len(words) > 4000:
                ...
                print(f"[DOC] Text truncated to 4000 words for processing")
    ...
    def _extract_pdf_metadata(self, pdf_content: bytes, file_name: str, total_pages: int = None, extraction_strategy: str = None) -> Dict:
        ...
        try:
            ...
            if PDF_LIBRARY_AVAILABLE:
                ...
                print(f"[LIST] Extracted comprehensive metadata: {page_count} pages, {metadata.get('document_size_category', 'unknown')} document")
                ...
                print(f"[LIST] Extracted metadata: {page_count} pages")
...
Important: Keep the summary concise, professional, and actionable. Focus on what matters most."""
            ...
            print(f"[AI] Generating intelligent summary using Ollama ({OLLAMA_MODEL})...")
...
- Professional and objective in tone"""
            ...
            print(f"[OK] Intelligent summary generated: {len(summary)} characters, {len(lines)} lines")
    ...
    def _process_pdf_after_upload(self, operation_id: int, s3_url: str, file_name: str):
        ...
        try:
            print(f"\n{'='*60}")
            print(f"[EMOJI] Starting Enhanced PDF Processing")
            print(f"[DOC] Operation ID: {operation_id}")
            print(f"[EMOJI] File: {file_name}")
            print(f"{'='*60}")
            
            # Step 1: Download PDF content from S3
            print(f"\n[Step 1/5] [EMOJI]  Downloading PDF from S3...")
            print(f"   URL: {s3_url}")
            ...
            print(f"   [OK] Downloaded: {len(pdf_content)} bytes ({file_size_mb} MB)")
            
            # Step 2: Extract text using intelligent strategy
            print(f"\n[Step 2/5] [DOC] Extracting text from PDF (smart extraction)...")
            ...
            print(f"\n[Step 4/5] [AI] Generating AI-powered summary...")
            ...
            if summary and not summary.startswith("Summary unavailable"):
                ...
                print(f"   [WARNING]  Summary generation had issues: {summary[:100]}...")
            
            # Step 5: Update database with all information
            print(f"\n[Step 5/5] [SAVE] Updating database with metadata and summary...")
            self._update_pdf_metadata_in_db(operation_id, metadata, summary)
            
            # Step 6: Analyze audit relevance (runs in background thread)
            print(f"\n[Step 6/6] [DEBUG] Starting audit relevance analysis in background...")
            try:
                ...
                if framework_id:
                    ...
                    print(f"   [OK] Background analysis started for framework {framework_id}")
                else:
                    print(f"   [WARNING]  No framework_id found, skipping audit relevance analysis")
            except Exception as e:
                print(f"   [WARNING]  Failed to start audit relevance analysis: {str(e)}")
            
            print(f"\n{'='*60}")
            print(f"[OK] PDF PROCESSING COMPLETED SUCCESSFULLY")
            ...
            print(f"\n[ERROR] ERROR: PDF processing failed for operation {operation_id}")
            print(f"   Error: {str(e)}")
            ...
            try:
                ...
                print(f"   [WARNING]  Also failed to update database: {str(db_error)}")
    ...
    def _update_pdf_metadata_in_db(self, operation_id: int, metadata: Dict, summary: str):
        ...
        """
        if not self.db_pool or not operation_id:
            print("[WARNING]  Database pool not available or invalid operation_id")
            return
    ...
    def _process_excel_after_upload(self, operation_id: int, s3_url: str, file_name: str):
        ...
        try:
            print(f"\n{'='*60}")
            print(f"[EMOJI] Starting Excel Processing")
            print(f"[DOC] Operation ID: {operation_id}")
            print(f"[EMOJI] File: {file_name}")
            print(f"{'='*60}")
            
            # Step 1: Download Excel content from S3
            print(f"\n[Step 1/6] [EMOJI]  Downloading Excel from S3...")
            print(f"   URL: {s3_url}")
            response = requests.get(s3_url, timeout=90)
            response.raise_for_status()
            ...
            print(f"   [OK] Downloaded: {len(excel_content)} bytes ({file_size_mb} MB)")
            
            # Step 2: Extract text from Excel
            print(f"\n[Step 2/6] [STATS] Extracting text from Excel...")
            text = self._extract_text_from_excel(excel_content, file_name)
            
            if not text:
                print("   [WARNING]  No text extracted from Excel")
                metadata = {
                    'file_type': 'excel',
                    'file_name': file_name,
                ...
                print(f"\n[Step 6/6] [SAVE] Updating database...")
                self._update_pdf_metadata_in_db(
                    operation_id, 
                    metadata, 
                    "No text content available for summary. Excel file may be empty or corrupted."
                )
                print(f"\n[WARNING]  Excel processing completed with limited results (no text extracted)")
                return
            
            # Step 3: Extract metadata
            print(f"\n[Step 3/6] [LIST] Extracting metadata...")
            ...
            # Step 4: Generate AI summary using OpenAI
            print(f"\n[Step 4/6] [AI] Generating AI-powered summary...")
            summary = self._generate_summary_with_ollama(text, metadata)
            ...
            # Step 5: Update database with all information
            print(f"\n[Step 5/6] [SAVE] Updating database with metadata and summary...")
            self._update_pdf_metadata_in_db(operation_id, metadata, summary)
            ...
            print(f"\n{'='*60}")
            print(f"[OK] EXCEL PROCESSING COMPLETED SUCCESSFULLY")
            print(f"   Operation ID: {operation_id}")
            print(f"   File: {file_name}")
            print(f"   Summary Length: {len(summary)} chars")
            print(f"{'='*60}\n")
            ...
            print(f"\n[ERROR] ERROR: Excel processing failed for operation {operation_id}")
            print(f"   Error: {str(e)}")
            print(f"   Type: {type(e).__name__}")
    ...
    def _extract_text_from_excel(self, excel_content: bytes, file_name: str) -> str:
        ...
        try:
            if not PANDAS_AVAILABLE:
                print("[WARNING]  pandas not available, cannot extract Excel content")
                return ""
            ...
            print(f"   [OK] Extracted {len(combined_text)} characters from {len(excel_data)} sheet(s)")
            return combined_text
            
        except Exception as e:
            print(f"[WARNING]  Error extracting text from Excel: {str(e)}")
            return ""
    ...
    def _extract_excel_metadata(self, excel_content: bytes, file_name: str, text: str) -> Dict:
        ...
        try:
            ...
            print(f"[WARNING]  Error extracting Excel metadata: {str(e)}")
            return {
                'file_type': 'excel',
                'file_name': file_name,
                'file_size_bytes': len(excel_content),
    ...
    def _get_file_framework_id(self, operation_id: int) -> Optional[int]:
        ...
        try:
            ...
            print(f"[WARNING]  Error getting framework_id: {str(e)}")
            return None
        finally:
            cursor.close()
            conn.close()
    ...
    def _get_all_database_data(self, framework_id: int) -> Dict:
        ...
        try:
            ...
            print(f"[WARNING]  Error getting database data: {str(e)}")
        finally:
            cursor.close()
            conn.close()
    ...
    def _get_all_documents_from_file_operations(self, framework_id: int) -> List[Dict]:
        ...
        try:
            ...
            print(f"[WARNING]  Error getting documents from file_operations: {str(e)}")
            return []
        finally:
            cursor.close()
            conn.close()
    ...
    def _analyze_database_data_relevance(self, audit_id: int, audit_details: Dict, database_data: Dict) -> Dict:
        ...
        for table_name, records in database_data.items():
            ...
            print(f"      [STATS] Analyzing {len(records)} {table_name} records for database relevance...")
...
{{
    ...
    "matched_compliances": []
                    ...
                    if result_text:
                        ...
                        if analysis:
                            ...
                            print(
                                f"         [SAVE] [{table_name}] record_id={record_id} "
                    ...
                    print(f"      [ERROR] Fatal error analyzing {table_name} record: {str(e)}")
                    import traceback
                    traceback.print_exc()
                    raise
            ...
            if analyzed_count:
                print(
                    f"      [OK] {table_name}: analyzed={analyzed_count}, "
                    f"relevant_>=0.6={relevant_count_60}, relevant_>=0.8={relevant_count_80}"
                )
            else:
                print(f"      ℹ[EMOJI]  No {table_name} records could be analyzed for relevance.")
    ...
    def _analyze_audit_relevance_background(self, operation_id: int, summary: str, metadata: Dict, framework_id: int):
        ...
        try:
            print(f"\n{'='*60}")
            print(f"[DEBUG] Starting Comprehensive Audit Relevance Analysis")
            ...
            if not audits:
                print(f"   ℹ[EMOJI]  No active audits found for framework {framework_id}")
                return
            
            print(f"   [LIST] Found {len(audits)} active audits to analyze")
            
            # Get ALL documents from file_operations
            all_documents = self._get_all_documents_from_file_operations(framework_id)
            print(f"   [DOC] Found {len(all_documents)} documents in file_operations")
            
            # Get ALL database data
            all_database_data = self._get_all_database_data(framework_id)
            db_count = sum(len(v) for v in all_database_data.values())
            print(f"   [SAVE] Found {db_count} database records across all tables")
            
            # Analyze relevance for each audit
            for audit in audits:
                try:
                    audit_id = audit['audit_id']
                    print(f"\n   [DEBUG] Analyzing Audit ID: {audit_id} - {audit.get('audit_title', 'No Title')[:50]}")
                    ...
                    print(f"      [DOC] Analyzing {len(all_documents)} documents...")
                    for doc in all_documents:
                        ...
                        if file_key and file_key in analyzed_file_keys:
                            print(f"      ⏭[EMOJI]  Skipping document {doc_id} (s3_key/stored_name='{file_key[:50]}...' already analyzed)")
                            continue
                        ...
                        if doc_id in analyzed_operation_ids:
                            print(f"      ⏭[EMOJI]  Skipping document {doc_id} (operation_id already analyzed - backward compatibility check)")
                            continue
                        ...
                        print(f"      [DEBUG] Analyzing document {doc_id} (file_name='{doc.get('file_name', 'unknown')[:50]}...', s3_key='{doc_s3_key[:50] if doc_s3_key else 'none'}...', stored_name='{doc_stored_name[:50] if doc_stored_name else 'none'}...')")
                        ...
                        if relevance_result:
                            ...
                            if relevance_result.get('relevance_score', 0) >= 0.6:
                                ...
                                print(f"      ⏭[EMOJI]  Document {doc_id} relevance score {relevance_result.get('relevance_score', 0):.2f} < 0.6, not storing in database")
                        else:
                            print(f"      [WARNING]  Document {doc_id} analysis returned no result")
                    # Analyze ALL database data (incremental using existing db_index)
                    print(f"      [SAVE] Analyzing database data...")
                    ...
                    print(f"      [SAVE] Database incremental mode: {new_total} new record(s) to analyze")
                    print(f"      [SAVE] Using {cached_total} cached database record(s) from JSON index")
                    ...
                    print(f"      [STATS] Overall Relevance Score: {overall_score:.2f}")
                    
                    # Determine what evidence exists
                    has_document_evidence = len(all_doc_scores) > 0
                    has_database_evidence = len(all_db_scores) > 0
                    
                    print(f"      [STATS] Evidence summary: {len(all_doc_scores)} document(s), {len(all_db_scores)} database record(s)")
                    
                    # Trigger audit if relevant
                    if overall_score >= 0.6:
                        print(f"      [OK] Audit {audit_id} is relevant - triggering audit execution")
                        ...
                        if has_document_evidence and has_database_evidence:
                            ...
                            print(f"      [EMOJI] BOTH document and database evidence found → triggering COMBINED EVIDENCE check")
                            if best_doc:
                                try:
                                    relevance_result = {
                                    ...
                                    print(f"      [OK] Auto-processing triggered - compliance check will use COMBINED EVIDENCE (document + database)")
                                except Exception as e:
                                    print(f"      [WARNING]  Error triggering combined check: {str(e)}")
                            else:
                                print(f"      [WARNING]  No document found to trigger combined check (database evidence exists but no document)")
                        elif has_document_evidence:
                            # Only document → trigger document-only check
                            print(f"      [DOC] Only document evidence found → triggering DOCUMENT-ONLY check")
                            if best_doc:
                                try:
                                    relevance_result = {
                                        "relevance_score": best_doc.get("relevance_score", 0),
                                    ...
                                    print(f"      [OK] Auto-processing triggered - compliance check will use DOCUMENT-ONLY evidence")
                                except Exception as e:
                                    print(f"      [WARNING]  Error triggering document-only check: {str(e)}")
                        elif has_database_evidence:
                            # Only database → database evidence already created, will be checked when document is uploaded
                            print(f"      [SAVE] Only database evidence found → database evidence created in ai_audit_data")
                            print(f"      [INFO] Database evidence will be checked when a document is uploaded or manually checked")
                        else:
                            print(f"      [WARNING]  No evidence found to trigger audit")
                    
                except Exception as e:
                    # Treat any error in a given audit as fatal for this background run
                    print(f"      [ERROR] Fatal error analyzing audit {audit.get('audit_id', 'unknown')}: {str(e)}")
                    import traceback
                    traceback.print_exc()
            ...
            print(f"\n{'='*60}")
            print(f"[OK] Comprehensive Audit Relevance Analysis Completed")
            print(f"{'='*60}\n")
            
        except Exception as e:
            print(f"\n[ERROR] Error in audit relevance analysis: {str(e)}")
            import traceback
            traceback.print_exc()
    ...
    def _create_ai_evidence_from_database_results(self, framework_id: Optional[int], audit_id: int, db_results: Dict):
        ...
        try:
                    ...
                    try:
                        ...
                        if comp_id:
                            try:
                                ...
                                if comp_row:
                                    ...
                                    print(f"      [WARNING]  No compliance row found for ComplianceId={comp_id}")
                            except Exception as comp_err:
                                print(f"      [WARNING]  Error resolving compliance mapping for {comp_id}: {str(comp_err)}")
                            ...
                            if existing:
                                ...
                                if needs_update:
                                    try:
                                        ...
                                        print(f"      [OK] Updated existing ai_audit_data record {existing_id} with analysis data")
                                    except Exception as upd_err:
                                        print(f"      [WARNING]  Error updating existing ai_audit_data mapping for {external_id}: {str(upd_err)}")
                                # Either way, we don't insert a duplicate row
                                continue
                        except Exception as check_err:
                            print(f"      [WARNING]  Error checking existing ai_audit_data for {external_id}: {str(check_err)}")
                        ...
                        if row:
                            ...
                            try:
                                ...
                                print(f"      [WARNING]  Error updating document_id for DB evidence row {new_doc_id}: {str(upd_err)}")
                        ...
                        if score >= checklist_threshold and primary_compliance_id:
                            try:
                                ...
                                print(f"      [WARNING]  Error updating lastchecklistitemverified for compliance {primary_compliance_id}: {str(checklist_err)}")
                        
                    except Exception as per_record_err:
                        print(f"      [WARNING]  Error creating DB evidence for {table_name} record {analysis.get('record_id')}: {str(per_record_err)}")
                        import traceback
                        traceback.print_exc()
                        continue
            ...
            print(f"      [ERROR] Error while creating ai_audit_data evidence from database results: {str(e)}")
            import traceback
            traceback.print_exc()
        finally:
            try:
    ...
    def _get_active_ai_audits(self, framework_id: int) -> List[Dict]:
        ...
        try:
            ...
            print(f"[WARNING]  Error getting active AI audits: {str(e)}")
            return []
        finally:
            cursor.close()
            conn.close()
    ...
    def trigger_audit_relevance_analysis_for_framework(self, framework_id: int):
        ...
        try:
            print(f"\n{'='*60}")
            print(f"[EMOJI] Manual Trigger: Audit Relevance Analysis for Framework {framework_id}")
            print(f"{'='*60}\n")
            
            # Get all active audits
            audits = self._get_active_ai_audits(framework_id)
            if not audits:
                print(f"   ℹ[EMOJI]  No active audits found")
                return
            
            # Get all documents
            all_documents = self._get_all_documents_from_file_operations(framework_id)
            print(f"   [DOC] Found {len(all_documents)} documents")
            
            # Get all database data
            all_database_data = self._get_all_database_data(framework_id)
            db_count = sum(len(v) for v in all_database_data.values())
            print(f"   [SAVE] Found {db_count} database records")
            ...
            print(f"\n[OK] Manual analysis completed for framework {framework_id}\n")
            
        except Exception as e:
            print(f"[ERROR] Error in manual analysis: {str(e)}")
            import traceback
            traceback.print_exc()
    ...
    def _get_audit_details(self, audit_id: int) -> Dict:
        ...
        try:
            ...
            print(f"[WARNING]  Error getting audit details: {str(e)}")
            return {}
        finally:
            cursor.close()
            conn.close()
    ...
    def _call_ollama(self, prompt: str, system_message: str = None, max_tokens: int = 800, temperature: float = 0.3) -> Optional[str]:
        ...
        try:
            ...
            print(f"[WARNING]  Error calling Ollama: {str(e)}")
            return None
    ...
    def _load_json_index(self, file_path: str) -> Dict:
        ...
        try:
            ...
            print(f"[WARNING]  Error loading JSON index {file_path}: {str(e)}")
        return {
            "framework_id": None,
            "audit_id": None,
            "last_updated": None,
                        ...
                        try:
                            ...
                            if self.db_pool:
                                ...
                                try:
                                    ...
                                    conn.commit()
                                    print(f"[OK] Marked {updated_count} document(s) as 'pending' for re-processing in audit {audit_id}")
                                except Exception as db_err:
                                    conn.rollback()
                                    print(f"[WARNING]  Database error triggering re-processing: {str(db_err)}")
                                finally:
                                    cursor.close()
                                    conn.close()
                            else:
                                print(f"[WARNING]  No database pool available to trigger re-processing")
                        except Exception as trigger_err:
                            print(f"[WARNING]  Error in background trigger: {str(trigger_err)}")
                    
                    trigger_thread = threading.Thread(target=trigger_processing, daemon=True)
                    trigger_thread.start()
                    print(f"[EMOJI] Started background thread to trigger audit re-processing for audit {audit_id}")
                except Exception as trigger_err:
                    # Don't fail JSON save if trigger fails
                    print(f"[WARNING]  Error triggering audit re-processing (non-fatal): {str(trigger_err)}")
                    
        except Exception as e:
            print(f"[WARNING]  Error saving JSON index {file_path}: {str(e)}")
    ...
    def _update_json_index_document(self, framework_id: int, audit_id: int, operation_id: int, analysis_result: Dict, doc_s3_key: str = None, doc_stored_name: str = None):
        ...
        if doc_index is not None:
            ...
            if existing.get("operation_id") != operation_id:
                print(f"      [EMOJI] Updating existing document entry: s3_key/stored_name='{file_key[:50] if file_key else 'none'}...' (operation_id: {existing.get('operation_id')} -> {operation_id})")
...
IMPORTANT: Only return IDs that are present in the framework lists provided above."""
            ...
            if not result_json:
                try:
                    ...
                    print(f"      [WARNING]  Failed to parse markdown response: {str(parse_err)}")
                    return None
            
            if not result_json:
                print(f"      [WARNING]  Could not parse response as JSON or markdown")
            ...
            print(f"      [STATS] Framework has {len(framework_policy_ids)} policies, {len(framework_subpolicy_ids)} subpolicies, {len(framework_compliance_ids)} compliances")
            ...
            if isinstance(matched_policies, list):
                ...
                if filtered_policies:
                    print(f"      [WARNING]  Filtered out policy IDs from other frameworks: {filtered_policies}")
                print(f"      [OK] Matched {len(result_json['matched_policies'])} policies: {result_json['matched_policies']}")
            ...
            if isinstance(matched_subpolicies, list):
                ...
                if filtered_subpolicies:
                    print(f"      [WARNING]  Filtered out subpolicy IDs from other frameworks: {filtered_subpolicies}")
                print(f"      [OK] Matched {len(result_json['matched_subpolicies'])} subpolicies: {result_json['matched_subpolicies']}")
            ...
            if isinstance(matched_compliances, list):
                ...
                if filtered_compliances:
                    print(f"      [WARNING]  Filtered out compliance IDs from other frameworks: {filtered_compliances}")
                print(f"      [OK] Matched {len(result_json['matched_compliances'])} compliances: {result_json['matched_compliances']}")
            else:
                result_json['matched_compliances'] = []
                print(f"      [WARNING]  matched_compliances was not a list, reset to empty array")
            ...
            print(f"[WARNING]  Error in audit relevance analysis: {str(e)}")
            return None
        ...
        try:
            ...
            if file_s3_key or file_stored_name:
                ...
                if other_operation:
                    ...
                    if existing:
                        ...
                        print(f"      [EMOJI] Found existing relevance record for same file (s3_key/stored_name='{file_key_display}...') (file_operation_id={existing_relevance_id}, current operation_id={operation_id})")
            ...
            if existing_relevance_id:
                ...
                try:
                    ...
                    cursor.execute(query, params)
                    print(f"      [OK] Updated existing relevance record (same file, different module upload)")
                    ...
                    if 'Unknown column' in str(col_err):
                        ...
                        cursor.execute(query, params)
                        print(f"      [OK] Updated existing relevance record (s3_key/stored_name columns not available)")
                    else:
                        raise
            else:
                ...
                try:
                    ...
                    cursor.execute(query, params)
                    print(f"      [SAVE] Stored new relevance in database")
                except mysql.connector.Error as col_err:
                    ...
                    if 'Unknown column' in str(col_err):
                        ...
                        cursor.execute(query, params)
                        print(f"      [SAVE] Stored new relevance in database (s3_key/stored_name columns not available)")
                    else:
                        raise
            ...
            try:
                print(f"      [AI] Triggering auto-processing for audit {audit_id}")
                ...
                print(f"      [OK] Auto-processing thread started")
            except Exception as auto_err:
                print(f"      [WARNING]  Error starting auto-processing: {str(auto_err)}")
                import traceback
                traceback.print_exc()
                # Don't fail the whole operation if auto-processing fails
            
        except mysql.connector.Error as e:
            print(f"      [ERROR] Database error storing relevance: {str(e)}")
        except Exception as e:
            print(f"      [ERROR] Error storing relevance: {str(e)}")
        finally:
            cursor.close()
            conn.close()
    ...
    def _auto_process_relevant_document(self, operation_id: int, audit_id: int, relevance_result: Dict):
        ...
        try:
            print(f"\n{'='*60}")
            print(f"[AI] AUTO-PROCESSING: Will process ALL relevant documents from JSON for Audit {audit_id}")
            print(f"   (Triggered by document {operation_id})")
            print(f"{'='*60}")
            
            if not self.db_pool:
                print(f"      [ERROR] Database pool not available")
                return
            ...
            try:
                ...
                if not audit_row or not audit_row.get('FrameworkId'):
                    print(f"      [ERROR] No FrameworkId found for audit {audit_id}")
                    return
                ...
                try:
                    ...
                    print(f"      [LIST] Looking for documents_analysis.json at: {json_path}")
                    ...
                    if os.path.exists(json_path):
                        try:
                            ...
                            print(f"      [LIST] Found {len(documents_in_json)} document(s) in documents_analysis.json - will process ALL documents with matched compliances")
                            ...
                            for doc_entry in documents_in_json:
                                ...
                                if matched_compliances_for_doc and doc_operation_id:
                                    ...
                                    print(f"      [LIST] Document operation_id={doc_operation_id} has {len(matched_compliances_for_doc)} matched compliances: {matched_compliances_for_doc} - will be processed")
                                elif doc_operation_id:
                                    print(f"      [LIST] Document operation_id={doc_operation_id} has NO matched compliances - skipping")
                            
                            print(f"      [OK][OK][OK] WILL PROCESS {len(all_relevant_documents_to_process)} relevant document(s) from JSON")
                            
                        except Exception as json_err:
                            print(f"      [WARNING] Could not read/parse documents_analysis.json: {json_err}")
                            import traceback
                            traceback.print_exc()
                    else:
                        print(f"      ℹ[EMOJI] documents_analysis.json not found at {json_path}, will process only uploaded document")
                
                except Exception as json_read_err:
                    print(f"      [WARNING] Error reading documents_analysis.json: {json_read_err}")
                    import traceback
                    traceback.print_exc()
                ...
                if not all_relevant_documents_to_process:
                    print(f"      [LIST] No relevant documents found in JSON - will process only the uploaded document (file_operation_id={operation_id})")
                    ...
                    if matched_compliances_for_uploaded:
                        ...
                        print(f"      [OK][OK][OK] MATCHED COMPLIANCES FOUND for file_operation_id={operation_id}, audit_id={audit_id}")
                        print(f"      [LIST] Found {len(matched_compliances_for_uploaded)} matched compliances: {matched_compliances_for_uploaded}")
                    else:
                ...
                if not uploaded_doc_in_list:
                    print(f"      [LIST] Adding uploaded document (file_operation_id={operation_id}) to processing list")
                    matched_compliances_for_uploaded = relevance_result.get('matched_compliances', [])
                    all_relevant_documents_to_process.append((operation_id, matched_compliances_for_uploaded))
                
                # STEP 4: Log what documents we found
                print(f"      [EMOJI] Found {len(all_relevant_documents_to_process)} relevant document(s) to process from JSON:")
                ...
                if raw_user_id is not None:
                    try:
                        ...
                        print(f"      [USER] Using numeric user_id: {uploaded_by}")
                        ...
                        try:
                            ...
                            if user_row and user_row.get('UserId'):
                                ...
                                print(f"      [USER] Resolved username '{raw_user_id}' to UserId {uploaded_by}")
                            else:
                                cursor.execute("SELECT UserId FROM users WHERE LOWER(UserName) = LOWER(%s) LIMIT 1", (raw_user_id,))
                                user_row = cursor.fetchone()
                                if user_row and user_row.get('UserId'):
                                    uploaded_by = int(user_row.get('UserId'))
                                    print(f"      [USER] Resolved username '{raw_user_id}' (case-insensitive) to UserId {uploaded_by}")
                        except Exception as user_lookup_err:
                            print(f"      [WARNING]  Error in user lookup query: {str(user_lookup_err)}")
                
                if not uploaded_by:
                    print(f"      [WARNING]  Could not resolve user_id (raw_user_id={raw_user_id}); notifications will be skipped")
                ...
                for doc_operation_id, doc_matched_compliances in all_relevant_documents_to_process:
                    try:
                        ...
                        if doc_operation_id in processed_operation_ids:
                            print(f"      ⏭[EMOJI] Skipping duplicate document operation_id={doc_operation_id} (already processed in this batch)")
                            continue
                        
                        print(f"      [DOC] Processing document operation_id={doc_operation_id} with {len(doc_matched_compliances)} matched compliances: {doc_matched_compliances}")
                        ...
                        if not file_op:
                            print(f"      [WARNING] File operation {doc_operation_id} not found or not completed - skipping")
                            continue
                        ...
                        if existing_count > 0:
                            print(f"      ⏭[EMOJI] Document operation_id={doc_operation_id} already exists in ai_audit_data for audit {audit_id} ({existing_count} record(s)) - skipping to avoid duplicate")
                        ...
                        print(f"        [LIST] Matched Compliances: {len(matched_compliances)}")
                        print(f"        [LIST] Matched Policies: {len(matched_policies)}")
                        print(f"        [LIST] Matched Subpolicies: {len(matched_subpolicies)}")
                        ...
                        print(f"        [INFO] Creating {len(mappings_to_create)} mapping(s) for document operation_id={doc_operation_id}")
                        ...
                        for mapping in mappings_to_create:
                            try:
                                ...
                                if document_id:
                                    ...
                                    print(f"        [OK] Created mapping: document_id={document_id}, policy_id={mapping['policy_id']}, subpolicy_id={mapping['subpolicy_id']}, compliance_id={mapping['compliance_id']}")
                            
                            except Exception as mapping_err:
                                print(f"        [ERROR] Error creating mapping: {str(mapping_err)}")
                                import traceback
                                traceback.print_exc()
                                continue
                        
                        conn.commit()
                        print(f"        [OK] Finished processing document operation_id={doc_operation_id} - created {len(doc_created_document_ids)} record(s)")
                        ...
                        print(f"      [ERROR] Error processing document {doc_operation_id}: {doc_process_err}")
                        import traceback
                        traceback.print_exc()
                        # Re-open dictionary cursor for next document
                        try:
                ...
                cursor.close()
                print(f"      [OK][OK][OK] Processed ALL {len(all_relevant_documents_to_process)} relevant document(s) from JSON - created {len(all_created_document_ids)} total record(s)")
                ...
                if all_created_document_ids:
                    print(f"      [DEBUG] Triggering automatic compliance checks for {len(all_created_document_ids)} document(s)...")
                    print(f"      [LIST] Note: Checks will automatically use COMBINED EVIDENCE if both document and database evidence exist")
                    ...
                    def trigger_compliance_check(doc_id, audit_id_val, user_id_val, compliance_id_val):
                        ...
                        try:
                            ...
                            if result.get('success'):
                                print(f"      [OK] Auto-compliance check completed for document_id={doc_id}")
                            else:
                                print(f"      [WARNING] Auto-compliance check failed for document_id={doc_id}: {result.get('error', 'Unknown error')}")
                        except Exception as check_err:
                            print(f"      [ERROR] Error in auto-compliance check for document_id={doc_id}: {str(check_err)}")
                            import traceback
                            traceback.print_exc()
                    ...
                    for doc_id in all_created_document_ids:
                        ...
                        check_thread.start()
                        print(f"      [INFO] Started compliance check thread for document_id={doc_id}")
                
                print(f"{'='*60}")
                print(f"[OK] AUTO-PROCESSING COMPLETED")
                ...
                def send_audit_completion_notification(audit_id_val, doc_count, user_id_val):
                    ...
                    try:
                        ...
                        if conn_notif:
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    print(f"      [NOTIFY] Calling create_audit_completion_notification: audit_id={audit_id_val}, user_id={user_id_val}, doc_count={doc_count}")
                                    ...
                                    if notification:
                                        print(f"      [EMOJI] Notification created successfully for audit {audit_id_val}: {audit_name}")
                                        print(f"      [EMOJI] Notification details: user_id={notification.get('user_id')}, title={notification.get('title')}")
                                    else:
                                        print(f"      [WARNING]  Notification creation returned None for audit {audit_id_val}")
                                except ImportError as import_err:
                                    print(f"      [WARNING]  Could not import notifications module: {str(import_err)}")
                                    import traceback
                                    traceback.print_exc()
                                except Exception as notif_create_err:
                                    print(f"      [WARNING]  Error creating notification: {str(notif_create_err)}")
                                    import traceback
                                    traceback.print_exc()
                            finally:
                                cursor_notif.close()
                        ...
                        print(f"      [WARNING] Error creating notification: {str(notif_err)}")
                        import traceback
                        traceback.print_exc()
                ...
                if all_created_document_ids:
                    if notification_user_id:
                        print(f"      [EMOJI] Will create notification for user_id={notification_user_id} (numeric UserId)")
                        ...
                        print(f"      [WARNING]  Skipping notification: Could not resolve user_id (raw_user_id={raw_user_id}, uploaded_by={uploaded_by})")
                
            except Exception as e:
                print(f"      [ERROR] Error in auto-processing: {str(e)}")
                import traceback
                traceback.print_exc()
            finally:
                cursor.close()
            ...
            print(f"      [ERROR] Fatal error in auto-processing: {str(e)}")
            import traceback
            traceback.print_exc()
    ...
    def test_connection(self) -> Dict:
        ...
        try:
            print("[EMOJI] Testing Direct microservice connection...")
        ...
        # Test MySQL database
        try:
            print("[EMOJI] Testing MySQL database connection...")
            if self.db_pool:
                conn = self._get_db_connection()
                if conn:
                    cursor = conn.cursor()
                ...
                print("[WARNING]  MySQL database: Not configured")
    ...
    def upload(self, file_path: str, user_id: str = "default-user", 
        ...
        try:
            ...
            print(f"[UPLOAD] Uploading {file_name} ({file_size} bytes) via Direct...")
            print(f"[EMOJI] Module: {module or 'general'}")
            print(f"[DOC] Original name: {original_file_name}")
            print(f"[DOC] New original name: {new_original_name}")
            ...
            print(f"[EMOJI] Upload URL: {url}")
            
            with open(file_path, 'rb') as file:
                files = {'file': (file_name, file, mimetypes.guess_type(file_path)[0])}
                
                print(f"[FOLDER] File details: name={file_name}, size={file_size}, type={mimetypes.guess_type(file_path)[0]}")
                
                try:
                    upload_start_time = datetime.datetime.now()
                    print(f"⏱[EMOJI]  [UPLOAD] Starting upload at {upload_start_time.strftime('%H:%M:%S')}")
                    print(f"⏱[EMOJI]  [UPLOAD] Timeout set to: 600 seconds (10 minutes)")
                    print(f"⏱[EMOJI]  [UPLOAD] File size: {file_size:,} bytes ({file_size / (1024*1024):.2f} MB)")
                    ...
                    print(f"⏱[EMOJI]  [UPLOAD] Upload completed in {upload_elapsed:.2f} seconds")
                    print(f"[STATS] [UPLOAD] Response status: {response.status_code}")
                    print(f"[INFO] [UPLOAD] Response headers: {dict(response.headers)}")
                    
                    if response.status_code != 200:
                        print(f"[ERROR] [UPLOAD] ERROR Response content: {response.text}")
                        
                    response.raise_for_status()
                    result = response.json()
                    print(f"[OK] [UPLOAD] SUCCESS Upload response: {result}")
                    
                except requests.exceptions.Timeout as timeout_error:
                    upload_elapsed = (datetime.datetime.now() - upload_start_time).total_seconds()
                    print(f"[ERROR] [UPLOAD] TIMEOUT after {upload_elapsed:.2f} seconds")
                    print(f"[ERROR] [UPLOAD] Microservice at {url} did not respond within 600 seconds")
                    ...
                    print(f"[ERROR] [UPLOAD] Request failed after {upload_elapsed:.2f} seconds: {str(e)}")
                    print(f"[ERROR] [UPLOAD] Error type: {type(e).__name__}")
                    if hasattr(e, 'response') and e.response is not None:
                        print(f"[ERROR] [UPLOAD] Error response status: {e.response.status_code}")
                        print(f"[ERROR] [UPLOAD] Error response content: {e.response.text}")
                    import traceback
                    traceback.print_exc()
                    raise
            ...
            if result.get('success'):
                ...
                if file_extension == '.pdf' and operation_id:
                    print(f"[DOC] PDF detected, starting background processing...")
                    ...
                    print(f"[OK] PDF processing thread started for operation {operation_id}")
                elif file_extension in ['.xlsx', '.xls'] and operation_id:
                    print(f"[STATS] Excel file detected, starting background processing...")
                    ...
                    processing_thread.start()
                    print(f"[OK] Excel processing thread started for operation {operation_id}")
    ...
    def download(self, s3_key: str, file_name: str, 
        ...
        try:
            print(f"[EMOJI]  Downloading {file_name} via Direct...")
    ...
    def export(self, data: Union[List[Dict], Dict], export_format: str, 
        ...
        try:
            ...
            print(f"[STATS] Exporting {record_count} records as {export_format.upper()} via Direct...")
            ...
            print(f"[LINK] Export URL: {url}")
            print(f"[FILE] Payload size: {len(str(payload))} characters")
            print(f"[KEY] Using AWS credentials: {aws_credentials['awsAccessKey'][:10]}...")
            ...
            print(f"[INFO] Full error details: {type(e).__name__}: {error_msg}")
...
def export_to_excel(data):
    ...
    try:
        ...
        try:
            ...
            print(f"[OK] Excel export successful with xlsxwriter. File size: {len(output.getvalue())} bytes")
            ...
            try:
                ...
                print(f"[OK] Excel export successful with openpyxl. File size: {len(output.getvalue())} bytes")
        ...
        print(f"[ERROR] Excel export error: {str(e)}")
        import traceback
        traceback.print_exc()
...
def export_data(data=None, file_format='xlsx', user_id='user123', options=None, s3_client_instance=None):
    ...
    print(f"\n{'='*80}")
    print(f"[INFO] [EXPORT] Starting export process at {start_time_total.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'='*80}")
    
    if data is None:
        data = []
        print(f"[WARNING]  [EXPORT] No data provided, using empty list")
        
    if options is None:
        options = {}
        print(f"ℹ[EMOJI]  [EXPORT] No options provided, using defaults")
    else:
        print(f"ℹ[EMOJI]  [EXPORT] Options received: {options}")
    ...
    print(f"\n[STATS] [EXPORT] Data validation:")
    print(f"   [EMOJI] Data size: {data_size:,} bytes ({data_size / (1024*1024):.2f} MB)")
    print(f"   [EMOJI] Record count: {record_count:,}")
    print(f"   [EMOJI] Format: {file_format}")
    ...
    print(f"\n[INFO] [EXPORT] Generating filename...")
    if options.get('file_name'):
        base_name = options.get('file_name')
        if '.' in base_name:
            base_name = base_name.rsplit('.', 1)[0]
        file_name = f"{base_name}.{file_format}"
        print(f"   [EMOJI] Using provided filename: {file_name}")
    else:
        file_name = f"export_{user_id}_{int(timestamp)}.{file_format}"
        print(f"   [EMOJI] Generated filename: {file_name}")
    
    # Get or create S3 client instance
    print(f"\n[PROC] [EXPORT] Initializing S3 client...")
    if s3_client_instance is None:
        try:
            print(f"   [EMOJI] S3 client not provided, creating new instance...")
            from django.conf import settings
            ...
            print(f"   [EMOJI] MySQL config: {mysql_config['host']}:{mysql_config['port']}/{mysql_config['database']}")
            s3_client_instance = create_direct_mysql_client(mysql_config)
            print(f"   [EMOJI] [OK] S3 client created successfully")
        except Exception as e:
            print(f"   [EMOJI] [ERROR] Could not create S3 client: {str(e)}")
            import traceback
            traceback.print_exc()
            s3_client_instance = None
    else:
        print(f"   [EMOJI] [OK] Using provided S3 client instance")
    
    try:
        # Check if S3 client is available
        print(f"\n[DEBUG] [EXPORT] Checking S3 client availability...")
        if not s3_client_instance:
            print(f"   [EMOJI] [WARNING]  S3 client not available, using local export only")
            return local_export_fallback(data, file_format, user_id, options)
        print(f"   [EMOJI] [OK] S3 client available")
        
        # Validate format
        print(f"\n[OK] [EXPORT] Validating export format...")
        all_supported_formats = ['json', 'csv', 'xml', 'txt', 'pdf', 'xlsx']
        ...
        print(f"   [EMOJI] Format supported by microservice: {format_supported_by_microservice}")
        print(f"   [EMOJI] Record count: {record_count:,}")
        print(f"   [EMOJI] Data size: {data_size_mb:.2f} MB")
        print(f"   [EMOJI] Threshold check: records > 1000? {record_count > 1000}, size > 1MB? {data_size_mb > 1.0}")
        print(f"   [EMOJI] Use local export: {use_local_export}")
        
        if use_local_export:
            # Use local export for unsupported formats OR large datasets to avoid timeout
            print(f"\n[EMOJI] [EXPORT] Using LOCAL EXPORT strategy")
            if not format_supported_by_microservice:
                print(f"   [EMOJI] Reason: Format '{file_format}' not supported by microservice")
            else:
                print(f"   [EMOJI] Reason: Large dataset detected ({record_count:,} records, {data_size_mb:.2f}MB)")
            
            # Export to file locally
            print(f"\n[DOC] [EXPORT] Step 1/3: Converting data to {file_format.upper()} format locally...")
            print(f"   [EMOJI] Filename: {file_name}")
            start_time = datetime.datetime.now()
            print(f"   [EMOJI] Started at: {start_time.strftime('%H:%M:%S')}")
            ...
            try:
                ...
                print(f"   [EMOJI] Calling export function: {export_func.__name__}")
                file_buffer = export_func(data)
                conversion_time = (datetime.datetime.now() - start_time).total_seconds()
                print(f"   [EMOJI] [OK] Conversion completed in {conversion_time:.2f} seconds")
                print(f"   [EMOJI] File size: {len(file_buffer):,} bytes ({len(file_buffer) / (1024*1024):.2f} MB)")
            except Exception as conv_error:
                print(f"   [EMOJI] [ERROR] Conversion failed: {str(conv_error)}")
                import traceback
                traceback.print_exc()
                raise
            
            # Upload to S3 using microservice
            print(f"\n[EMOJI]  [EXPORT] Step 2/3: Uploading file to S3...")
            upload_start = datetime.datetime.now()
            print(f"   [EMOJI] Started at: {upload_start.strftime('%H:%M:%S')}")
            ...
            try:
                ...
                print(f"   [EMOJI] Creating temporary file with extension: .{file_extension}")
                with tempfile.NamedTemporaryFile(delete=False, suffix=f".{file_extension}") as temp_file:
                    temp_file.write(file_buffer)
                    temp_file_path = temp_file.name
                print(f"   [EMOJI] Temporary file created: {temp_file_path}")
                print(f"   [EMOJI] File size: {os.path.getsize(temp_file_path):,} bytes")
                
                # Check microservice health first
                print(f"   [EMOJI] Checking microservice health...")
                try:
                    health_url = f"{s3_client_instance.api_base_url}/health"
                    health_response = requests.get(health_url, timeout=10)
                    if health_response.status_code == 200:
                        print(f"   [EMOJI] [OK] Microservice is reachable")
                    else:
                        print(f"   [EMOJI] [WARNING]  Microservice health check returned status {health_response.status_code}")
                except Exception as health_error:
                    print(f"   [EMOJI] [WARNING]  Microservice health check failed: {str(health_error)}")
                    print(f"   [EMOJI] [WARNING]  Will still attempt upload, but may timeout")
                
                # Upload using S3 microservice
                print(f"   [EMOJI] Calling S3 upload...")
                print(f"   [EMOJI] Temp file: {temp_file_path}")
                print(f"   [EMOJI] File size: {os.path.getsize(temp_file_path):,} bytes")
                print(f"   [EMOJI] User ID: {user_id}")
                print(f"   [EMOJI] Custom filename: {file_name}")
                ...
                try:
                    ...
                    print(f"   [EMOJI] Upload call completed in {upload_call_time:.2f} seconds")
                except Exception as upload_ex:
                    upload_call_time = (datetime.datetime.now() - upload_call_start).total_seconds()
                    print(f"   [EMOJI] [ERROR] Upload call failed after {upload_call_time:.2f} seconds: {str(upload_ex)}")
                    import traceback
                    traceback.print_exc()
                    raise
                ...
                print(f"   [EMOJI] Temporary file deleted")
                
                upload_time = (datetime.datetime.now() - upload_start).total_seconds()
                print(f"   [EMOJI] Upload completed in {upload_time:.2f} seconds")
                
                if upload_result['success']:
                    file_info = upload_result['file_info']
                    total_duration = (datetime.datetime.now() - start_time).total_seconds()
                    
                    print(f"   [EMOJI] [OK] Upload successful!")
                    print(f"\n[STATS] [EXPORT] Step 3/3: Finalizing...")
                    print(f"   [EMOJI] S3 URL: {file_info['url']}")
                    print(f"   [EMOJI] Stored name: {file_info.get('storedName', file_name)}")
                    print(f"   [EMOJI] Total time: {total_duration:.2f} seconds")
                    print(f"   [EMOJI] [OK] Export completed successfully!")
                    
                    total_time = (datetime.datetime.now() - start_time_total).total_seconds()
                    print(f"\n{'='*80}")
                    print(f"[OK] [EXPORT] SUCCESS - Total time: {total_time:.2f} seconds")
                    print(f"{'='*80}\n")
                ...
                upload_time = (datetime.datetime.now() - upload_start).total_seconds()
                print(f"   [EMOJI] [ERROR] S3 upload failed after {upload_time:.2f} seconds: {str(s3_error)}")
                import traceback
                traceback.print_exc()
                print(f"\n[EMOJI] [EXPORT] Falling back to local export...")
                # Fallback to local save
                return local_export_fallback(data, file_format, user_id, options)
        
        else:
            # Use microservice directly for small supported formats
            print(f"\n[API] [EXPORT] Using MICROSERVICE DIRECT strategy")
            print(f"   [EMOJI] Reason: Small dataset ({record_count:,} records, {data_size_mb:.2f}MB)")
            
            # Export using microservice
            print(f"\n[EMOJI] [EXPORT] Calling S3 microservice...")
            start_time = datetime.datetime.now()
            print(f"   [EMOJI] Started at: {start_time.strftime('%H:%M:%S')}")
            print(f"   [EMOJI] Format: {file_format}")
            print(f"   [EMOJI] Filename: {file_name}")
            ...
            try:
                ...
                print(f"   [EMOJI] Microservice call completed in {duration:.2f} seconds")
                
                if export_result['success']:
                    export_info = export_result['export_info']
                    print(f"   [EMOJI] [OK] Export successful!")
                    print(f"\n[STATS] [EXPORT] Finalizing...")
                    print(f"   [EMOJI] S3 URL: {export_info['url']}")
                    print(f"   [EMOJI] File size: {export_info.get('size', 0):,} bytes")
                    print(f"   [EMOJI] [OK] Export completed successfully!")
                    ...
                    print(f"   [EMOJI] [ERROR] Export failed: {error_msg}")
                    total_time = (datetime.datetime.now() - start_time_total).total_seconds()
                    print(f"\n{'='*80}")
                    print(f"[ERROR] [EXPORT] FAILED after {total_time:.2f} seconds")
                    print(f"{'='*80}\n")
                    return {
                        'success': False,
                        'error': error_msg
                ...
                duration = (datetime.datetime.now() - start_time).total_seconds()
                print(f"   [EMOJI] [ERROR] Microservice call failed after {duration:.2f} seconds: {str(microservice_error)}")
                import traceback
                traceback.print_exc()
                print(f"\n[EMOJI] [EXPORT] Falling back to local export...")
                return local_export_fallback(data, file_format, user_id, options)
        ...
        total_time = (datetime.datetime.now() - start_time_total).total_seconds()
        print(f"\n{'='*80}")
        print(f"[ERROR] [EXPORT] EXCEPTION after {total_time:.2f} seconds: {str(e)}")
        print(f"{'='*80}")
        import traceback
        traceback.print_exc()
        print(f"\n[EMOJI] [EXPORT] Falling back to local export...")
...
def local_export_fallback(data, file_format, user_id, options):
    ...
    try:
        print(f"[EMOJI] Using local export fallback for format: {file_format}")
        ...
        print(f"[OK] Local export successful: {local_path}")
        ...
        print(f"[ERROR] Local export fallback failed: {str(e)}")
        import traceback
        traceback.print_exc()
        return {
            'success': False,
...
def test_all_export_formats():
    ...
    print("[INFO] Testing All Export Formats")
    print("[API] Direct URL: http://15.207.1.40:3000")
    print("[STATS] Testing: JSON, CSV, XML, TXT, PDF")
    print("=" * 60)
    ...
    print(f"[STATS] Data: {len(sample_data)} employee records")
    ...
    for i, export_format in enumerate(export_formats, 1):
        ...
        try:
            ...
            print(f"[UPLOAD] Exporting as {export_format.upper()}...")
            ...
            if export_result['success']:
                ...
                print(f"   [DOC] File: {export_result['export_info']['storedName']}")
                print(f"   [SAVE] Size: {export_result['export_info']['size']} bytes")
                print(f"   [LINK] URL: {export_result['export_info']['url']}")
                print(f"   [EMOJI] Operation ID: {export_result['operation_id']}")
                ...
                try:
                    ...
                    if download_response.status_code == 200:
                        print(f"   [EMOJI] Download: SUCCESS ({len(download_response.content)} bytes)")
                        ...
                        print(f"   [SAVE] Saved locally: {local_filename}")
                    else:
    ...
    # Summary report
    print("\n" + "=" * 60)
    print("[STATS] EXPORT TEST SUMMARY")
    print("=" * 60)
    ...
    if successful_formats:
        print(f"   [EMOJI] Working formats: {', '.join(successful_formats)}")
    
    if failed_formats:
        print(f"   [WARNING]  Failed formats: {', '.join(failed_formats)}")
    
    # Show operation history
    print(f"\n[LIST] Recent operation history:")
    ...
    print(f"\n[EMOJI] Export format testing completed!")
    return results

def main():
    """Example usage of Direct S3 Client with MySQL from Django settings"""
    
    print("[INFO] Direct S3 Microservice Client with MySQL")
    print("[API] Direct URL: http://15.207.1.40:3000")
    print("[EMOJI]  Database: MySQL (from Django settings)")
    print("[SECURE] AWS Credentials: Handled by microservice")
    print("=" * 60)
    ...
    print("\n[EMOJI] Render + MySQL integration test completed!")

def test_pdf_processing():
    """Test PDF upload with automatic metadata extraction and summary generation"""
    
    print("[INFO] Testing PDF Processing with OpenAI Summarization")
    print("[API] Direct URL: http://15.207.1.40:3000")
    print("[AI] AI: OpenAI for document summarization")
    print("=" * 60)
    ...
    print(f"   PyPDF2: {'[OK] Available' if PDF_LIBRARY_AVAILABLE else '[ERROR] Not available'}")
    print(f"   pdfplumber: {'[OK] Available' if PDFPLUMBER_AVAILABLE else '[ERROR] Not available'}")
    print(f"   OpenAI: {'[OK] Available' if OPENAI_AVAILABLE else '[ERROR] Not available'}")
    
    if not PDF_LIBRARY_AVAILABLE and not PDFPLUMBER_AVAILABLE:
        print("\n[WARNING]  WARNING: No PDF processing library available!")
        print("   Install PyPDF2 or pdfplumber: pip install PyPDF2 pdfplumber")
        return
    
    if not OPENAI_AVAILABLE:
        print("\n[WARNING]  WARNING: OpenAI library not available!")
        print("   Install OpenAI: pip install openai")
        return
    
    # You would need to provide a test PDF file path
    print("\n3. Upload a PDF file to test...")
    print("   [INFO] Note: Provide a PDF file path to test the feature")
    ...
    if upload_result['success']:
        ...
        print(f"[OK] Upload successful! Operation ID: {operation_id}")
        print(f"[DOC] PDF processing: {upload_result.get('pdf_processing', 'N/A')}")
        ...
        status = client.get_pdf_processing_status(operation_id)
        print(f"\\n[STATS] Processing Status: {status['status']}")
    ...
    print("\n[OK] PDF processing feature is ready to use!")
    print("\n[INFO] Tips:")
...
def test_enhanced_pdf_processing_with_sample(pdf_path: str = None):
    ...
    print("="*80)
    print("[INFO] ENHANCED PDF PROCESSING TEST")
    print("="*80)
    print("\nThis test demonstrates the NEW intelligent PDF processing features:")
    print("[OK] Smart extraction strategy (optimized for cost & time)")
    print("[OK] Comprehensive metadata extraction")
    print("[OK] AI-powered summary generation (GPT-3.5-turbo)")
    print("[OK] Automatic document categorization")
    print("[OK] Full database integration")
    print("="*80)
    ...
    if not pdf_path or not os.path.exists(pdf_path):
        ...
        print("\n[INFO] To test with your own PDF, run:")
        print("""
...
test_enhanced_pdf_processing_with_sample('/path/to/large_manual.pdf')
        """)
        print("\n[OK] Enhanced PDF processing feature is READY and CONFIGURED!")
    ...
    while elapsed < max_wait:
        ...
        if status['status'] == 'completed':
            print(f"   [OK] Processing completed in {elapsed} seconds!")
            break
        elif status['status'] == 'error' or status['status'] == 'failed':
            print(f"   [ERROR] Processing failed: {status.get('message')}")
            return
        else:
    ...
    if status['status'] == 'completed':
        ...
        print("="*80)
        print("\n[STATS] Feature Highlights Demonstrated:")
        print(f"   [OK] Document Size: {metadata.get('document_size_category', 'unknown').upper()}")
        print(f"   [OK] Extraction Strategy: {metadata.get('extraction_strategy', 'unknown')}")
        print(f"   [OK] Pages Processed: {metadata.get('page_count', 'unknown')}")
        print(f"   [OK] AI Model Used: {metadata.get('ai_model', 'unknown')}")
        print(f"   [OK] Auto-Category: {metadata.get('suggested_category', 'unknown')}")
        print(f"   [OK] Summary Generated: Yes ({len(summary)} characters)")
    else:
