...
def process_export_async(self, export_task_id, data, file_format, user_id, options=None, module='general'):
    ...
    try:
        ...
        print(f"\n{'='*80}")
        print(f"[INFO] [ASYNC EXPORT] Starting export task {export_task_id}")
        print(f"   [EMOJI] Module: {module}")
        print(f"   [EMOJI] Format: {file_format}")
        print(f"   [EMOJI] User ID: {user_id}")
        print(f"   [EMOJI] Record count: {len(data) if isinstance(data, list) else 1}")
        print(f"   [EMOJI] Started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"{'='*80}\n")
        ...
        if data_size > max_size:
            ...
            export_task.save()
            print(f"[ERROR] [ASYNC EXPORT] {error_msg}")
            return export_task_id
        
        # For very large datasets, use chunking
        if record_count > 5000:
            print(f"[WARNING]  [ASYNC EXPORT] Large dataset detected ({record_count} records), using chunked processing")
        ...
        if result.get('success'):
            ...
            print(f"\n{'='*80}")
            print(f"[OK] [ASYNC EXPORT] Export completed successfully")
            print(f"   [EMOJI] Task ID: {export_task_id}")
            print(f"   [EMOJI] File URL: {result.get('file_url', 'N/A')}")
            print(f"   [EMOJI] Completed at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
            print(f"{'='*80}\n")
        else:
            export_task.status = 'failed'
            export_task.error = result.get('error', 'Unknown error')
            export_task.save()
            print(f"[ERROR] [ASYNC EXPORT] Export failed: {result.get('error', 'Unknown error')}")
        ...
        if self.request.retries < self.max_retries:
            print(f"[EMOJI] [ASYNC EXPORT] Retrying export task (attempt {self.request.retries + 1}/{self.max_retries})")
...
def process_large_export_chunked(export_task, data, file_format, user_id, options):
    ...
    try:
        ...
        print(f"[FILE] [ASYNC EXPORT] Processing {total_records} records in chunks of {chunk_size}")
...
def create_export_task(user_id, file_format, module='general', export_data_dict=None, framework_id=None):
    ...
    try:
        ...
        print(f"[ERROR] [ASYNC EXPORT] Error creating export task: {str(e)}")
        import traceback
        traceback.print_exc()
        raise
