...
class AzureEmailSender:
    ...
    def __init__(self):
        ...
        if self.is_configured():
            ...
            logger.info(f"  Client Secret: {'[EMOJI] Set' if self.client_secret else '[EMOJI] Not set'}")
            logger.info(f"  From Email: {self.from_email}")
        else:
            logger.info("AzureEmailSender initialized but not fully configured (will use SMTP fallback)")
            logger.info(f"  Tenant ID: {'[EMOJI] Set' if self.tenant_id else '[EMOJI] Missing'}")
            logger.info(f"  Client ID: {'[EMOJI] Set' if self.client_id else '[EMOJI] Missing'}")
            logger.info(f"  Client Secret: {'[EMOJI] Set' if self.client_secret else '[EMOJI] Missing'}")
            logger.info(f"  From Email: {'[EMOJI] Set' if self.from_email else '[EMOJI] Missing'}")
    ...
    def send_email_via_graph(self, to_email, subject, html_body, from_email=None, from_name=None):
        ...
        try:
            ...
            if response.status_code == 202:
                ...
                logger.info(f"[AZURE] [OK] Email accepted by Graph API (202 Accepted) - to: {to_email}")
                return True
            elif response.status_code == 200:
                logger.info(f"[AZURE] [OK] Email sent successfully via Graph API (200 OK) - to: {to_email}")
                return True
            else:
                # Log detailed error information
                error_detail = response.text
                logger.error(f"[AZURE] [ERROR] Graph API returned status {response.status_code}")
    ...
    def is_configured(self):
        ...
        if not has_creds:
            ...
            logger.warning(f"  Tenant ID: {'[EMOJI]' if self.tenant_id else '[EMOJI]'}")
            logger.warning(f"  Client ID: {'[EMOJI]' if self.client_id else '[EMOJI]'}")
            logger.warning(f"  Client Secret: {'[EMOJI]' if self.client_secret else '[EMOJI]'}")
...
class NotificationService:
    def __init__(self):
        ...
        if self.azure_email_sender.is_configured():
            logger.info("[NOTIFICATION SERVICE INIT] [OK] Azure email will be used as PRIMARY method")
            logger.info(f"[NOTIFICATION SERVICE INIT] Azure From Email: {self.azure_email_sender.from_email}")
        else:
            logger.warning("[NOTIFICATION SERVICE INIT] [WARNING] Azure email NOT configured - will use SMTP only")
    ...
    def init_templates(self):
        ...
        self.email_templates = {
            ...
            'passwordExpired': {
                'subject': '[WARNING] URGENT: Your Password Has Expired - {platform_name}',
                'template': lambda user_name, days_since_expiry, platform_name: f"""
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff;">
                  <div style="background-color: #e74c3c; padding: 20px; text-align: center;">
                    <h1 style="color: #ffffff; margin: 0; font-size: 28px;">[WARNING] Password Expired</h1>
                  </div>
                  <div style="padding: 20px;">
                    <p style="color: #333333; font-size: 16px;">Hello {user_name},</p>
            ...
            'policyAcknowledgementRequired': {
                ...
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff; border: 1px solid #e0e0e0;">
                  <div style="background-color: #4CAF50; padding: 25px; text-align: center;">
                    <h1 style="color: #ffffff; margin: 0; font-size: 28px;">[LIST] Policy Acknowledgement Required</h1>
                  </div>
    ...
    def send_email(self, to, email_type, notification_type, template_data):
        ...
        try:
            ...
            if azure_configured:
                ...
                logger.info(f"[EMAIL START] [WARNING] email_type parameter '{email_type}' will be IGNORED - using Azure instead")
            logger.info("=" * 60)
            ...
            if azure_configured:
                logger.info("=" * 60)
                logger.info("[EMAIL STATUS] [EMOJI] Attempting to send email via Azure AD Graph API")
                ...
                logger.info(f"[AZURE] [OK] Using Azure AD registered email as sender: {from_email}")
                ...
                if success:
                    logger.info("=" * 60)
                    logger.info("[EMAIL STATUS] [OK] Email sent successfully via AZURE GRAPH API")
                    ...
                    logger.warning("=" * 60)
                    logger.warning("[EMAIL STATUS] [WARNING] Azure Graph API email sending failed, falling back to SMTP")
                    logger.warning("=" * 60)
            
            # Fallback to SMTP only if Azure is not configured or failed
            if not self.azure_email_sender.is_configured():
                logger.warning("=" * 60)
                logger.warning("[EMAIL STATUS] [WARNING] Azure not configured - using SMTP")
                logger.warning("=" * 60)
            else:
                logger.info("=" * 60)
                logger.info("[EMAIL STATUS] [EMOJI] Using SMTP fallback for email sending (Azure failed)")
            ...
            try:
                ...
                error_msg = (
                    ...
                    f"[WARNING] NOTE: Azure email should be used instead. Check Azure AD configuration."
                )
                logger.error(error_msg)
            ...
            with smtplib.SMTP(config['host'], config['port']) as server:
                ...
                try:
                    ...
                    logger.info("=" * 60)
                    logger.info("[EMAIL STATUS] [OK] Email sent successfully via SMTP")
                    logger.info(f"[EMAIL STATUS] Recipient: {to}")
    ...
    def check_email_configuration(self):
        ...
        if self.azure_email_sender.is_configured():
            ...
            config_status["azure_details"] = {
                "tenant_id": "[EMOJI] Set" if self.azure_email_sender.tenant_id else "[EMOJI] Missing",
                "client_id": "[EMOJI] Set" if self.azure_email_sender.client_id else "[EMOJI] Missing",
                "client_secret": "[EMOJI] Set" if self.azure_email_sender.client_secret else "[EMOJI] Missing",
                "from_email": self.azure_email_sender.from_email or "[EMOJI] Missing"
            }
            config_status["recommended_method"] = "azure_graph_api"
        else:
            config_status["azure_details"] = {
                "tenant_id": "[EMOJI] Missing",
                "client_id": "[EMOJI] Missing",
                "client_secret": "[EMOJI] Missing",
                "from_email": "[EMOJI] Missing",
        ...
        for email_type, config in self.email_configs.items():
            if config.get('auth', {}).get('user') and config.get('auth', {}).get('pass'):
                ...
                config_status["smtp_details"][email_type] = {
                    ...
                    "user": "[EMOJI] Set" if config.get('auth', {}).get('user') else "[EMOJI] Missing",
                    "password": "[EMOJI] Set" if config.get('auth', {}).get('pass') else "[EMOJI] Missing"
                }
    ...
    def print_email_configuration_status(self):
        ...
        if status["azure_configured"]:
            logger.info("  Status: [OK] CONFIGURED - Will attempt to use Azure Graph API first")
            for key, value in status["azure_details"].items():
                logger.info(f"    {key}: {value}")
        else:
            logger.info("  Status: [ERROR] NOT CONFIGURED - Will use SMTP fallback")
            for key, value in status["azure_details"].items():
                logger.info(f"    {key}: {value}")
        
        logger.info("\n[SMTP FALLBACK]")
        if status["smtp_configured"]:
            logger.info("  Status: [OK] CONFIGURED")
            for email_type, details in status["smtp_details"].items():
                logger.info(f"    {email_type.upper()}:")
                for key, value in details.items():
                    logger.info(f"      {key}: {value}")
        else:
            logger.info("  Status: [ERROR] NOT CONFIGURED")
        
        logger.info(f"\n[RECOMMENDED METHOD]")
        if status["recommended_method"] == "azure_graph_api":
            logger.info("  [OK] Using: Azure Graph API (Primary)")
        elif status["recommended_method"] == "smtp":
            logger.info("  [WARNING]  Using: SMTP (Fallback)")
        else:
            logger.info("  [ERROR] ERROR: No email method configured!")
    ...
    def test_email_sending(self, test_email=None):
        ...
        logger.info("=" * 70)
        logger.info(f"Configuration: {'[OK] Azure' if config_status['azure_configured'] else '[WARNING] SMTP Only'}")
        logger.info(f"Test Email Sent: {'[OK] Yes' if test_result['test_email_sent'] else '[ERROR] No'}")
