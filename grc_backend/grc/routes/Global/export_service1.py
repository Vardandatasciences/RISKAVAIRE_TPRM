    ...
    try:
        ...
        try:
            ...
            try:
                ...
                print(f"[ERROR] Both xlsxwriter and openpyxl not available. Error: {openpyxl_error}")
...
def export_data(data=None, file_format='xlsx', user_id='user123', options=None):
    ...
    print(f"[STATS] Export data validation:")
    ...
    try:
        ...
        if not s3_client:
            print("[WARNING]  S3 microservice client not available, using local export fallback")
        ...
        if use_local_export:
            ...
            print(f"[FOLDER] Final filename will be: {file_name}")
...
def test_export_service():
    ...
    print("[EMOJI] Testing updated export service with S3 microservice...")
    ...
    try:
        ...
        if result['success']:
            ...
            print(f"[ERROR] JSON export failed: {result['error']}")
        
        # Test XLSX export (requires local export + upload)
        print("\n[STATS] Testing XLSX export...")
        result = export_data(test_data, 'xlsx', 'test_user', {'file_name': 'test_export.xlsx'})
        
        if result['success']:
            print(f"[OK] XLSX export successful!")
            print(f"   File URL: {result['file_url']}")
            print(f"   File name: {result['file_name']}")
            print(f"   Method: {result['metadata']['method']}")
        else:
            print(f"[ERROR] XLSX export failed: {result['error']}")
