...
from ...rbac.permissions import (
    AuditReviewPermission, AuditConductPermission, AuditViewPermission
)
...
def upload_to_s3(file_path: str, bucket_name: str, s3_file_name: str) -> Optional[str]:
    ...
    try:
        ...
        # Upload file
        s3_client.upload_file(file_path, aws_bucket, module='Audit', s3_file_name=s3_file_name)
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute("""
                INSERT INTO audit_version (
                    AuditId, Version, ExtractedInfo, 
                    UserId, Date, ActiveInactive, ApprovedRejected, FrameworkId
...
def extract_risks_and_mitigations_from_json(json_data: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    """
    Extract selected risks and mitigations from JSON data structure.
    Returns a dictionary with compliance_id as key and extracted data as value.
    """
    extracted_data = {}
    ...
    for key, value in json_data.items():
        ...
        if isinstance(value, dict):
            ...
            extracted_data[compliance_id] = {
                ...
                'compliance_status': value.get('compliance_status', '0'),
                'review_status': value.get('review_status', ''),
                'review_comments': value.get('review_comments', ''),
                'evidence': value.get('evidence', ''),
                'criticality': value.get('criticality', ''),
                'severity_rating': value.get('severity_rating', ''),
                'impact': value.get('impact', ''),
                'comments': value.get('comments', ''),
                'how_to_verify': value.get('how_to_verify', ''),
                'details_of_finding': value.get('details_of_finding', ''),
                'underlying_cause': value.get('underlying_cause', ''),
                'why_to_verify': value.get('why_to_verify', ''),
                'what_to_verify': value.get('what_to_verify', ''),
                'suggested_action_plan': value.get('suggested_action_plan', ''),
                'responsible_for_plan': value.get('responsible_for_plan', ''),
                'mitigation_date': value.get('mitigation_date', ''),
                're_audit': value.get('re_audit', False),
                're_audit_date': value.get('re_audit_date', ''),
                'recommendation': value.get('recommendation', '')
            }
    
    return extracted_data

def _convert_compliance_status_to_check(compliance_status):
    """
    Convert compliance_status (text or numeric) to numeric Check value.
    Returns: '0' (Not Compliant), '1' (Partially Compliant), '2' (Fully Compliant), or '3' (Not Applicable)
    """
    if not compliance_status:
        return '0'  # Default to Not Compliant
    
    # If already a valid numeric string, return as-is
    if isinstance(compliance_status, str) and compliance_status in ['0', '1', '2', '3']:
        return compliance_status
    
    # If it's a number, convert to string
    if isinstance(compliance_status, (int, float)):
        int_val = int(compliance_status)
        return str(max(0, min(3, int_val)))  # Clamp between 0-3
    
    # Convert text values to numeric
    status_upper = str(compliance_status).upper().strip()
    
    if status_upper in ['NOT_COMPLIANT', 'NOT COMPLIANT', 'NON_COMPLIANT', 'NON COMPLIANT', '0']:
        return '0'  # Not Compliant
    elif status_upper in ['PARTIALLY_COMPLIANT', 'PARTIALLY COMPLIANT', '1']:
        return '1'  # Partially Compliant
    elif status_upper in ['COMPLIANT', 'FULLY_COMPLIANT', 'FULLY COMPLIANT', '2']:
        return '2'  # Fully Compliant
    elif status_upper in ['NOT_APPLICABLE', 'NOT APPLICABLE', '3']:
        return '3'  # Not Applicable
    else:
        # Try to parse as number
        try:
            int_val = int(float(compliance_status))
            return str(max(0, min(3, int_val)))  # Clamp between 0-3
...
def _call_ai_api_for_incident_fields(prompt):
    """
    Call AI API to generate comprehensive incident fields.
    Uses the same AI infrastructure as audit findings.
    """
    try:
...
def generate_comprehensive_incident_fields(audit_finding_data, compliance_data, audit_data):
    """
    Generate comprehensive incident fields using AI based on audit finding data.
    
    Args:
        audit_finding_data: Dict containing audit finding fields (Impact, Recommendation, DetailsOfFinding, etc.)
        compliance_data: Dict containing compliance information (ComplianceTitle, ComplianceItemDescription, etc.)
        audit_data: Dict containing audit information (Title, Scope, Objective, BusinessUnit, etc.)
    
    Returns:
        dict: Comprehensive incident fields generated by AI
    """
    try:
        # Extract key information
        compliance_title = compliance_data.get('ComplianceTitle', 'N/A')
        compliance_desc = compliance_data.get('ComplianceItemDescription', 'N/A')
        check_status = audit_finding_data.get('Check', '0')
        impact = audit_finding_data.get('Impact', '')
        recommendation = audit_finding_data.get('Recommendation', '')
        details_of_finding = audit_finding_data.get('DetailsOfFinding', '')
        major_minor = audit_finding_data.get('MajorMinor', '0')
        comments = audit_finding_data.get('Comments', '')
        underlying_cause = audit_finding_data.get('UnderlyingCause', '')
        evidence = audit_finding_data.get('Evidence', '')
        
        audit_title = audit_data.get('Title', 'N/A')
        audit_scope = audit_data.get('Scope', 'N/A')
        audit_objective = audit_data.get('Objective', 'N/A')
        business_unit = audit_data.get('BusinessUnit', 'N/A')
        ...
        # Note: MajorMinor is passed to AI for analysis, but Criticality will be determined by AI
        
        # Create comprehensive AI prompt
        prompt = f"""You are an expert GRC incident management specialist. Based on the audit finding data below, generate comprehensive incident fields.

=== COMPLIANCE REQUIREMENT ===
Title: {compliance_title}
Description: {compliance_desc[:1000] if len(compliance_desc) > 1000 else compliance_desc}

=== AUDIT CONTEXT ===
Audit Title: {audit_title}
Audit Objective: {audit_objective[:500] if len(audit_objective) > 500 else audit_objective}
Audit Scope: {audit_scope[:500] if len(audit_scope) > 500 else audit_scope}
Business Unit: {business_unit}

=== AUDIT FINDING DETAILS ===
Compliance Status: {compliance_status}
Major/Minor: {"Major" if major_minor == '1' else "Minor" if major_minor == '0' else "Not Applicable"}
Impact: {impact[:1000] if len(impact) > 1000 else impact}
Recommendation: {recommendation[:1000] if len(recommendation) > 1000 else recommendation}
Details of Finding: {details_of_finding[:1000] if len(details_of_finding) > 1000 else details_of_finding}
Underlying Cause: {underlying_cause[:1000] if len(underlying_cause) > 1000 else underlying_cause}
Evidence: {evidence[:1000] if len(evidence) > 1000 else evidence}
Comments: {comments[:500] if len(comments) > 500 else comments}

=== YOUR TASK ===
Generate comprehensive incident fields in JSON format. Return ONLY valid JSON (no markdown, no explanations).

Required fields:
1. **IncidentTitle**: Create a concise, descriptive title for this incident based on the compliance requirement and finding details
2. **Description**: Comprehensive description of the incident, combining compliance requirement, finding details, and impact
3. **PossibleDamage**: Detailed analysis of potential damage including financial, operational, reputational, and regulatory consequences
4. **RiskCategory**: Categorize the risk (e.g., "Compliance Risk", "Operational Risk", "Regulatory Risk", "Security Risk", "Financial Risk")
5. **IncidentCategory**: Categorize the incident type (e.g., "Non-Compliance", "Control Failure", "Policy Violation", "Regulatory Breach")
6. **RiskPriority**: Priority level ("Low", "Medium", "High", "Critical") - analyze based on impact, severity, and business criticality
7. **Criticality**: Analyze and determine criticality level ("Low", "Medium", "High", "Critical") based on the finding's severity, impact, and business risk - DO NOT use the Major/Minor field directly, analyze independently
8. **InitialImpactAssessment**: Comprehensive assessment of the initial impact including financial, operational, reputational, and regulatory impacts
9. **AffectedBusinessUnit**: Use the provided business unit: "{business_unit}" or infer from context
10. **SystemsAssetsInvolved**: Identify systems, assets, or processes affected by this non-compliance
11. **RelevantPoliciesProceduresViolated**: List relevant policies or procedures that were violated
12. **ControlFailures**: Describe control failures that led to this non-compliance
13. **LessonsLearned**: Key lessons learned from this finding
14. **IncidentClassification**: Classify the incident (e.g., "Compliance Violation", "Control Deficiency", "Policy Non-Adherence")

Return JSON in this exact format:
{{
    "IncidentTitle": "Concise descriptive title...",
    "Description": "Comprehensive description...",
    "PossibleDamage": "Detailed potential damage analysis...",
    "RiskCategory": "Compliance Risk",
    "IncidentCategory": "Non-Compliance",
    "RiskPriority": "High",
    "Criticality": "High",
    "InitialImpactAssessment": "Detailed impact assessment...",
    "AffectedBusinessUnit": "{business_unit}",
    "SystemsAssetsInvolved": "Systems and assets affected...",
    "RelevantPoliciesProceduresViolated": "Policies/procedures violated...",
    "ControlFailures": "Control failures identified...",
    "LessonsLearned": "Key lessons learned...",
    "IncidentClassification": "Compliance Violation"
}}

CRITICAL RULES:
- All fields MUST be filled with actual, specific content based on the provided data
- IncidentTitle should be concise (max 100 characters) and descriptive
- Description should comprehensively explain the incident, combining all relevant information
- PossibleDamage should analyze potential consequences in detail
- Criticality MUST be analyzed independently based on severity, impact, and risk - do NOT simply map Major/Minor
- RiskPriority should be determined by analyzing the overall risk level, not just Major/Minor
- Base your analysis on the provided impact, recommendation, details, and underlying cause
- Be specific and actionable in all assessments
- All fields should be related and consistent with each other
- If information is not available, use reasonable inferences based on the compliance requirement and audit context

Return JSON now:"""
        
        # Call AI API
        print(f"[AI] Generating comprehensive incident fields for compliance: {compliance_title[:50]}...")
        ai_response = _call_ai_api_for_incident_fields(prompt)
        
        if not ai_response:
            raise Exception("AI API returned no response")
        ...
        try:
            ...
            print(f"[OK] Successfully generated comprehensive incident fields. Keys: {list(incident_fields.keys())}")
            return incident_fields
        except json.JSONDecodeError as json_err:
            print(f"[ERROR] JSON parsing failed: {json_err}")
            print(f"[ERROR] Response content (first 500 chars): {cleaned_response[:500]}")
            raise
        
    except Exception as e:
        print(f"[ERROR] Error generating comprehensive incident fields: {e}")
        import traceback
        traceback.print_exc()
...
def create_incidents_for_findings(audit_id: int) -> None:
    ...
    try:
        with connection.cursor() as cursor:
            ...
            audit_data = {
                'Title': audit_row[0] if audit_row and audit_row[0] else 'N/A',
                'Scope': audit_row[1] if audit_row and audit_row[1] else 'N/A',
                'Objective': audit_row[2] if audit_row and audit_row[2] else 'N/A',
                'BusinessUnit': audit_row[3] if audit_row and audit_row[3] else 'N/A',
                'FrameworkId': audit_row[4] if audit_row and audit_row[4] else None
            } if audit_row else {
                'Title': 'N/A',
                'Scope': 'N/A',
                'Objective': 'N/A',
            ...
            # Get relevant audit findings and compliance details with more fields
            cursor.execute("""
                SELECT 
                    af.ComplianceId,
                    af.Check,
            ...
            for finding in findings:
                ...
                comments = finding[2]
                user_id = finding[3]
                framework_id = finding[4] or audit_data.get('FrameworkId')
                impact = finding[5] or ''
                recommendation = finding[6] or ''
                details_of_finding = finding[7] or ''
                major_minor = finding[8] or '0'
                evidence = finding[9] or ''
                underlying_cause = finding[10] or ''
                severity_rating = finding[11]
                predictive_risks = finding[12]
                corrective_actions = finding[13]
                compliance_title = finding[14]
                compliance_desc = finding[15] or ''
                ...
                mitigation_value = mitigation
                if isinstance(mitigation, (dict, list)):
                    mitigation_value = json.dumps(mitigation)
                elif mitigation and not isinstance(mitigation, str):
                    mitigation_value = str(mitigation)
                
                # Only use AI for AI audits
                if is_ai_audit:
                    # Prepare audit finding data for AI
                    audit_finding_data = {
                        'Check': check_status,
                        'Impact': impact,
                        'Recommendation': recommendation,
                        'DetailsOfFinding': details_of_finding,
                        'MajorMinor': major_minor,
                        'Evidence': evidence,
                        'UnderlyingCause': underlying_cause,
                        'Comments': comments,
                        'SeverityRating': severity_rating,
                        'PredictiveRisks': predictive_risks,
                    ...
                    compliance_data = {
                        ...
                        'ComplianceItemDescription': compliance_desc
                    }
                    
                    # Generate comprehensive incident fields using AI (only for AI audits)
                    print(f"[AI] Generating AI-powered incident fields for AI audit ComplianceId {compliance_id}...")
                    try:
                        ...
                        print(f"[OK] AI generated {len(ai_generated_fields)} incident fields")
                        ...
                        """, [
                            ...
                            ai_generated_fields.get('Description', compliance_desc or details_of_finding[:500] if details_of_finding else "Non-compliance identified during audit"),
                            ai_generated_fields.get('PossibleDamage', possible_damage or impact[:500] if impact else ""),
                            mitigation_value,
                            framework_id,
                            audit_id,
                            compliance_id,
                            ...
                            comments or "",
                            "Open",
                            ai_generated_fields.get('RiskCategory'),
                            ai_generated_fields.get('IncidentCategory'),
                            ai_generated_fields.get('RiskPriority'),
                            ai_generated_fields.get('Criticality'),
                            ai_generated_fields.get('InitialImpactAssessment'),
                            ai_generated_fields.get('AffectedBusinessUnit'),
                            ai_generated_fields.get('SystemsAssetsInvolved'),
                            "North America",  # Hardcoded as requested
                            ai_generated_fields.get('RelevantPoliciesProceduresViolated'),
                        ...
                        print(f"[OK] Created comprehensive incident for ComplianceId {compliance_id} in AI audit {audit_id} with AI-generated fields")
                    except Exception as ai_err:
                        print(f"[ERROR] AI generation failed for AI audit: {str(ai_err)}")
                        import traceback
                        traceback.print_exc()
                        # Skip creating incident if AI fails for AI audit
                        print(f"[WARNING] Skipping incident creation for ComplianceId {compliance_id} due to AI generation failure")
                        continue
                else:
                    ...
                    print(f"[OK] Created incident for ComplianceId {compliance_id} in AuditId {audit_id}")
            ...
            with connection.cursor() as cursor:
                ...
                if json_data:
                    ...
                    for compliance_id, data in extracted_data.items():
                        ...
                        selected_mitigations = data['selected_mitigations']
                        ...
                        """, [
                            ...
                            data.get('why_to_verify', ''),
                            data.get('what_to_verify', ''),
                            data.get('suggested_action_plan', ''),
                            data.get('mitigation_date'),
                            data.get('responsible_for_plan', ''),
                            data.get('re_audit', 0),
                            data.get('re_audit_date'),
                            data.get('recommendation', ''),
                            # Convert compliance_status to numeric Check value (0=Not Compliant, 1=Partially Compliant, 2=Fully Compliant)
                            _convert_compliance_status_to_check(data.get('compliance_status', '0')),
                            audit_id,
                            compliance_id
                        ])
                    ...
                    for review in compliance_reviews:
                        ...
                        selected_risks = review.get('predictive_risks', [])
                ...
                if version_row:
                    ...
                    metadata = {
                        ...
                        'overall_status': 'Completed',  # Show "Completed" when reviewer approves
...
def test_json_extraction(request):
    ...
    try:
        ...
        result = {
            ...
            'summary': {
                'total_compliances': len(extracted_data),
                'total_risks': sum(len(data['selected_risks']) for data in extracted_data.values()),
                'total_mitigations': sum(len(data['selected_mitigations']) for data in extracted_data.values())
            }
        }
