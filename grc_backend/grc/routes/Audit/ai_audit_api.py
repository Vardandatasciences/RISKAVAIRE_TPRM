...
def trigger_database_analysis(request, audit_id):
    ...
    try:
        logger.info(f"[DEBUG] Manual database analysis triggered for audit {audit_id}")
        ...
        logger.info(f"[LIST] Audit {audit_id}: {audit_title}, Framework: {framework_id}, Status: {audit_status}")
        ...
        def run_database_analysis():
            try:
                logger.info(f"[INFO] Starting database analysis for audit {audit_id} (framework {framework_id})")
                ...
                logger.info(f"[SAVE] Found {db_count} database records across all tables for framework {framework_id}")
                
                if db_count == 0:
                    logger.warning(f"[WARNING] No database records found for framework {framework_id}")
                    return
                ...
                logger.info(f"[OK] Database analysis completed. Found {len(db_results)} tables with relevant records")
                ...
                logger.info(f"[OK] Database analysis and evidence creation completed for audit {audit_id}")
                
            except Exception as e:
                logger.error(f"[ERROR] Error in database analysis for audit {audit_id}: {str(e)}", exc_info=True)
        ...
        logger.error(f"[ERROR] Error triggering database analysis: {str(e)}", exc_info=True)
        return JsonResponse({
            'success': False,
...
def _call_ollama_api(prompt, audit_id=None, document_id=None, model_type='compliance'):
    ...
    logger.info(f"[AI] Calling Ollama at {base_url} with model: {model}, temperature: {temperature}")
    ...
    try:
        ...
        content = content or ""
        logger.info(f"[AI] TinyLlama/Ollama response length: {len(content)} characters")
        return content
            ...
            for temp_file in temp_files:
                try:
                    if os.path.exists(temp_file):
                        ...
                        logger.info(f"[EMOJI] Cleaned up temporary file: {temp_file}")
                except Exception as temp_e:
                    logger.warning(f"[WARNING] Could not delete temporary file {temp_file}: {temp_e}")
        
        return cleaned_files
        
    except Exception as e:
        logger.warning(f"[WARNING] Error during temporary file cleanup: {e}")
        return []
...
class AIAuditDocumentUploadView(View):
    ...
    def post(self, request, audit_id):
        ...
        try:
            ...
            print("=" * 80)
            logger.info("[INFO][INFO][INFO] AI AUDIT UPLOAD ENDPOINT CALLED - NEW CODE VERSION [INFO][INFO][INFO]")
            logger.info(f"[UPLOAD] Upload request for audit {audit_id}")
            logger.info(f"[UPLOAD] Upload audit_id type: {type(audit_id)}")
            logger.info(f"[UPLOAD] Upload audit_id value: '{audit_id}'")
            ...
            logger.info(f"[UPLOAD] Authenticated user ID: {user_id}")
            ...
            try:
                ...
                if audit_row:
                    ...
                    logger.info(f"[OK] Found audit {audit_id} in audit table with type: {audit_type}")
                else:
                    logger.error(f"[ERROR] Audit {audit_id} not found in audit table")
                    return JsonResponse({
                        'success': False,
                        'error': f'Audit {audit_id} not found. Please ensure the audit exists before uploading documents.'
                    }, status=404)
            except Exception as e:
                logger.error(f"[ERROR] Error validating audit {audit_id}: {e}")
                return JsonResponse({
                    'success': False,
            ...
            mappings = []
            logger.info(f"[UPLOAD] Received mappings_json: {mappings_json}")
            if mappings_json:
                try:
                    import json
                    ...
                    if not isinstance(mappings, list):
                        mappings = []
                    logger.info(f"[UPLOAD] Parsed {len(mappings)} mapping(s) from JSON")
                except Exception as e:
                    logger.warning(f"[WARNING] Failed to parse mappings JSON: {e}")
                    mappings = []
            ...
            if not mappings and policy_id:
                ...
                logger.info(f"[UPLOAD] Using single mapping: policy_id={policy_id}, subpolicy_id={subpolicy_id}")
            elif not mappings:
                # Default: no mapping
                mappings = [{'policy_id': None, 'subpolicy_id': None}]
                logger.info("[UPLOAD] No mappings provided, using default (no mapping)")
            
            logger.info(f"[UPLOAD] Final mappings count: {len(mappings)}")
            ...
            if file_operation_id:
                ...
                logger.info(f"[UPLOAD] Reusing file from file_operations ID: {file_operation_id}")
                try:
                    with connection.cursor() as cursor:
                        cursor.execute("""
                        ...
                        logger.info(f"[UPLOAD] Found file: file_name={file_name} (from stored_name={stored_name}, file_name_db={file_name_from_db}, original_name={original_name}), s3_url: {aws_file_link}, s3_key: {s3_key}, file_size: {file_size}")
                        ...
                        try:
                            ...
                            if framework_row and framework_row[0]:
                                ...
                                logger.info(f"[LIST] Looking for documents_analysis.json at: {json_path}")
                                ...
                                if os.path.exists(json_path):
                                    try:
                                        ...
                                        logger.info(f"[LIST] Found {len(documents_in_json)} document(s) in documents_analysis.json - will process ALL documents with matched compliances")
                                        ...
                                        for doc_entry in documents_in_json:
                                            ...
                                            if matched_compliances_for_doc and doc_operation_id:
                                                ...
                                                logger.info(f"[LIST] Document operation_id={doc_operation_id} has {len(matched_compliances_for_doc)} matched compliances: {matched_compliances_for_doc} - will be processed")
                                            elif doc_operation_id:
                                                logger.info(f"[LIST] Document operation_id={doc_operation_id} has NO matched compliances - skipping")
                                        
                                        logger.info(f"[OK][OK][OK] WILL PROCESS {len(all_relevant_documents_to_process)} relevant document(s) from JSON")
                                        
                                    except Exception as json_err:
                                        logger.warning(f"[WARNING] Could not read/parse documents_analysis.json: {json_err}")
                                        import traceback
                                        logger.warning(f"[WARNING] Error details: {traceback.format_exc()}")
                                else:
                                    logger.info(f"ℹ[EMOJI] documents_analysis.json not found at {json_path}, will check database instead")
                            else:
                                logger.warning(f"[WARNING] Could not get framework_id for audit {audit_id}")
                        except Exception as json_read_err:
                            logger.warning(f"[WARNING] Error reading documents_analysis.json: {json_read_err}")
                        ...
                        if not all_relevant_documents_to_process:
                            logger.info(f"[LIST] No relevant documents found in JSON - will process only the uploaded document (file_operation_id={file_operation_id})")
                            ...
                            try:
                                ...
                                if not relevance_row or not relevance_row[0]:
                                    logger.info(f"[LIST] No relevance found by file_operation_id={file_operation_id}, checking by s3_key/stored_name...")
                                    ...
                                    if file_key:
                                        ...
                                        if other_operation:
                                            ...
                                            logger.info(f"[LIST] Found other file_operation_id={other_op_id} with same s3_key/stored_name, checking relevance...")
                                            ...
                                            relevance_row = cursor.fetchone()
                                            if relevance_row:
                                                logger.info(f"[OK] Found relevance record for same file (different module upload) - file_operation_id={other_op_id}")
                                ...
                                if relevance_row and relevance_row[0]:
                                    try:
                                        ...
                                        if matched_compliances_for_uploaded:
                                            ...
                                            logger.info(f"[OK][OK][OK] MATCHED COMPLIANCES FOUND in database for file_operation_id={file_operation_id}, audit_id={audit_id}")
                                            logger.info(f"[LIST] Found {len(matched_compliances_for_uploaded)} matched compliances from database: {matched_compliances_for_uploaded}")
                                    except Exception as e:
                                        logger.warning(f"[WARNING] Could not parse matched_compliances from relevance analysis: {e}")
                                        import traceback
                                        logger.warning(f"[WARNING] Error details: {traceback.format_exc()}")
                                else:
                                    logger.warning(f"[ERROR][ERROR][ERROR] NO MATCHED COMPLIANCES FOUND in document_audit_relevance for file_operation_id={file_operation_id}, audit_id={audit_id}")
                                    logger.warning(f"[ERROR] relevance_row: {relevance_row}")
                            except Exception as e:
                                logger.warning(f"[ERROR][ERROR][ERROR] ERROR looking up matched compliances for file_operation_id {file_operation_id} and audit {audit_id}: {e}")
                                import traceback
                                logger.warning(f"[ERROR] Error details: {traceback.format_exc()}")
                                ...
                                if relevance_row and relevance_row[0]:
                                    try:
                                        ...
                                        if matched_compliances_for_uploaded and len(matched_compliances_for_uploaded) > 0:
                                            all_relevant_documents_to_process.append((file_operation_id, matched_compliances_for_uploaded))
                                            logger.info(f"[OK] Added uploaded document (file_operation_id={file_operation_id}) with {len(matched_compliances_for_uploaded)} matched compliances")
                                        else:
                                            logger.info(f"⏭[EMOJI] Skipping uploaded document (file_operation_id={file_operation_id}) - no matched compliances (not relevant)")
                                    except Exception as e:
                                        logger.warning(f"[WARNING] Could not parse matched_compliances: {e}")
                                        logger.info(f"⏭[EMOJI] Skipping uploaded document (file_operation_id={file_operation_id}) - invalid relevance data")
                                else:
                                    logger.info(f"⏭[EMOJI] Skipping uploaded document (file_operation_id={file_operation_id}) - no relevance found (not relevant)")
                            except Exception as e:
                                logger.warning(f"[WARNING] Error checking relevance for uploaded document: {e}")
                                logger.info(f"⏭[EMOJI] Skipping uploaded document (file_operation_id={file_operation_id}) - cannot verify relevance")
                        
                        # STEP 4: Log what documents we found
                        logger.info(f"[EMOJI] Found {len(all_relevant_documents_to_process)} relevant document(s) to process from JSON:")
                    ...
                    logger.error(f"[ERROR] Error fetching file_operation {file_operation_id}: {e}")
                    return JsonResponse({
                        'success': False,
                ...
                logger.info(f"[UPLOAD] S3 file metadata: s3_key={s3_key}, stored_name={stored_name}, aws_file_link={aws_file_link}")
                ...
                logger.info(f"[UPLOAD] Creating metadata record for already uploaded file: {file_name}")
            else:
                ...
                logger.info(f"[UPLOAD] Uploading file: {file.name} ({file.size} bytes)")
                ...
                if was_compressed:
                    ...
                    logger.info(f"[FILE] Decompressed file: {compression_stats['ratio']}% reduction, saved {compression_stats['bandwidth_saved_kb']} KB")
                
                logger.info(f"[UPLOAD] File saved to: {full_path}")
                
                # Upload to S3 for backup and cloud storage
                try:
                    logger.info(f"[EMOJI] Uploading file to S3...")
                    ...
                    if connection_test.get('overall_success', False):
                        ...
                        if upload_result.get('success'):
                            ...
                            logger.info(f"[OK] File uploaded to S3: {aws_file_link}")
                        else:
                            logger.warning(f"[WARNING] S3 upload failed: {upload_result.get('error', 'Unknown error')}")
                    else:
                        logger.warning(f"[WARNING] S3 service unavailable, continuing with local file")
                except Exception as s3_error:
                    logger.warning(f"[WARNING] S3 upload error (continuing with local file): {str(s3_error)}")
                
                logger.info(f"[UPLOAD] File will be stored ONCE with path: {file_path}")
                logger.info(f"[UPLOAD] This path will be reused for all {len(mappings)} mapping(s)")
            ...
            if all_relevant_documents_to_process and len(all_relevant_documents_to_process) > 0:
                logger.info(f"[EMOJI] Processing ALL {len(all_relevant_documents_to_process)} relevant document(s) from JSON")
                ...
                with connection.cursor() as cursor:
                    try:
                        ...
                        if framework_row and framework_row[0]:
                            ...
                            logger.info(f"[OK] Got FrameworkId {framework_id_for_processing} for processing all documents")
                        else:
                            logger.error(f"[ERROR] No FrameworkId found for audit {audit_id}")
                            framework_id_for_processing = None
                    except Exception as framework_err:
                        logger.error(f"[ERROR] Error querying FrameworkId: {framework_err}")
                        framework_id_for_processing = None
                    ...
                    for doc_operation_id, doc_matched_compliances in all_relevant_documents_to_process:
                        try:
                            ...
                            if doc_operation_id in processed_operation_ids:
                                logger.info(f"⏭[EMOJI] Skipping duplicate document operation_id={doc_operation_id} (already processed in this batch)")
                                continue
                            
                            logger.info(f"[DOC] Processing document operation_id={doc_operation_id} with {len(doc_matched_compliances)} matched compliances: {doc_matched_compliances}")
                            ...
                            with connection.cursor() as cursor:
                                ...
                                if not doc_file_op:
                                    logger.warning(f"[WARNING] File operation {doc_operation_id} not found or not completed - skipping")
                                    continue
                                ...
                                if existing_count > 0:
                                    logger.info(f"⏭[EMOJI] Document operation_id={doc_operation_id} already exists in ai_audit_data for audit {audit_id} ({existing_count} record(s)) - skipping to avoid duplicate")
                                ...
                                if doc_matched_compliances and len(doc_matched_compliances) > 0:
                                    try:
                                        ...
                                        logger.info(f"[LIST] Created {len(doc_mappings)} mapping(s) for document operation_id={doc_operation_id}")
                                    except Exception as mapping_err:
                                        logger.error(f"[ERROR] Error creating mappings for document {doc_operation_id}: {mapping_err}")
                                        continue
                                ...
                                for mapping in doc_mappings:
                                    try:
                                        ...
                                        if record_id:
                                            ...
                                            doc_created_count += 1
                                            logger.info(f"[OK] Created record ID {record_id} for document operation_id={doc_operation_id}, compliance_id={map_compliance_id}")
                                    except Exception as insert_err:
                                        logger.error(f"[ERROR] Error inserting record for document {doc_operation_id}: {insert_err}")
                                        continue
                                
                                logger.info(f"[OK] Finished processing document operation_id={doc_operation_id} - created {doc_created_count} record(s) for this document")
                                
                                # Commit the transaction (Django auto-commits, but explicit commit ensures it)
                                try:
                                    connection.commit()
                                    logger.info(f"[OK] Committed database transaction for document operation_id={doc_operation_id}")
                                except Exception as commit_err:
                                    logger.error(f"[ERROR] Error committing transaction for document {doc_operation_id}: {commit_err}")
                                    connection.rollback()
                                
                        except Exception as doc_process_err:
                            logger.error(f"[ERROR] Error processing document {doc_operation_id}: {doc_process_err}")
                            import traceback
                            logger.error(f"[ERROR] Error details: {traceback.format_exc()}")
                            try:
                                connection.rollback()
                            except:
                                pass
                ...
                logger.info(f"[OK][OK][OK] Processed ALL {len(all_relevant_documents_to_process)} relevant document(s) from JSON - created {len(all_created_document_ids)} total record(s)")
                ...
                if len(all_created_document_ids) > 0:
                    logger.info(f"[EMOJI] Document Handling batch upload detected - checking if automatic compliance check needed for {len(all_created_document_ids)} mapping(s)")
                    ...
                    with connection.cursor() as cursor:
                        ...
                        if doc_info:
                            ...
                            if completed_count > 0:
                                logger.info(f"⏭[EMOJI] Skipping automatic compliance check - File '{doc_name}' (size: {doc_size}) is already completed for audit {audit_id}")
                                ...
                                try:
                                    ...
                                    logger.info(f"[OK] Updated {updated_count} newly created record(s) to 'completed' status to match existing completed file")
                                except Exception as update_err:
                                    logger.warning(f"[WARNING] Could not update new records to completed status: {update_err}")
                            else:
                                logger.info(f"[INFO] Documents are new/not audited - will automatically trigger compliance checks for {len(all_created_document_ids)} mapping(s)")
                                
                                # Trigger compliance checks asynchronously (don't block the upload response)
                                import threading
                                def auto_check_compliance():
                                    try:
                                        logger.info(f"[INFO] Starting automatic compliance checks for Document Handling batch upload")
                                        ...
                                        try:
                                            logger.info(f"[DEBUG] Auto-checking compliance for primary document_id={primary_document_id}")
                                            ...
                                            if isinstance(result, DRFResponse):
                                                if result.status_code == 200:
                                                    logger.info(f"[OK] Automatic compliance check completed successfully for document_id={primary_document_id}")
                                                else:
                                                    logger.warning(f"[WARNING] Automatic compliance check returned status {result.status_code}")
                                            else:
                                                logger.info(f"ℹ[EMOJI] Compliance check triggered for document_id={primary_document_id}")
                                                
                                        except Exception as check_err:
                                            logger.warning(f"[WARNING] Could not auto-check document_id={primary_document_id}: {check_err}")
                                            import traceback
                                            logger.warning(traceback.format_exc())
                                        
                                        logger.info(f"[OK] Automatic compliance checks completed for Document Handling batch upload")
                                    except Exception as auto_err:
                                        logger.error(f"[ERROR] Error in automatic compliance check: {auto_err}")
                                        import traceback
                                ...
                                logger.info(f"[INFO] Started background thread for automatic compliance checks")
            ...
            try:
                with connection.cursor() as cursor:
                    ...
                    try:
                        logger.info(f"[DEBUG] Querying FrameworkId for audit {audit_id}")
                        cursor.execute("SELECT FrameworkId FROM audit WHERE AuditId = %s", [int(audit_id) if str(audit_id).isdigit() else audit_id])
                        framework_result = cursor.fetchone()
                        logger.info(f"[DEBUG] FrameworkId query result: {framework_result}")
                        
                        if framework_result and framework_result[0] is not None:
                            framework_id = framework_result[0]
                            logger.info(f"[OK] Found FrameworkId {framework_id} for audit {audit_id}")
                        else:
                            logger.error(f"[ERROR] No FrameworkId found for audit {audit_id}. Audit record may not exist or FrameworkId is NULL.")
                            return JsonResponse({
                                'success': False,
                        ...
                        logger.error(f"[ERROR] Error querying FrameworkId for audit {audit_id}: {framework_err}")
                        return JsonResponse({
                            'success': False,
                    ...
                    for mapping_idx, mapping in enumerate(mappings):
                        ...
                        try:
                            ...
                            print("=" * 80)
                            logger.info(f"[DEBUG] About to insert mapping {mapping_idx + 1}/{len(mappings)} into ai_audit_data:")
                            ...
                            try:
                                ...
                                if map_compliance_id:
                                    try:
                                        ...
                                        logger.info(f"[OK] Inserted with compliance_id={map_compliance_id}")
                            ...
                            logger.info(f"[OK] Insert mapping {mapping_idx + 1} into ai_audit_data successful")
                            ...
                            if record_id:
                                ...
                                logger.info(f"[UPLOAD] Created mapping record with ID: {record_id}")
                        except Exception as insert_err:
                            logger.error(f"[ERROR] Insert error for mapping {mapping_idx + 1}: {insert_err}")
                    ...
                    logger.info(f"[OK] Created {len(created_document_ids)} mapping record(s) for file {file_name}")
            except Exception as e:
                logger.error(f"[ERROR] Could not store in ai_audit_data table: {e}")
                raise
            ...
            if external_source == 'evidence_attachment' and created_document_ids:
                logger.info(f"[EMOJI] Document Handling upload detected - checking if automatic compliance check needed for {len(created_document_ids)} mapping(s)")
                ...
                if completed_count > 0:
                    logger.info(f"⏭[EMOJI] Skipping automatic compliance check - File '{file_name}' (size: {file_size}) is already completed for audit {audit_id}")
                    # Update the newly created records to 'completed' status to match existing ones
                    try:
                        with connection.cursor() as cursor:
                    ...
                    logger.info(f"[INFO] Document is new/not audited - will automatically trigger compliance checks for {len(created_document_ids)} mapping(s)")
                    
                    # Trigger compliance checks asynchronously (don't block the upload response)
                    import threading
                    def auto_check_compliance():
                        try:
                            logger.info(f"[INFO] Starting automatic compliance checks for Document Handling upload")
                            ...
                            logger.info(f"[OK] Automatic compliance checks completed for Document Handling upload")
                        except Exception as auto_err:
                            logger.error(f"[ERROR] Error in automatic compliance check: {auto_err}")
                            import traceback
                            logger.error(traceback.format_exc())
                    ...
                    try:
                        ...
                        if audit_obj.Status == 'Yet to Start':
                            ...
                            audit_obj.save()
                            logger.info(f"[OK] Updated audit {audit_id} status to 'Work In Progress' (auto-check started)")
                    except Exception as status_err:
                        logger.warning(f"[WARNING] Could not update audit status: {status_err}")
            ...
            if external_source == 'manual' and created_document_ids:
                try:
                    ...
                    if audit_obj.Status == 'Yet to Start':
                        audit_obj.Status = 'Work In Progress'
                        audit_obj.save()
                        logger.info(f"[OK] Updated audit {audit_id} status to 'Work In Progress' (manual document upload)")
                except Exception as status_err:
                    logger.warning(f"[WARNING] Could not update audit status for manual upload: {status_err}")
...
def save_ai_compliance_to_checklist(audit_id, document_id, analyses, user_id, framework_id, policy_id=None, subpolicy_id=None):
    ...
    try:
                ...
                try:
                    with connection.cursor() as cursor:
                        ...
                        if user_row:
                            ...
                            logger.info(f"[OK] Resolved username '{user_id}' to UserId {numeric_user_id}")
                        else:
                            logger.warning(f"[WARNING] Could not resolve username '{user_id}' to UserId; using NULL")
                            numeric_user_id = None
                except Exception as user_lookup_err:
                    logger.warning(f"[WARNING] Error resolving username '{user_id}' to UserId: {user_lookup_err}")
                    numeric_user_id = None
        ...
        logger.info(f"[SAVE] Saving {len(analyses_list)} compliance results to lastchecklistitemverified for audit {audit_id}")
        ...
        with connection.cursor() as cursor:
            ...
            for analysis in analyses_list:
                ...
                if not compliance_id:
                    logger.warning(f"[WARNING] Analysis item missing compliance_id: {analysis}")
                    continue
                ...
                if not current_subpolicy_id or not current_policy_id:
                    logger.warning(f"[WARNING] Could not resolve SubPolicyId/PolicyId for compliance {compliance_id}")
                    continue
                ...
                try:
                    ...
                    saved_count += 1
                    logger.info(
                        f"[OK] Inserted into lastchecklistitemverified for compliance {compliance_id} (Complied={complied_value})"
                    )
                    ...
                    updated_count += 1
                    logger.info(
                        f"[OK] Updated existing lastchecklistitemverified for compliance {compliance_id} (Complied={complied_value})"
                    )
                ...
                try:
                    if is_ai_audit:
                        ...
                        try:
                            ...
                            logger.warning(f"[WARNING] Could not fetch audit context: {ctx_err}")
                        
                        # Generate comprehensive audit finding fields using AI
                        logger.info(f"[AI] Generating comprehensive audit finding fields for compliance {compliance_id} using AI...")
                        try:
                            ...
                            logger.info(f"[OK] AI generation completed for compliance {compliance_id}. Fields: {list(comprehensive_fields.keys())}")
                        except Exception as ai_gen_err:
                            logger.error(f"[ERROR] AI generation failed for compliance {compliance_id}: {ai_gen_err}")
                            import traceback
                            logger.error(f"[ERROR] AI generation traceback: {traceback.format_exc()}")
                            ...
                            logger.info(f"[OK] Using fallback fields for compliance {compliance_id}")
                        ...
                        logger.info(f"[OK] JSON fields formatted - PredictiveRisks type: {type(predictive_risks)}, CorrectiveActions type: {type(corrective_actions)}")
                        logger.info(f"[OK] All fields converted to strings - Recommendation type: {type(recommendation)}, Impact type: {type(impact)}")
                        ...
                        if isinstance(complied_value, str):
                            ...
                            logger.warning(f"[WARNING] Unknown complied_value type: {type(complied_value)}, value: {complied_value}. Defaulting to '0'")
                            check_value = '0'
                        
                        # Final validation: ensure check_value is exactly '0', '1', '2', or '3'
                        if check_value not in ['0', '1', '2', '3']:
                            logger.error(f"[ERROR] Invalid check_value '{check_value}' for compliance {compliance_id}. Defaulting to '0'")
                            check_value = '0'
                        ...
                        if existing_finding:
                            ...
                            logger.info(f"[OK] Updated audit finding (ID: {existing_finding[0]}) for compliance {compliance_id} with ALL AI-generated fields")
                        else:
                            ...
                            logger.info(f"[OK] Created audit finding (ID: {finding_id}) for compliance {compliance_id} with ALL AI-generated fields")
                            logger.info(f"[OK] Inserted values - Check: {check_value}, MajorMinor: {major_minor}, SeverityRating: {severity_rating}")
                except Exception as finding_err:
                    import traceback
                    logger.error(f"[ERROR] Error creating/updating audit finding for compliance {compliance_id} in audit {audit_id}: {finding_err}")
                    logger.error(f"[ERROR] Traceback: {traceback.format_exc()}")
                    # Don't re-raise - continue with other compliances
        
        logger.info(f"[OK] Saved AI compliance results: {saved_count} inserted, {updated_count} updated in lastchecklistitemverified")
       
    except Exception as e:
        logger.error(f"[ERROR] Error saving to lastchecklistitemverified: {e}")
        import traceback
        logger.error(traceback.format_exc())
...
class AIAuditDocumentsView(View):
    ...
    def get(self, request, audit_id):
        ...
        try:
            logger.info(f"[LIST] Getting documents for audit {audit_id}")
            ...
            with connection.cursor() as cursor:
                ...
                logger.info(f"[LIST] Querying documents for audit_id: {audit_id} (type: {type(audit_id)})")
                logger.info(f"[LIST] Converted audit_id: {converted_audit_id} (type: {type(converted_audit_id)})")
                ...
                try:
                    ...
                    if 'compliance_id' in str(compliance_id_err).lower() or 'Unknown column' in str(compliance_id_err):
                        logger.warning(f"[WARNING] compliance_id column not found in ai_audit_data table, selecting without it")
                        cursor.execute("""
                            SELECT id, document_id, document_name, document_type, file_size, 
                                created_at, upload_status, ai_processing_status, 
                                external_source, external_id, mime_type, document_path,
                ...
                logger.info(f"[LIST] Raw SQL result rows: {len(rows)}")
                ...
                try:
                    ...
                    logger.warning(f"ℹ[EMOJI] Could not fetch policy/subpolicy names for audit {audit_id}: {e}")
            ...
            for row in rows:
                ...
                if doc_policy_id or doc_subpolicy_id:
                    try:
                        ...
                        logger.warning(f"[WARNING] Could not fetch policy/sub-policy names for document {doc_dict.get('document_id')}: {e}")
            ...
            logger.info(f"[OK] Returning {len(documents)} documents for audit {audit_id}")
            ...
            logger.error(f"[ERROR] Error getting documents: {e}")
            return JsonResponse({
                'success': False,
                'error': str(e)
            }, status=500)
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            try:
                ...
                logger.warning(f"ℹ[EMOJI] Mapping columns may not exist on audit_document: {e}")
                return Response({
                    'success': False,
        ...
        logger.error(f"[ERROR] Error updating mapping: {e}")
        return Response({
            'success': False,
            'error': str(e)
...
class AIAuditStatusView(View):
    ...
    def get(self, request, audit_id):
        ...
        try:
            logger.info(f"[STATS] Getting AI audit status for audit {audit_id}")
            ...
            logger.error(f"[ERROR] Error getting AI status: {e}")
            return JsonResponse({
                'success': False,
                'error': str(e)
            }, status=500)
...
def trigger_audit_reprocessing_on_json_update(audit_id, framework_id):
    ...
    """
    try:
        logger.info(f"[EMOJI] Triggering audit re-processing for audit {audit_id} due to JSON update")
        ...
        with connection.cursor() as cursor:
            ...
            updated_count = cursor.rowcount
            logger.info(f"[OK] Marked {updated_count} document(s) as 'pending' for re-processing in audit {audit_id}")
        ...
        logger.error(f"[ERROR] Error triggering audit re-processing: {e}")
        import traceback
        logger.error(f"[ERROR] Error details: {traceback.format_exc()}")
        return {
            'success': False,
...
def start_ai_audit_processing_api(request, audit_id):
    ...
    try:
        logger.info(f"[INFO] Starting AI processing for audit {audit_id}")
        ...
        logger.info(f"[INFO] Processing request from user: {user_id}")
        ...
        logger.info(f"[INFO] Found {len(documents)} documents to process")
        ...
        logger.info(f"[INFO] Updated {updated_count} documents to processing status")
        ...
        for doc in documents:
            ...
            try:
                ...
                logger.info(f"[OK] Document {doc_name} processed successfully")
                
            except Exception as e:
                logger.error(f"[ERROR] Error processing document {doc_name}: {e}")
        ...
        logger.error(f"[ERROR] Error starting AI processing: {e}")
        return Response({
            'success': False,
            'error': str(e)
        }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def process_document_with_ai(doc_id, doc_name, file_path, doc_type, audit_id):
    ...
    try:
        logger.info(f"[AI] Starting AI processing for document: {doc_name}")
        ...
        logger.info(f"[DEBUG] Checking file path: {full_path}")
        if not os.path.exists(full_path):
            logger.error(f"[ERROR] File not found: {full_path}")
            raise Exception(f"File not found: {full_path}")
        
        logger.info(f"[OK] File found, size: {os.path.getsize(full_path)} bytes")
        ...
        logger.info(f"[OK] AI processing completed for {doc_name}")
        return result
        
    except Exception as e:
        logger.error(f"[ERROR] AI processing failed for {doc_name}: {e}")
        raise
...
Return JSON now:"""
        ...
        # Call AI API
        logger.info(f"[AI] Generating comprehensive audit finding fields for compliance: {compliance_description[:50]}...")
        logger.info(f"[AI] Prompt length: {len(prompt)} characters")
        try:
            ai_response = call_ai_api(prompt, None, None, 'analysis')
            logger.info(f"[OK] AI API call completed. Response length: {len(ai_response) if ai_response else 0}")
        except Exception as api_err:
            logger.error(f"[ERROR] AI API call failed: {api_err}")
            import traceback
            logger.error(f"[ERROR] AI API traceback: {traceback.format_exc()}")
            raise
        ...
        try:
            ...
            logger.info(f"[OK] Successfully generated comprehensive audit finding fields. Keys: {list(finding_fields.keys())}")
            return finding_fields
        except json.JSONDecodeError as json_err:
            logger.error(f"[ERROR] JSON parsing failed: {json_err}")
            logger.error(f"[ERROR] Response content (first 1000 chars): {cleaned_response[:1000]}")
            logger.error(f"[ERROR] Response content (last 500 chars): {cleaned_response[-500:]}")
            ...
            try:
                ...
                logger.info(f"[OK] Fixed truncated JSON by closing structures. Keys: {list(finding_fields.keys())}")
                return finding_fields
            except:
                logger.error(f"[ERROR] Could not fix JSON, using fallback fields")
                raise
        
    except Exception as e:
        logger.error(f"[ERROR] Error generating comprehensive audit finding fields: {e}")
        import traceback
        logger.error(f"[ERROR] Traceback: {traceback.format_exc()}")
...
def _get_policy_requirements(policy_id: int, subpolicy_id: int = None):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            for r in rows:
                ...
                if description and description in used_descriptions:
                    logger.info(f"[EMOJI] Skipping duplicate compliance {r[0]}: '{description[:50]}...'")
                    continue
                ...
                logger.info(f"[DEBUG] Requirement {r[0]}: '{base_title}' -> '{unique_title}'")
        ...
        logger.error(f"[ERROR] Error loading requirements for policy {policy_id}: {e}")
        return []
...
def _ai_score_requirements_with_openai(document_text: str, requirements: list, schema: dict = None, audit_id=None, document_id=None, audit_context=None):
    ...
    results = []
    logger.info(f"[DEBUG] Processing {len(requirements)} requirements sequentially with 600s timeout each")
    
    # Process requirements sequentially
    for i, req in enumerate(requirements):
        global_idx = i + 1
        logger.info(f"[DEBUG] Processing requirement {global_idx}: {req.get('title', 'Requirement')}")
        ...
        logger.info(f"[OK] Completed requirement {global_idx}")
...
def _process_single_requirement_batch(document_text: str, batch: list, global_idx: int, audit_id=None, document_id=None, audit_context=None):
    ...
    logger.info("[AI] Using unified AI API for compliance checking")
    logger.info(f"⏱[EMOJI] Processing requirement {global_idx} with 600s timeout (10 minutes)...")
...
Return JSON now:"""
    ...
    logger.info(f"[AI] TinyLlama response length: {len(data)} characters")
    ...
    try:
        ...
        if code_block_match:
            ...
            logger.info(f"[PROC] Extracted JSON from markdown code block")
        else:
            ...
            if json_start > 0:
                ...
                if temp_data[0] == '{':
                    ...
                    logger.info(f"[PROC] Removed {json_start} characters of leading text and extracted complete JSON object (length: {len(cleaned_data)})")
                elif temp_data[0] == '[':
                    ...
                    cleaned_data = temp_data[:end_pos]
                    logger.info(f"[PROC] Removed {json_start} characters of leading text and extracted complete JSON array (length: {len(cleaned_data)})")
                else:
                    cleaned_data = temp_data
                    logger.info(f"[PROC] Removed {json_start} characters of leading text before JSON")
            elif json_start == -1:
                ...
                if analysis_match:
                    ...
                    if brace_pos >= 0:
                        ...
                        logger.info(f"[PROC] Found JSON structure starting with 'analysis' key")
        ...
        if not cleaned_data.startswith('{') and not cleaned_data.startswith('['):
            ...
            if json_obj_match and (not json_array_match or json_obj_match.start() < json_array_match.start()):
                ...
                cleaned_data = temp_data[:end_pos]
                logger.info(f"[PROC] Extracted complete JSON object from markdown text response (length: {len(cleaned_data)})")
                ...
                cleaned_data = temp_data[:end_pos]
                logger.info(f"[PROC] Extracted complete JSON array from markdown text response (length: {len(cleaned_data)})")
            else:
                ...
                if analysis_key_match:
                    ...
                    if start_pos >= 0:
                        ...
                        if end_pos > start_pos:
                            ...
                            logger.info(f"[PROC] Extracted JSON structure around 'analysis' key from markdown response")
        
        # Try to parse the JSON
        parsed = json.loads(cleaned_data)
    except json.JSONDecodeError as e:
        # If JSON is truncated or malformed, try to repair it
        logger.warning(f"[WARNING] JSON parsing error at position {e.pos}: {e.msg}. Attempting to repair...")
        ...
        try:
            ...
            if analysis_match:
                ...
                if obj_end > obj_start:
                    ...
                    try:
                        ...
                        logger.info(f"[OK] Successfully extracted first analysis object from truncated JSON")
                    except:
                        ...
                        if compliance_id_match:
                            ...
                            logger.info(f"[OK] Successfully extracted analysis from truncated JSON using regex fallback")
                        else:
                            raise
                else:
                    raise
            ...
            logger.error(f"[ERROR] Failed to repair JSON. Error: {repair_err}")
            logger.error(f"[ERROR] Original response (first 500 chars): {data[:500]}")
    ...
    if 'analysis' in parsed and isinstance(parsed['analysis'], list):
        ...
        logger.info("[AI] Using OpenAI for compliance checking")
        logger.info(f"⏱[EMOJI] Processing {len(batch)} requirement with 60s timeout...")
        req_lines = []
...
JSON:"""
        ...
        try:
            ...
            logger.info(f"[AI] TinyLlama response length: {len(data)} characters")
            logger.info(f"[AI] TinyLlama response preview: {data[:300]}...")
            logger.info(f"[AI] Full TinyLlama response: {data}")
            ...
            for a in parsed.get('analysis', []):
                ...
                if not isinstance(a, dict):
                    logger.error(f"[ERROR] Analysis item is not a dict: {type(a)} - {a}")
                ...
                if global_index in seen_indexes_global or title_key in seen_titles_global:
                    logger.info(f"[EMOJI] Skipping duplicate analysis for global index {global_index} / title '{a['requirement_title']}'")
                    continue
                seen_indexes_global.add(global_index)
                seen_titles_global.add(title_key)
                results.append(a)
            
            logger.info(f"[OK] Processed batch {i//BATCH + 1} successfully")
            
        except Exception as e:
            logger.error(f"[ERROR] OpenAI batch {i//BATCH + 1} failed: {e}")
            raise Exception(f"OpenAI processing failed: {e}")

    logger.info(f"[OK] OpenAI compliance check completed for {len(requirements)} requirements")
    return results
...
def _check_document_compliance_internal(audit_id, document_id, user_id=None, selected_compliance_ids=None):
    ...
    """
    try:
        logger.info(f"[DEBUG] Internal compliance check: audit_id={audit_id}, document_id={document_id}")
                        ...
                        if relevance_row and relevance_row[0]:
                            ...
                            try:
                                ...
                                if matched_compliances_from_relevance and isinstance(matched_compliances_from_relevance, list) and len(matched_compliances_from_relevance) > 0:
                                    ...
                                    logger.info(f"[LIST] Document from Document Handling: Using ONLY {len(selected_compliance_ids)} matched compliances from relevance analysis: {selected_compliance_ids}")
                                    logger.info(f"[LIST] Will NOT check all compliances in policy/subpolicy - only these specific ones")
                                else:
                                    logger.info(f"[LIST] Document from Document Handling but no matched_compliances found - will use policy/subpolicy defaults")
                            except Exception as e:
                                logger.warning(f"[WARNING] Could not parse matched_compliances from relevance analysis: {e}")
                        else:
                            logger.info(f"[LIST] No relevance analysis found for file_operation_id {file_operation_id_for_lookup} - will use policy/subpolicy defaults")
                except Exception as e:
                    logger.info(f"ℹ[EMOJI] Could not retrieve matched_compliances from document_audit_relevance: {e}")
        ...
        if external_source in ['s3', 'evidence_attachment'] and external_id:
            ...
            try:
                ...
                if external_source == 'evidence_attachment':
                    logger.info(f"[DEBUG] Looking up file_operations for external_id: {external_id}")
                    try:
                        with connection.cursor() as cursor:
                        ...
                        logger.warning(f"[WARNING] Error looking up file_operations: {lookup_err}")
                ...
                logger.error(f"[ERROR] Error handling S3 file: {e}")
        ...
        if selected_compliance_ids:
            ...
            try:
                with connection.cursor() as cursor:
                    ...
                    rows = cursor.fetchall()
                if not rows:
                    logger.warning(f"[WARNING] No requirements found for selected compliances: {selected_compliance_ids}")
                    return {'success': False, 'error': f'No requirements found for selected compliances: {selected_compliance_ids}'}

                logger.info(f"[OK] Loaded {len(rows)} requirement(s) for selected compliances: {selected_compliance_ids}")
                requirements = []
                for r in rows:
                    description = r[2] or ''
                ...
                logger.error(f"[ERROR] Error loading requirements: {e}")
            ...
            logger.info(f"[LIST] No selected compliances provided - using all compliances for policy_id={policy_id}, subpolicy_id={subpolicy_id}")
        ...
        if db_evidence_count > 0:
            logger.info(f"[DEBUG] [AUTO] [OK] Both document and database evidence found ({db_evidence_count} DB records).")
            logger.info(f"[DEBUG] [AUTO] [LIST] Using COMBINED EVIDENCE approach - will analyze both together in ONE check.")
            ...
            try:
                ...
                if result.get('success'):
                    ...
                    logger.warning(f"[WARNING] Combined evidence check failed: {result.get('error')}, falling back to document-only")
                    # Fall through to document-only check
            except Exception as combined_err:
                logger.warning(f"[WARNING] Combined evidence check failed, falling back to document-only: {combined_err}")
                # Fall through to document-only check

        # =============================
        # DOCUMENT-ONLY EVIDENCE PATH
        # =============================
        logger.info(f"[DEBUG] [AUTO] Only document evidence found. Using document-only compliance check.")
        ...
        try:
            ...
            with connection.cursor() as cursor:
                ...
                logger.info(f"[OK] Updated {rows_updated} record(s) for document {document_id}")
                ...
                try:
                    ...
                    if audit_info and audit_info[0] == 'A':  # AI Audit
                        ...
                        logger.info(f"[STATS] Audit {audit_id} document status: total={total_docs}, completed={completed_docs}, failed={failed_docs}")
                        ...
                        if total_docs > 0 and (completed_docs + failed_docs) == total_docs:
                            ...
                            try:
                                ...
                                logger.info(f"[DEBUG] Current audit status: '{current_status}'")
                                ...
                                if audit_obj.Status not in ['Under review', 'Completed']:
                                    ...
                                    audit_obj.save()
                                    logger.info(f"[OK] Updated AI audit {audit_id} status from '{current_status}' to 'Under review' (all {total_docs} documents processed, ready for reviewer)")
                                    ...
                                    try:
                                        ...
                                        if auditor_id:
                                            ...
                                            logger.info(f"[OK] Automatically created audit_version for AI audit {audit_id} (document handling): {version_result}")
                                        else:
                                            logger.warning(f"[WARNING] Could not create audit_version for audit {audit_id}: No auditor_id found")
                                    except Exception as version_err:
                                        logger.error(f"[ERROR] Failed to create audit_version automatically (document handling): {version_err}")
                                        import traceback
                                    ...
                                    try:
                                        ...
                                        logger.info(f"[DEBUG] Creating incidents for non-compliant findings in AI audit {audit_id}")
                                        create_incidents_for_findings(int(audit_id) if str(audit_id).isdigit() else audit_id)
                                        logger.info(f"[OK] Successfully created incidents for AI audit {audit_id}")
                                    except Exception as incident_err:
                                        logger.error(f"[ERROR] Failed to create incidents for AI audit {audit_id}: {incident_err}")
                                        import traceback
                                        logger.error(f"Traceback: {traceback.format_exc()}")
                                else:
                                    logger.info(f"ℹ[EMOJI] Audit {audit_id} already in status '{current_status}', skipping status update")
                            except Exception as audit_update_err:
                                logger.error(f"[ERROR] Failed to update audit status: {audit_update_err}")
                                import traceback
                                logger.error(f"Traceback: {traceback.format_exc()}")
                                raise  # Re-raise to be caught by outer except
                except Exception as status_check_err:
                    logger.error(f"[WARNING] Could not check/update audit completion status: {status_check_err}")
                    import traceback
                    logger.error(f"Traceback: {traceback.format_exc()}")
                ...
                try:
                    ...
                    logger.warning(f"[WARNING] Could not save to lastchecklistitemverified: {e}")
        except Exception as e:
            logger.warning(f"ℹ[EMOJI] Could not persist compliance results: {e}")
        ...
        logger.error(f"[ERROR] Error in internal compliance check: {e}")
        import traceback
        traceback.print_exc()
...
def check_document_compliance(request, audit_id, document_id):
    ...
    try:
        ...
        logger.info(f"[DEBUG] Compliance check request from user: {user_id}")
        ...
        with connection.cursor() as cursor:
            ...
            if audit_row:
                ...
                logger.info(f"[LIST] Audit context: Title='{audit_title}', Objective='{audit_objective[:100] if audit_objective else None}...', Scope='{audit_scope[:100] if audit_scope else None}...'")
        ...
        with connection.cursor() as cursor:
            ...
            doc_path, doc_type, policy_id, subpolicy_id, external_source, external_id, document_name, file_size = row
            logger.info(f"[LIST] Document lookup: external_source={external_source}, external_id={external_id} (type: {type(external_id)}), document_name={document_name}, file_size={file_size}")
        ...
        if policy_id:
            with connection.cursor() as cursor:
                try:
                    ...
                    logger.warning(f"[WARNING] Could not fetch policy name: {e}")
        ...
        if subpolicy_id:
            with connection.cursor() as cursor:
                try:
                    ...
                    logger.warning(f"[WARNING] Could not fetch subpolicy name: {e}")
        ...
        if external_source in ['s3', 'evidence_attachment'] and external_id:
            ...
            try:
                    ...
                    try:
                        with connection.cursor() as cursor:
                            ...
                            if file_op:
                                ...
                                logger.info(f"[OK] Found file_operations record: s3_key={s3_key_from_db}, s3_url={s3_url_from_db}, file_name={file_name_from_db}, stored_name={stored_name_from_db}")
                                ...
                                if s3_url_from_db:
                                    if 'amazonaws.com/' in s3_url_from_db:
                                        ...
                                        if extracted_key:
                                            s3_key = extracted_key
                                            logger.info(f"[OK] Extracted s3_key from s3_url: {s3_key}")
                                    elif 's3.' in s3_url_from_db or 's3-' in s3_url_from_db:
                                        ...
                                        try:
                                            ...
                                            if parsed.path:
                                                s3_key = parsed.path.lstrip('/')
                                                logger.info(f"[OK] Extracted s3_key from s3_url (parsed): {s3_key}")
                                        except Exception as parse_err:
                                            logger.warning(f"[WARNING] Could not parse s3_url: {parse_err}")
                                
                                # Fallback to s3_key from database if we couldn't extract from URL
                                if not s3_key and s3_key_from_db:
                                    s3_key = s3_key_from_db
                                    logger.info(f"[OK] Using s3_key from database: {s3_key}")
                                
                                # Store for potential fallback
                                s3_url_for_fallback = s3_url_from_db if s3_url_from_db else None
                                stored_name_for_fallback = stored_name_from_db
                            else:
                                logger.warning(f"[WARNING] File not found in file_operations table for external_id: {external_id}")
                                # Try to use external_id as s3_key if it looks like a valid key
                                if external_id and '/' in external_id:
                                    s3_key = external_id
                                    logger.info(f"[WARNING] Using external_id as s3_key: {s3_key}")
                    except Exception as lookup_err:
                        logger.warning(f"[WARNING] Error looking up file_operations: {lookup_err}")
                        import traceback
                        traceback.print_exc()
                ...
                logger.info(f"[DEBUG] Downloading from S3: s3_key={s3_key}, file_name={file_name}")
                ...
                if stored_name_for_fallback:
                    if stored_name_for_fallback not in s3_keys_to_try:
                        s3_keys_to_try.append(stored_name_for_fallback)
                        logger.info(f"[EMOJI] Added stored_name as fallback: {stored_name_for_fallback}")
                
                # Try each s3_key variation
                for attempt_key in s3_keys_to_try:
                    try:
                        logger.info(f"[EMOJI] Attempting download with s3_key: {attempt_key}")
                        ...
                        if download_result.get('success'):
                            ...
                            logger.info(f"[OK] Download successful with s3_key: {attempt_key}")
                            break
                        else:
                            last_error = download_result.get('error', 'Unknown error')
                            logger.warning(f"[WARNING] Download failed with s3_key {attempt_key}: {last_error}")
                    except Exception as download_ex:
                        last_error = str(download_ex)
                        logger.warning(f"[WARNING] Exception with s3_key {attempt_key}: {last_error}")
                        import traceback
                        traceback.print_exc()
                
                if not download_success:
                    error_msg = last_error or 'Unknown error'
                    logger.error(f"[ERROR] All S3 download attempts failed. Last error: {error_msg}")
                    logger.error(f"[ERROR] Tried s3_keys: {s3_keys_to_try}")
                    logger.error(f"[ERROR] file_name: {file_name}, external_id: {external_id}")
                    
                    # If we have s3_url, try direct download as last resort
                    if s3_url_for_fallback:
                        logger.info(f"[INFO] Attempting direct download from S3 URL: {s3_url_for_fallback}")
                        # Try one more time with the s3_url if it's a pre-signed URL
                        if 'https://' in s3_url_for_fallback or 'http://' in s3_url_for_fallback:
                            try:
                                import requests as req_lib
                                logger.info(f"[EMOJI] Attempting direct download from S3 URL...")
                                url_response = req_lib.get(s3_url_for_fallback, timeout=60, stream=True)
                                if url_response.status_code == 200:
                                    ...
                                    if os.path.exists(temp_file_path):
                                        download_success = True
                                        logger.info(f"[OK] Direct download from S3 URL successful!")
                                        full_path = temp_file_path
                                        temp_file_created = True
                            except Exception as direct_dl_err:
                                logger.warning(f"[WARNING] Direct download from URL also failed: {direct_dl_err}")
                ...
                if download_success and not temp_file_created and 'download_result' in locals():
                    ...
                    full_path = temp_file_path
                    temp_file_created = True
                    logger.info(f"[DEBUG] Downloaded S3 file to: {full_path}")
        ...
        if selected_ids:
            logger.info(f"[DEBUG] check_document_compliance using explicit selected_compliance_ids: {len(selected_ids)} items")
            ...
            try:
                ...
                logger.error(f"[ERROR] Error loading requirements for explicit compliance IDs: {e}")
        ...
        if external_source != 'manual':
            ...
            if db_evidence_count > 0:
                ...
                logger.info(f"[DEBUG] Both document and database evidence found ({db_evidence_count} DB records). Using combined evidence approach.")
            else:
                logger.info(f"[DEBUG] Only document evidence found. Using document-only compliance check.")
        else:
            logger.info(f"[DEBUG] Manually uploaded document (external_source='manual'). Using document-only compliance check (no combined evidence).")
        ...
        logger.info(f"[DEBUG] Analyses type: {type(analyses)}, length: {len(analyses) if analyses else 0}")
        if analyses:
            logger.info(f"[DEBUG] First analysis item type: {type(analyses[0])}, content: {analyses[0]}")
        status_label, confidence = _determine_status(requirements, analyses)
        ...
        try:
            ...
            with connection.cursor() as cursor:
                ...
                # Get FrameworkId from the audit record
                try:
                    logger.info(f"[DEBUG] Querying FrameworkId for audit {audit_id} in compliance check")
                    cursor.execute("SELECT FrameworkId FROM audit WHERE AuditId = %s", [int(audit_id) if str(audit_id).isdigit() else audit_id])
                    framework_result = cursor.fetchone()
                    logger.info(f"[DEBUG] FrameworkId query result: {framework_result}")
                ...
                if external_source in ['evidence_attachment', 's3'] and external_id:
                    ...
                    if document_name and file_size:
                        ...
                        logger.info(
                            f"[EMOJI] Updating all ai_audit_data rows for audit_id={audit_id}, external_source={external_source}, external_id={external_id}/{external_id_str} OR (document_name={document_name}, file_size={file_size})"
                        )
                    else:
                        ...
                        logger.info(
                            f"[EMOJI] Updating all ai_audit_data rows for audit_id={audit_id}, external_source={external_source}, external_id={external_id}/{external_id_str}"
                        )
                elif doc_path:
                    ...
                    where_params = [int(audit_id), doc_path]
                    logger.info(
                        f"[EMOJI] Updating all ai_audit_data rows for audit_id={audit_id}, document_path={doc_path}"
                    )
                    ...
                    logger.info(
                        f"[EMOJI] Updating all ai_audit_data rows for audit_id={audit_id}, document_name={document_name}, file_size={file_size} (manually uploaded document)"
                    )
                else:
                    ...
                    where_params = [int(document_id), int(audit_id)]
                    logger.info(
                        f"[EMOJI] Updating single ai_audit_data row for document_id={document_id}, audit_id={audit_id} (fallback)"
                    )
                ...
                rows_updated = cursor.rowcount
                logger.info(f"[OK] Updated {rows_updated} record(s) for document {document_id} in ai_audit_data table")
                ...
                if rows_updated == 0 and external_source in ['evidence_attachment', 's3'] and document_name and file_size:
                    logger.warning(f"[WARNING] No rows updated with external_id match, trying fallback match by document_name + file_size")
                    cursor.execute("""
                        UPDATE ai_audit_data 
                        SET ai_processing_status = 'completed',
                            compliance_status = %s,
                    ...
                    rows_updated = cursor.rowcount
                    logger.info(f"[OK] Fallback update: Updated {rows_updated} record(s) for document_name={document_name}, file_size={file_size}")
                
                # If still no rows were updated, log a warning
                if rows_updated == 0:
                    logger.warning(f"[WARNING] No existing record found for document_id={document_id}, audit_id={audit_id}, external_source={external_source}, external_id={external_id}. Document may need to be uploaded first.")
                ...
                try:
                    ...
                    logger.info(f"[OK] Saved compliance results to lastchecklistitemverified for document {document_id}")
                except Exception as e:
                    logger.warning(f"[WARNING] Could not save to lastchecklistitemverified: {e}")
                    ...
                    if audit_info and audit_info[0] == 'A':  # AI Audit
                        ...
                        if total_docs > 0 and (completed_docs + failed_docs) == total_docs:
                            ...
                            if audit_obj.Status not in ['Under review', 'Completed']:
                                ...
                                audit_obj.ReviewStartDate = timezone.now()
                                audit_obj.save()
                                logger.info(f"[OK] Updated AI audit {audit_id} status to 'Under review' after single document check (all {total_docs} documents processed)")
                                ...
                                try:
                                    ...
                                    if auditor_id:
                                        version_result = create_audit_version(audit_id, auditor_id)
                                        logger.info(f"[OK] Automatically created audit_version for AI audit {audit_id} (single document check): {version_result}")
                                    else:
                                        logger.warning(f"[WARNING] Could not create audit_version for audit {audit_id}: No auditor_id found")
                                except Exception as version_err:
                                    logger.error(f"[ERROR] Failed to create audit_version automatically (single document check): {version_err}")
                                    import traceback
                                    logger.error(f"Traceback: {traceback.format_exc()}")
                except Exception as status_check_err:
                    logger.warning(f"[WARNING] Could not check/update audit completion status after single document check: {status_check_err}")
                    
        except Exception as e:
            logger.warning(f"ℹ[EMOJI] Could not persist compliance results for doc {document_id}: {e}")
        ...
        logger.error(f"[ERROR] Error checking document compliance: {e}")
        ...
        if 'temp_file_created' in locals() and temp_file_created and 'full_path' in locals():
            try:
                if os.path.exists(full_path):
                    os.remove(full_path)
                    logger.info(f"[EMOJI] Cleaned up temporary file: {full_path}")
            except Exception as e:
                logger.warning(f"[WARNING] Failed to clean up temporary file {full_path}: {e}")
        ...
        if cleaned_temp_files:
            logger.info(f"[EMOJI] Additional cleanup - removed {len(cleaned_temp_files)} temporary files")
    ...
    def fix_timestamp_corruption(key):
        ...
        for ts in timestamps:
            if len(ts) > 13:
                ...
                logger.info(f"[PROC] [COMBINED] Fixed corrupted timestamp: '{original_ts}' (length: {len(original_ts)}) -> '{fixed_ts}' (length: 13)")
        return fixed_key
    try:
        logger.info(f"[DEBUG] Combined evidence check for audit {audit_id}, compliances: {compliance_ids}")
        ...
        try:
            ...
            logger.info("[OK] [COMBINED] Created reusable S3 client for all document downloads")
        except Exception as s3_err:
            logger.warning(f"[WARNING] [COMBINED] Could not create reusable S3 client: {s3_err}")
            s3_client_reused = None
        
        for req in requirements:
            compliance_id = req['compliance_id']
            logger.info(f"[DEBUG] Processing compliance {compliance_id}: {req['title']}")
            ...
            with connection.cursor() as cursor:
                ...
                for doc_row in doc_rows:
                    ...
                    if cache_key in document_text_cache:
                        logger.info(f"[LIST] [COMBINED] Using cached text for document {doc_id} (cache_key={cache_key[:50]}...)")
                            ...
                            try:
                                with connection.cursor() as lookup_cursor:
                                    ...
                                    if file_op:
                                        ...
                                        logger.info(f"[DEBUG] [COMBINED] file_operations data for ext_id {ext_id}: s3_key={s3_key_from_db}, file_name={file_name_from_db}, stored_name={stored_name_from_db}, s3_url={s3_url_from_db[:100] if s3_url_from_db else None}")
                                        ...
                                        if s3_url_from_db and 'amazonaws.com/' in s3_url_from_db:
                                            try:
                                                ...
                                                logger.info(f"[DEBUG] [COMBINED] Extracted s3_key from s3_url: {s3_key_from_url[:80]}...")
                                            except:
                                                pass
                                        ...
                                        if s3_key_from_url:
                                            ...
                                            logger.info(f"[DEBUG] [COMBINED] Using s3_key extracted from s3_url (most reliable): {s3_key[:80]}...")
                                            ...
                                            if validate_s3_key(stored_name_fixed, ref_name):
                                                ...
                                                logger.info(f"[DEBUG] [COMBINED] Using validated stored_name as s3_key: {s3_key[:80]}...")
                                            else:
                                                logger.warning(f"[WARNING] [COMBINED] stored_name appears corrupted, trying alternatives: {stored_name_from_db[:80]}...")
                                                # Try s3_key_from_db instead
                                                if s3_key_from_db:
                                                    s3_key = fix_timestamp_corruption(s3_key_from_db)
                                                    logger.info(f"[DEBUG] [COMBINED] Using s3_key_from_db as fallback: {s3_key[:80]}...")
                                                    ...
                                                    logger.info(f"[DEBUG] [COMBINED] Using s3_key extracted from s3_url as fallback: {s3_key[:80]}...")
                                        elif s3_key_from_db:
                                            s3_key = fix_timestamp_corruption(s3_key_from_db)
                                            logger.info(f"[DEBUG] [COMBINED] Using s3_key_from_db: {s3_key[:80]}...")
                                        ...
                                        logger.info(f"[DEBUG] [COMBINED] Final s3_key: {s3_key[:80] if s3_key else 'None'}..., file_name: {actual_file_name[:80] if actual_file_name else 'None'}... for document {doc_id}")
                            except Exception as lookup_err:
                                logger.warning(f"[WARNING] Error looking up file_operations for ext_id {ext_id}: {lookup_err}")
                        ...
                        if s3_key and s3_key.strip():
                            try:
                                    ...
                                    if orig_timestamps and download_timestamps and len(orig_timestamps) == len(download_timestamps):
                                        ...
                                        for i, (orig_ts, dl_ts) in enumerate(zip(orig_timestamps, download_timestamps)):
                                            if len(dl_ts) > len(orig_ts):
                                                logger.warning(f"[WARNING] [COMBINED] Detected timestamp corruption in s3_key! Original timestamp: {orig_ts}, Corrupted: {dl_ts}")
                                                logger.warning(f"[WARNING] [COMBINED] Using original s3_key to avoid corruption: {original_s3_key[:80]}...")
                                                download_s3_key = original_s3_key
                                                break
                                
                                if download_s3_key != original_s3_key and download_s3_key != original_s3_key.strip():
                                    logger.warning(f"[WARNING] [COMBINED] s3_key was modified during copy! Original: {original_s3_key[:80]}, Modified: {download_s3_key[:80] if download_s3_key else None}")
                                ...
                                if not s3_client_reused:
                                    from ..Global.s3_fucntions import create_direct_mysql_client
                                    s3_client_reused = create_direct_mysql_client()
                                    logger.info("[OK] [COMBINED] Created S3 client (reused for all downloads)")
                                    ...
                                    try:
                                        ...
                                        if final_s3_key_from_url:
                                            if final_s3_key_from_url != download_s3_key:
                                                logger.info(f"[EMOJI] [COMBINED] Using s3_key from s3_url (bypassing potential DB corruption). Old: {download_s3_key[:60] if download_s3_key else None}..., New: {final_s3_key_from_url[:60]}...")
                                            ...
                                            logger.info(f"[EMOJI] [COMBINED] Updated file_name to match s3_key from s3_url: {file_name[:60]}...")
                                    except Exception as verify_err:
                                        logger.warning(f"[WARNING] [COMBINED] Could not extract s3_key from s3_url: {verify_err}")
                                ...
                                logger.info(f"[EMOJI] [COMBINED] Downloading document {doc_id} from S3: s3_key={download_s3_key[:80] if download_s3_key else None}..., file_name={file_name[:60] if file_name else None}...")
                                logger.info(f"[DEBUG] [COMBINED] s3_key length: {len(download_s3_key) if download_s3_key else 0}, file_name length: {len(file_name)}")
                                ...
                                if download_result.get('success'):
                                    ...
                                    if os.path.exists(downloaded_path):
                                        ...
                                        logger.info(f"[OK] [COMBINED] Successfully extracted text from document {doc_id} ({len(doc_text)} chars)")
                                        # Clean up temp file
                                        try:
                                        ...
                                        logger.warning(f"[WARNING] [COMBINED] Downloaded file not found at {downloaded_path}")
                                else:
                                    error_msg = download_result.get('error', 'Unknown error')
                                    logger.warning(f"[WARNING] [COMBINED] Download failed for document {doc_id}: {error_msg}")
                                    ...
                                    if ('404' in str(error_msg) or 'Not Found' in str(error_msg)) and s3_url_for_fallback:
                                        try:
                                            logger.info(f"[EMOJI] [COMBINED] Attempting direct download from s3_url for document {doc_id}")
                                            import requests
                                            ...
                                            if response.status_code == 200:
                                                ...
                                                if os.path.exists(temp_file_path_direct):
                                                    ...
                                                    logger.info(f"[OK] [COMBINED] Successfully extracted text via direct s3_url download for document {doc_id} ({len(doc_text)} chars)")
                                                    # Clean up temp file
                                                    try:
                                                ...
                                                logger.warning(f"[WARNING] [COMBINED] Direct s3_url download also failed: HTTP {response.status_code}")
                                        except Exception as direct_download_err:
                                            logger.warning(f"[WARNING] [COMBINED] Direct s3_url download failed: {direct_download_err}")
                            except Exception as download_err:
                                logger.error(f"[ERROR] [COMBINED] Could not download/extract document {doc_id}: {download_err}", exc_info=True)
                    ...
                    if doc_text:
                        ...
                        if cache_key not in document_text_cache:
                            ...
                            logger.info(f"[SAVE] [COMBINED] Cached text for document {doc_id} (cache_key={cache_key[:50]}...), length: {len(doc_text)} chars")
                ...
                for db_row in db_rows:
                    ...
                    if analyses:
                        try:
                            ...
                            logger.warning(f"[WARNING] Error parsing database evidence analyses for {db_name}: {e}")
            ...
            if document_evidence_texts:
                ...
                logger.info(f"[DOC] [COMBINED] Found {len(document_evidence_texts)} document(s) for compliance {compliance_id}")
            ...
            if database_evidence_texts:
                ...
                combined_evidence.append("\n")
                logger.info(f"[SAVE] [COMBINED] Found {len(database_evidence_texts)} database record(s) for compliance {compliance_id}")
            
            if not combined_evidence:
                logger.warning(f"[WARNING] No evidence found for compliance {compliance_id}")
                all_results.append({
            ...
            logger.info(f"[AI] [COMBINED] Analyzing compliance {compliance_id} with {len(document_evidence_texts)} document(s) + {len(database_evidence_texts)} database record(s) = ONE combined check")
            ...
            if not analyses or len(analyses) == 0:
                logger.warning(f"[WARNING] No analysis returned for compliance {compliance_id}")
                all_results.append({
                    'compliance_id': compliance_id,
            ...
            try:
                ...
                if document_evidence_details and len(document_evidence_details) > 0:
                    ...
                    logger.info(f"[INFO] Using primary_document_id {primary_document_id} to update status (document download failed)")
                ...
                if documents_to_update:
                    try:
                        ...
                        with connection.cursor() as cursor:
                            ...
                            updated_count = cursor.rowcount
                            logger.info(f"[OK] Updated {updated_count} document(s) with combined evidence result (group_id={combined_check_group_id}): {documents_to_update}")
                    except Exception as update_err:
                        logger.warning(f"[WARNING] Could not update documents ai_audit_data: {update_err}")
                ...
                if database_evidence_details and len(database_evidence_details) > 0:
                    try:
                        with connection.cursor() as cursor:
                            ...
                            logger.info(f"[OK] Marked {len(database_evidence_details)} database evidence items as part of combined check")
                    except Exception as db_mark_err:
                        logger.warning(f"[WARNING] Could not mark database evidence items: {db_mark_err}")
                
                logger.info(f"[OK] Saved combined evidence result for compliance {compliance_id}")
                ...
                logger.error(f"[ERROR] Error saving combined evidence result: {e}")
                all_results.append({
                    'compliance_id': compliance_id,
                    'status': 'SAVE_FAILED',
                    'error': str(e)
        ...
        logger.error(f"[ERROR] Error in combined evidence check: {e}")
        import traceback
        traceback.print_exc()
        return {
            'success': False,
...
def check_all_documents_compliance(request, audit_id):
    ...
    try:
        ...
        logger.info(f"[DEBUG] Bulk compliance check request from user: {user_id}")
        ...
        if has_doc_evidence and has_db_evidence:
            ...
            logger.info(f"[DEBUG] Both document and database evidence found. Using combined evidence approach for {len(all_compliance_ids)} compliance requirements.")
            ...
            if result.get('success'):
                try:
                    ...
                    logger.info(f"[EMOJI] Bulk check: status update result for audit {audit_id}: {status_update}")
                except Exception as status_err:
                    logger.error(f"[WARNING] Bulk check: could not update audit status after combined evidence check: {status_err}")
                return Response(result, status=status.HTTP_200_OK)
            else:
                return Response(result, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        else:
            # Fallback to document-by-document check (original behavior)
            logger.info(f"[DEBUG] Only {'document' if has_doc_evidence else 'database'} evidence found. Using individual document checks.")
            ...
            for doc_id in doc_ids:
                try:
                    ...
                    logger.warning(f"[WARNING] Error checking document {doc_id}: {e}")
            ...
            try:
                status_update = check_and_update_ai_audit_status(audit_id)
                logger.info(f"[EMOJI] Bulk doc-by-doc check: status update result for audit {audit_id}: {status_update}")
            except Exception as status_err:
                logger.error(f"[WARNING] Bulk doc-by-doc check: could not update audit status after checks: {status_err}")
        ...
        logger.error(f"[ERROR] Error checking all documents: {e}")
        return Response({'success': False, 'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def delete_audit_document_api(request, audit_id, document_id):
    ...
    try:
        logger.info(f"[EMOJI] Deleting document {document_id} from audit {audit_id}")
        ...
        logger.info(f"[EMOJI] Delete request from user: {user_id}")
        ...
        if not row:
            ...
            with connection.cursor() as debug_cursor:
                ...
                if debug_rows:
                    logger.warning(f"[EMOJI] Document {document_id} exists but not in audit {audit_id}. Found in audits: {[r[2] for r in debug_rows]}")
                else:
                    logger.warning(f"[EMOJI] Document {document_id} does not exist in ai_audit_data table at all")
        ...
        logger.info(f"[EMOJI] Found document: id={doc_id}, document_id={doc_document_id}, name={doc_name}, external_id={external_id}")
        ...
        if external_id or doc_path:
            with connection.cursor() as related_cursor:
                ...
                if external_id:
                    ...
                    logger.info(f"[EMOJI] Found {len(related_ids)} additional records with same external_id={external_id} and file_size={file_size}")
                ...
                if doc_path and not external_id:
                    ...
                    related_ids = [r[0] for r in related_cursor.fetchall()]
                    related_record_ids.extend(related_ids)
                    logger.info(f"[EMOJI] Found {len(related_ids)} additional records with same document_path={doc_path}")
        
        # Remove duplicates
        related_record_ids = list(set(related_record_ids))
        logger.info(f"[EMOJI] Will delete {len(related_record_ids)} record(s) total: {related_record_ids}")
            ...
            with connection.cursor() as cursor:
                for record_id in related_record_ids:
                    try:
                        ...
                        logger.info(f"[EMOJI] Deleted record id={record_id}")
                    except Exception as delete_err:
                        logger.error(f"[ERROR] Error deleting record {record_id}: {delete_err}")
                        # Continue with other records even if one fails
                        continue
        except Exception as db_err:
            logger.error(f"[ERROR] Database error during delete: {db_err}")
            return Response({
                'success': False,
        ...
        try:
            if doc_path:
                ...
                if os.path.exists(full_path):
                    os.remove(full_path)
                    logger.info(f"[EMOJI] Deleted file: {full_path}")
                else:
                    logger.warning(f"[EMOJI] File not found on disk: {full_path}")
        except Exception as e:
            logger.warning(f"[EMOJI] Could not delete file {doc_path}: {e}")
        ...
        try:
            ...
            if cleaned_temp_files:
                logger.info(f"[EMOJI] Cleaned up {len(cleaned_temp_files)} temporary files for document {document_id}")
        except Exception as cleanup_err:
            logger.warning(f"[EMOJI] Could not clean up temp files: {cleanup_err}")
        ...
        logger.info(f"[OK] Document {doc_name} deleted successfully")
        ...
        logger.error(f"[ERROR] Error deleting document: {e}")
        return Response({
            'success': False,
...
def delete_all_audit_documents_api(request, audit_id):
    ...
    try:
        logger.info(f"[EMOJI] Bulk delete requested for all documents/evidence of audit {audit_id}")
        ...
        logger.info(f"[OK] Bulk delete completed for audit {audit_id}: {deleted_count} row(s) removed from ai_audit_data")
        ...
        logger.error(f"[ERROR] Error in bulk delete for audit {audit_id}: {e}")
        return Response({
            'success': False,
            'error': str(e)
        }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def download_audit_report(request, audit_id):
    ...
    try:
        logger.info(f"[STATS] Generating comprehensive AI audit report for audit {audit_id}")
        ...
        report_data = {
            ...
            'audit_information': {
                ...
                'audit_assignment_issue': '[WARNING] WARNING: This audit was created as Internal but has AI processing data. It should be treated as an AI Audit.' if (audit_row[1] == 'I' and is_actual_ai_audit) else None
            },
            'processing_summary': {
        ...
        logger.info(f"[OK] Generated comprehensive AI audit report for audit {audit_id}")
        return response
        
    except Exception as e:
        logger.error(f"[ERROR] Error generating AI audit report: {e}")
        return Response({
            'success': False,
            'error': str(e)
        }, status=500)
...
def test_structured_compliance_api(request, audit_id):
    ...
    try:
        ...
        logger.error(f"[ERROR] Error testing structured compliance: {e}")
        return Response({
            'success': False,
            'error': str(e)
        }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def get_relevant_documents_for_audit(request, audit_id):
    ...
    try:
        logger.info(f"[LIST] Getting relevant documents for audit {audit_id}")
        ...
        if not framework_id:
            logger.warning(f"[WARNING] Audit {audit_id} has no FrameworkId assigned")
            return Response({
                'success': True,
                'documents': [],
                'count': 0,
        ...
        with connection.cursor() as cursor:
            ...
            logger.info(f"[STATS] File operations stats for framework {framework_id}: {total_files} total, {completed_files} completed")
            ...
            logger.info(f"[STATS] Documents analyzed for audit {audit_id}: {analyzed_count}")
        ...
        logger.info(f"[OK] Found {len(formatted_documents)} relevant documents for audit {audit_id}")
        ...
        logger.error(f"[ERROR] Error getting relevant documents: {e}")
        import traceback
        traceback.print_exc()
        return Response({
            'success': False,
...
def check_and_update_ai_audit_status(audit_id):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            logger.info(f"[STATS] Manual status check for audit {audit_id}: total={total_docs}, completed={completed_docs}, failed={failed_docs}, current_status='{current_status}'")
            ...
            if total_docs > 0 and (completed_docs + failed_docs) == total_docs:
                ...
                if current_status not in ['Under review', 'Completed']:
                    ...
                    audit_obj.ReviewStartDate = timezone.now()
                    audit_obj.save()
                    logger.info(f"[OK] Updated AI audit {audit_id} status from '{current_status}' to 'Under review'")
                    ...
                    try:
                        ...
                        if auditor_id:
                            version_result = create_audit_version(audit_id, auditor_id)
                            logger.info(f"[OK] Automatically created audit_version for AI audit {audit_id}: {version_result}")
                        else:
                            logger.warning(f"[WARNING] Could not create audit_version for audit {audit_id}: No auditor_id found")
                    except Exception as version_err:
                        logger.error(f"[ERROR] Failed to create audit_version automatically: {version_err}")
                        import traceback
                        logger.error(f"Traceback: {traceback.format_exc()}")
        ...
        logger.error(f"[ERROR] Error checking/updating audit status: {e}")
        import traceback
        logger.error(f"Traceback: {traceback.format_exc()}")
