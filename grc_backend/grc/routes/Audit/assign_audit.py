...
def get_frameworks(request):
    ...
    try:
        ...
        if framework_id:
            print(f"[OK] [GET FRAMEWORKS] Framework filter active: {framework_id}")
            frameworks = Framework.objects.filter(FrameworkId=framework_id, tenant_id=tenant_id).values('FrameworkId', 'FrameworkName')
            print(f"[OK] [GET FRAMEWORKS] Returning {frameworks.count()} filtered framework(s)")
        else:
            print(f"[OK] [GET FRAMEWORKS] No framework filter - returning ALL frameworks")
            frameworks = Framework.objects.filter(tenant_id=tenant_id).values('FrameworkId', 'FrameworkName')
            print(f"[OK] [GET FRAMEWORKS] Returning {frameworks.count()} total frameworks")
        
        frameworks_list = list(frameworks)
        print(f"[OK] [GET FRAMEWORKS] Final response: {frameworks_list}")
    ...
    try:
        ...
        if framework_id_to_use:
            try:
                ...
                print(f"[OK] [AUDIT CREATE] Using FrameworkId: {framework_id_to_use}")
            ...
            print("ℹ[EMOJI] [AUDIT CREATE] No framework selected - creating audit without FrameworkId")
