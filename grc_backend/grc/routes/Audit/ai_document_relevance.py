...
def analyze_document_relevance(request, audit_id):
    ...
    try:
        logger.info(f"[AI] Starting AI document relevance analysis for audit: {audit_id}")
        ...
        logger.info(f"[DOC] Document: {document_name} (ID: {document_id})")
        logger.info(f"[LIST] Compliance requirements: {len(compliance_requirements)}")
        ...
        try:
            ...
            if not row:
                logger.error(f"[ERROR] Document not found: {document_id}")
                return Response({
                    'success': False,
            ...
            logger.info(f"[OK] Found document in database: {doc_name}")
        ...
        logger.info(f"[EMOJI] Extracting text from: {file_path}")
        ...
        if not document_text or len(document_text.strip()) < 10:
            logger.error(f"[ERROR] Unable to extract text from document: {document_name}")
            return Response({
                'success': False,
                'error': 'Unable to extract text from document'
            }, status=status.HTTP_400_BAD_REQUEST)
        
        logger.info(f"[OK] Extracted {len(document_text)} characters from document")
        ...
        logger.info(f"[OK] AI relevance analysis complete for: {document_name}")
        logger.info(f"[TARGET] Found {len(final_suggestions)} relevant compliance requirements")
        
        return Response(response_data)
        
    except Exception as e:
        logger.error(f"[ERROR] Error in document relevance analysis: {str(e)}")
        return Response({
            'success': False,
...
def analyze_relevance_with_ai(document_text, document_name, compliance_requirements):
    ...
    """
    try:
        logger.info("[AI] Using Ollama AI for document relevance analysis")
...
Focus on content relevance, not just keyword matching. Score based on how well the document addresses the compliance requirement."""
        ...
        # Send to Ollama
        logger.info(f"[AI] Sending relevance analysis request to Ollama")
        ...
        logger.info(f"[AI] AI Response length: {len(ai_response)} characters")
        ...
        try:
            ...
            logger.info(f"[OK] AI analysis processed {len(relevance_scores)} compliance requirements")
            logger.info(f"[TARGET] Found {len(suggested_compliances)} relevant suggestions")
            ...
            logger.error(f"[ERROR] Failed to parse AI JSON response: {str(e)}")
            logger.error(f"[ERROR] AI Response: {ai_response[:500]}...")
            raise Exception(f"AI returned invalid JSON: {str(e)}")
            
    except Exception as e:
        logger.error(f"[ERROR] AI relevance analysis failed: {str(e)}")
        return {
            'success': False,
