            ...
            if auth_header and auth_header.startswith('Bearer '):
                try:
                    ...
                    if payload and 'user_id' in payload:
                        ...
                        print(f"[OK] [AUDIT] Found user_id in JWT token: {user_id}")
                except Exception as jwt_error:
                    print(f"[WARNING] [AUDIT] JWT extraction failed: {str(jwt_error)}")
        
        # If still no user_id, use default user for testing
        if not user_id:
            user_id = '1'  # Default to user ID 1 for testing
            print(f"[WARNING] [AUDIT] No user ID found - using default user ID: {user_id}")
        
        # Get framework from context
        framework_id = get_framework_context(user_id)
        
        if framework_id:
            print(f"[OK] [AUDIT] Framework filter active: {framework_id} for user {user_id}")
        else:
            print(f"ℹ[EMOJI] [AUDIT] No framework filter (All frameworks selected) for user {user_id}")
        
        return framework_id
        
    except Exception as e:
        print(f"[ERROR] [AUDIT] Error getting framework filter: {str(e)}")
        return None
...
def apply_framework_filter(queryset: QuerySet, request, framework_field: str = 'FrameworkId') -> QuerySet:
    ...
    try:
        ...
        if framework_id is None:
            print(f"[STATS] [AUDIT] No framework filter - returning all results")
            return queryset
        ...
        print(f"[STATS] [AUDIT] Framework filter applied: {framework_id}, Results: {count}")
        
        return filtered_queryset
        
    except Exception as e:
        print(f"[ERROR] [AUDIT] Error applying framework filter: {str(e)}")
...
def get_framework_sql_filter(request, table_alias: str = 'a') -> tuple:
    ...
    try:
        ...
        if framework_id is None:
            print(f"[STATS] [AUDIT] No framework filter for SQL query")
            return ("", {})
        
        print(f"[STATS] [AUDIT] Adding framework SQL filter: {framework_id}")
        return (f"AND {table_alias}.FrameworkId = %(framework_id)s", {"framework_id": framework_id})
        
    except Exception as e:
        print(f"[ERROR] [AUDIT] Error getting framework SQL filter: {str(e)}")
        return ("", {})
