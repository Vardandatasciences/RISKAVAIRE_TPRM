    ...
    if request.method == 'GET':
        ...
        if user_id:
            ...
            if framework_id_from_session:
                print(f"[OK] DEBUG: Found framework_id in framework context: {framework_id_from_session}")
        ...
        if not framework_id_from_session:
            ...
            if session_framework_id:
                ...
                print(f"[WARNING] DEBUG: Found framework_id in session (fallback): {framework_id_from_session}")
        
        print(f"[DEBUG] DEBUG: framework_list POST - checking for framework context")
        print(f"[STATS] DEBUG: Framework ID: {framework_id_from_session}")
                ...
                for policy_data in policies_data:
                    ...
                    try:
                        ...
                        if existing_approval:
                            print(f"DEBUG: [WARNING] Duplicate prevention: Policy approval already exists for PolicyId {policy.PolicyId} with ApprovalId: {existing_approval.ApprovalId}")
                            ...
                            print(f"DEBUG: [OK] Created new PolicyApproval for PolicyId {policy.PolicyId}")
                    except Exception as approval_error:
        ...
        if existing_today_approval:
            ...
            try:
                ...
                if latest_approval:
                    ...
                    new_approval.save()
                    print(f"[OK] Created PolicyApproval for policy {policy.PolicyId}:")
                    ...
                    new_approval.save()
                    print(f"[OK] Created basic PolicyApproval for policy {policy.PolicyId}:")
                    print(f"   - ApprovalId: {new_approval.ApprovalId}")
                    print(f"   - Version: {new_version}")
                    print(f"   - ReviewerId: {reviewer_id}")
                    print(f"   - ApprovedNot: {new_approval.ApprovedNot}")
            except Exception as approval_err:
                print(f"[ERROR] Error creating policy approval: {str(approval_err)}")
                import traceback
                traceback.print_exc()
...
def submit_policy_approval_review(request, policy_id):
    ...
    try:
        ...
        if existing_approval:
            print(f"DEBUG: [WARNING] Duplicate prevention: Policy approval already exists with ApprovalId: {existing_approval.ApprovalId}")
        ...
        print(f"DEBUG: [OK] Created new PolicyApproval record:")
        print(f"  - ApprovalId: {new_approval.ApprovalId}")
...
def list_policy_approvals_for_reviewer(request):
    ...
    print(f"[DEBUG] DEBUG: list_policy_approvals_for_reviewer called with framework_id: {framework_id}")
    ...
    if framework_id:
        print(f"[DEBUG] DEBUG: Filtering reviewer policy approvals by framework_id: {framework_id}")
        ...
        print(f"[OK] Framework filter applied to reviewer approvals through Policy relationship. Found {approvals.count()} policy approvals.")
...
def all_policies_get_policy_versions(request, policy_id):
    ...
    try:
        ...
        try:
            ...
            # PolicyVersion doesn't have tenant_id, filter through PolicyId relationship
            direct_versions = list(PolicyVersion.objects.filter(
                PolicyId=policy,
                PolicyId__tenant_id=tenant_id
            ))
    ...
    try:
        ...
        if user_id:
            ...
            if selected_framework_id:
                print(f"[OK] DEBUG: Found framework_id in framework context: {selected_framework_id}")
        ...
        if not selected_framework_id:
            ...
            if session_framework_id:
                ...
                print(f"[WARNING] DEBUG: Found framework_id in session (fallback): {selected_framework_id}")
        ...
        if not selected_framework_id and request.data.get('frameworkId'):
            ...
            print(f"[OK] DEBUG: Found framework_id in request data: {selected_framework_id}")
        
        print(f"[DEBUG] DEBUG: Policy creation - checking for selected framework")
        print(f"[STATS] DEBUG: Selected framework ID: {selected_framework_id}")
        print(f"[USER] DEBUG: User ID: {user_id}")
        print(f"[DEBUG] DEBUG: Session keys: {list(request.session.keys()) if hasattr(request, 'session') else 'No session'}")
        print(f"[INFO] DEBUG: Request data framework ID: {request.data.get('frameworkId', 'Not provided')}")
        ...
        with transaction.atomic():
            ...
            if existing_approval:
                print(f"DEBUG: [WARNING] Duplicate prevention: Policy approval already exists for PolicyId {new_policy.PolicyId} with ApprovalId: {existing_approval.ApprovalId}")
                print(f"  - Skipping duplicate creation")
                ...
                print(f"DEBUG: [OK] Created new policy approval: {policy_approval.ApprovalId}")
...
def save_policy_category(request):
    ...
    print(f"[DEBUG] DEBUG: save_policy_category endpoint called!")
    print(f"[DEBUG] DEBUG: Request method: {request.method}")
    print(f"[DEBUG] DEBUG: Request data: {request.data}")
    print(f"[DEBUG] DEBUG: Request user: {request.user}")
    print(f"[DEBUG] DEBUG: Request session: {dict(request.session) if hasattr(request, 'session') else 'No session'}")
    ...
    try:
        ...
        if not selected_framework_id:
            session_framework_id = request.session.get('selected_framework_id') or request.session.get('grc_framework_selected')
            print(f"[DEBUG] DEBUG: Session framework_id: {session_framework_id}")
            if session_framework_id:
                selected_framework_id = session_framework_id
                print(f"[WARNING] DEBUG: Using session framework_id (fallback): {selected_framework_id}")
        
        # If still no framework_id, try request data
        if not selected_framework_id and request.data.get('frameworkId'):
            selected_framework_id = request.data.get('frameworkId')
            print(f"[OK] DEBUG: Using request data framework_id: {selected_framework_id}")
        
        # Get target framework
        target_framework = None
        print(f"[DEBUG] DEBUG: save_policy_category - selected_framework_id: {selected_framework_id}")
        print(f"[DEBUG] DEBUG: save_policy_category - session keys: {list(request.session.keys()) if hasattr(request, 'session') else 'No session'}")
        print(f"[DEBUG] DEBUG: save_policy_category - request.data: {request.data}")
        
        if selected_framework_id:
            try:
                target_framework = Framework.objects.get(FrameworkId=selected_framework_id, tenant_id=tenant_id)
                print(f"[OK] DEBUG: Found target framework: {target_framework.FrameworkName} (ID: {target_framework.FrameworkId})")
            except Framework.DoesNotExist:
                print(f"[ERROR] DEBUG: Framework {selected_framework_id} not found")
                return Response({"error": "Selected framework not found"}, status=status.HTTP_400_BAD_REQUEST)
        else:
            print(f"[ERROR] DEBUG: No framework selected")
        ...
        print(f"[DEBUG] DEBUG: Creating PolicyCategory with FrameworkId: {target_framework.FrameworkId}")
        ...
        print(f"[OK] DEBUG: Created PolicyCategory with ID: {new_category.Id}, FrameworkId: {new_category.FrameworkId}")
        ...
        print(f"[ERROR] ERROR in save_policy_category: {str(e)}")
        import traceback
...
def list_policy_approvals_for_user(request, user_id):
    ...
    try:
        framework_id = request.GET.get('framework_id', None)
        print(f"[DEBUG] DEBUG: list_policy_approvals_for_user called with framework_id: {framework_id}")
        print(f"[DEBUG] DEBUG: framework_id type: {type(framework_id)}")
        print(f"[DEBUG] DEBUG: All request GET params: {dict(request.GET)}")
        ...
        # Apply framework filter if provided
        if framework_id:
            print(f"[DEBUG] DEBUG: Filtering policy approvals by framework_id: {framework_id}")
            # Convert to integer if it's a string
            try:
                framework_id_int = int(framework_id)
                print(f"[DEBUG] DEBUG: Converted framework_id to int: {framework_id_int}")
                
                # Filter through Policy relationship: PolicyApproval -> Policy -> FrameworkId
                policy_approvals = policy_approvals.filter(PolicyId__FrameworkId=framework_id_int)
                print(f"[OK] Framework filter applied through Policy relationship. Found {policy_approvals.count()} policy approvals.")
                ...
                print(f"[DEBUG] DEBUG: Total approvals for user {user_id}: {total_approvals}")
                print(f"[DEBUG] DEBUG: Filtered approvals for framework {framework_id_int}: {policy_approvals.count()}")
                
                # Debug: Show which frameworks actually have policy approvals for this user
                if policy_approvals.count() == 0:
                    print(f"[WARNING] DEBUG: No policy approvals found for framework {framework_id_int}")
                    ...
                    print(f"[DEBUG] DEBUG: Frameworks with policy approvals for user {user_id}: {list(frameworks_with_approvals)}")
                    ...
                    try:
                        ...
                        if frameworks_with_approvals:
                            ...
                            print(f"[DEBUG] DEBUG: Framework names with data: {list(framework_names)}")
                    except Exception as e:
                        print(f"[ERROR] ERROR: Could not fetch framework names: {e}")
                
            except (ValueError, TypeError) as e:
                print(f"[ERROR] ERROR: Could not convert framework_id to int: {e}")
                print(f"[DEBUG] DEBUG: Using original framework_id value: {framework_id}")
                policy_approvals = policy_approvals.filter(PolicyId__FrameworkId=framework_id)
                print(f"[OK] Framework filter applied through Policy relationship (as string). Found {policy_approvals.count()} policy approvals.")
...
def list_policy_approvals_for_reviewer_by_id(request, user_id):
    ...
    try:
        framework_id = request.GET.get('framework_id', None)
        print(f"[DEBUG] DEBUG: list_policy_approvals_for_reviewer_by_id called with framework_id: {framework_id}")
        print(f"[DEBUG] DEBUG: framework_id type: {type(framework_id)}")
        print(f"[DEBUG] DEBUG: All request GET params: {dict(request.GET)}")
        ...
        if framework_id:
            ...
            try:
                ...
                # Filter through Policy relationship: PolicyApproval -> Policy -> FrameworkId
                policy_approvals = policy_approvals.filter(PolicyId__FrameworkId=framework_id_int)
                print(f"[OK] Framework filter applied to reviewer approvals through Policy relationship. Found {policy_approvals.count()} policy approvals.")
                
                # Additional debug: Check if any approvals exist
                total_approvals = PolicyApproval.objects.filter(PolicyId__tenant=tenant_id, ReviewerId=user_id).count()
                print(f"[DEBUG] DEBUG: Total reviewer approvals for user {user_id}: {total_approvals}")
                print(f"[DEBUG] DEBUG: Filtered reviewer approvals for framework {framework_id_int}: {policy_approvals.count()}")
                
                # Debug: Show which frameworks actually have reviewer policy approvals for this user
                if policy_approvals.count() == 0:
                    print(f"[WARNING] DEBUG: No reviewer policy approvals found for framework {framework_id_int}")
                    ...
                    print(f"[DEBUG] DEBUG: Frameworks with reviewer policy approvals for user {user_id}: {list(frameworks_with_reviewer_approvals)}")
                    ...
                    try:
                        ...
                        if frameworks_with_reviewer_approvals:
                            ...
                            print(f"[DEBUG] DEBUG: Framework names with reviewer data: {list(framework_names)}")
                    except Exception as e:
                        print(f"[ERROR] ERROR: Could not fetch framework names: {e}")
                ...
                print(f"[DEBUG] DEBUG: Using original framework_id value: {framework_id}")
                policy_approvals = policy_approvals.filter(PolicyId__FrameworkId=framework_id)
                print(f"[OK] Framework filter applied to reviewer approvals through Policy relationship (as string). Found {policy_approvals.count()} policy approvals.")
