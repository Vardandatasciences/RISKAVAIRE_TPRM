...
print(f"[API] Ollama Configuration (OPTIMIZED):")
...
def detect_and_parse_risk_blocks(text: str) -> list[dict]:
    ...
    for i, match in enumerate(risk_matches):
        ...
        print(f"  [LIST] Risk {risk_num}: {risk_title[:60]}...")
...
def infer_single_field(field_name: str, current_record: dict, document_context: str) -> tuple[Any, dict]:
    ...
    print(f"[AI] AI PREDICTING FIELD: {field_name} (OPTIMIZED)")
...
"""
    try:
        print(f"   [UPLOAD] Sending prompt to Ollama ({model}) for {field_name}...")
...
def parse_risks_from_text(text: str) -> list[dict]:
    ...
    """
    print(f"[STATS] parse_risks_from_text() called with {len(text)} chars of text (OPTIMIZED)")
    ...
    print(f"[OK] Detected {len(detected_risks)} risk(s), now processing each (OPTIMIZED)...")
    
    # Step 2: Process each risk with optimized AI
    completed_risks = []
    for idx, risk_data in enumerate(detected_risks, 1):
        print(f"\n[PROC] Processing Risk {idx}: {risk_data.get('RiskTitle', 'Unknown')[:50]}... (OPTIMIZED)")
        ...
        if missing_fields:
            ...
            print(f"  [AI] Using OPTIMIZED Ollama to infer missing fields...")
        ...
        print(f"  [OK] Risk {idx} completed! (OPTIMIZED)")
...
print(f"[UPLOAD] Upload request for risk document (OPTIMIZED VERSION)")
    ...
    try:
        ...
        try:
            print(f"[DEBUG] STEP 1: Starting text extraction from {ext} file... (OPTIMIZED)")
            text = extract_text_from_file(file_path, ext)
            
            if not text or len(text.strip()) < 50:
                print(f"[ERROR] ERROR: Could not extract meaningful text. Length: {len(text) if text else 0}")
                resp = JsonResponse({'status': 'error', 'message': 'Could not extract meaningful text from document'}, status=400)
                resp['Access-Control-Allow-Origin'] = '*'
                return resp

            print(f"[OK] STEP 1 COMPLETE: Extracted {len(text)} characters from document")
            
            print(f"[AI] STEP 2: Calling OPTIMIZED Ollama model to extract risks...")
            risks = parse_risks_from_text(text)
            
            print(f"[OK] STEP 2 COMPLETE: OPTIMIZED AI extracted {len(risks)} risk(s) from document")
