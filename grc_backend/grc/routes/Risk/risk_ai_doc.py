...
if AI_PROVIDER not in ['openai', 'ollama']:
    ...
    if OLLAMA_BASE_URL and OLLAMA_MODEL_DEFAULT:
        AI_PROVIDER = 'ollama'
        print("[DEBUG] Auto-selected Ollama as AI provider (Ollama configured)")
    elif OPENAI_API_KEY:
        AI_PROVIDER = 'openai'
        print("[DEBUG] Auto-selected OpenAI as AI provider (OpenAI configured)")
    else:
        AI_PROVIDER = 'openai'  # Default fallback
        print("[WARNING]  WARNING: No AI provider fully configured, defaulting to OpenAI")
...
if AI_PROVIDER == 'openai':
    if not OPENAI_API_KEY:
        ...
        print(f"[OpenAI] OpenAI Configuration:")
...
def _call_ollama_json_internal(prompt: str, model: str = None, retries: int = 3, timeout: int = None) -> Any:
    ...
    if len(prompt) > optimal_context:
        ...
        print(f"[INFO] Optimized context: {len(prompt)} chars (reduced from larger size)")
    ...
    print(f"[AI] Calling Ollama API (OPTIMIZED)")
    print(f"   Model: {model}")
    print(f"   Prompt length: {len(prompt)} chars")
    print(f"   URL: {url}")
    
    for attempt in range(retries):
        print(f"[AI] Attempt {attempt + 1}/{retries}...")
        resp = None
        try:
            ...
            print(f"[OK] Ollama API responded with status {resp.status_code} in {elapsed:.2f}s")
            
            response_data = resp.json()
            raw = response_data.get("response", "")
            
            print(f"[INFO] Response length: {len(raw)} chars")
            print(f"[INFO] First 200 chars: {raw[:200]}...")
            
            result = _json_from_llm_text(raw)
            print(f"[OK] Successfully parsed JSON from Ollama response")
            return result
            
        except json.JSONDecodeError as je:
            print(f"[ERROR] JSON parsing error on attempt {attempt + 1}: {je}")
            if attempt < retries - 1:
                print(f"⏳ Retrying in 1 second...")
                time.sleep(1)
                continue
            print(f"[ERROR] All retries exhausted. Raw response: {raw[:500] if 'raw' in locals() else 'N/A'}...")
            raise RuntimeError(f"Failed to parse JSON from Ollama response after {retries} attempts")
            
        except requests.exceptions.HTTPError as he:
            print(f"[ERROR] HTTP error on attempt {attempt + 1}: {he}")
            if resp is None:
            ...
            try:
                ...
                print(f"[DEBUG] Ollama Error: {error_message}")
            except:
            ...
            print(f"[ERROR] Connection error on attempt {attempt + 1}: {ce}")
            if attempt < retries - 1:
            ...
            print(f"[ERROR] Timeout error on attempt {attempt + 1}: {te}")
            if attempt < retries - 1:
                print(f"⏳ Retrying in 2 seconds...")
                time.sleep(2)
                continue
...
def _call_openai_json_internal(prompt: str, retries: int = 3, timeout: int = 120) -> Any:
    ...
    print(f"[DEBUG] Model check - Original: '{OPENAI_MODEL}', Cleaned: '{model_clean}', Lower: '{model_lower}'")
    
    # Explicitly exclude gpt-4o-mini first (it contains "gpt-4o" but doesn't support json_object)
    if "gpt-4o-mini" in model_lower:
        supports_json_format = False
        print(f"[DEBUG] Model '{model_clean}' is gpt-4o-mini - NOT adding response_format")
    else:
        ...
        print(f"[DEBUG] Model '{model_clean}' supports_json_format check result: {supports_json_format}")
    ...
    if supports_json_format:
        ...
        print(f"[OK] Adding response_format: json_object (model '{model_clean}' supports it)")
    else:
        print(f"[WARNING]  NOT adding response_format (model '{model_clean}' does not support json_object)")
    
    print(f"[AI] Calling OpenAI API at {OPENAI_API_URL}")
    print(f"[AI] Model: {model_clean}")
    print(f"[AI] Prompt length: {len(prompt)} chars")
    print(f"[DEBUG] Payload keys: {list(payload.keys())}")
    if 'response_format' in payload:
        print(f"[DEBUG] response_format value: {payload['response_format']}")
    else:
        print(f"[DEBUG] response_format: NOT in payload")
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            resp.raise_for_status()
            print(f"[OK] OpenAI API responded with status {resp.status_code}")
            ...
            result = _json_from_llm_text(raw)
            print(f"[OK] Successfully parsed JSON from OpenAI response")
            return result
            ...
            if resp is None:
                print(f"[WARNING]  Response object is None - error occurred before response was received")
            ...
            try:
                ...
                print(f"[DEBUG] Raw response text (first 1000 chars): {response_text[:1000]}")
                ...
                try:
                    ...
                    if isinstance(error_message, dict):
                        ...
                        print(f"[DEBUG] OpenAI Error Details:")
                        print(f"   Type: {error_type}")
                        print(f"   Code: {error_code}")
                        print(f"   Message: {error_detail}")
                        print(f"   Full error object: {error_message}")
                    else:
                        print(f"[DEBUG] OpenAI Error Response (non-dict): {error_response}")
                except (ValueError, AttributeError, json.JSONDecodeError) as json_err:
                    print(f"[WARNING]  Response is not valid JSON: {json_err}")
            ...
            if resp.status_code == 401:
                ...
                print(f"[WARNING]  Rate limit exceeded. Waiting 5 seconds...")
                if attempt < retries - 1:
                    time.sleep(5)
                    continue
            elif resp.status_code >= 500:
                print(f"[WARNING]  OpenAI server error. Retrying...")
                if attempt < retries - 1:
                    time.sleep(2)
                    continue
            elif resp.status_code == 400:
                # For 400 errors, log the payload to help debug
                print(f"[DEBUG] Debugging 400 error - Payload being sent:")
...
def detect_and_parse_risk_blocks(text: str) -> list[dict]:
    ...
    """
    print(f"[DEBUG] Detecting risk blocks in document...")
    ...
    if not risk_matches:
        print(f"[WARNING]  No risks found with 'Risk X:' pattern")
        return []
    
    print(f"[OK] Found {len(risk_matches)} risk(s) in document")
    ...
    for i, match in enumerate(risk_matches):
        ...
        print(f"  [RISK] Risk {risk_num}: {risk_title[:60]}...")
...
def infer_single_field(field_name: str, current_record: dict, document_context: str, 
    ...
    if is_rag_available():
        try:
            ...
            if retrieved:
                ...
                print(f"   [RAG] Phase 3 RAG: Retrieved {len(retrieved)} relevant document chunks")
        except Exception as e:
    ...
    try:
        ...
        print(f"   [RAG] Using few-shot prompt template for {field_name}")
    except Exception as e:
...
"""
    ...
    try:
        if AI_PROVIDER == 'ollama':
            print(f"   [INFO] Sending prompt to Ollama for {field_name}...")
            ...
            print(f"   [INFO] Sending prompt to OpenAI for {field_name}...")
...
def parse_risks_from_text(text: str, document_hash: str = None) -> list[dict]:
    ...
    """
    print(f"[STATS] parse_risks_from_text() called with {len(text)} chars of text")
    
    # Step 1: Detect and parse risk blocks from document
    detected_risks = detect_and_parse_risk_blocks(text)
    
    if not detected_risks:
        print(f"[WARNING]  No structured risks found. Falling back to old AI extraction...")
        # Fallback to old behavior if no "Risk X:" pattern found
        return fallback_risk_extraction(text)
    
    print(f"[OK] Detected {len(detected_risks)} risk(s), now processing each...")
    ...
    for idx, risk_data in enumerate(detected_risks, 1):
        print(f"\n[PROC] Processing Risk {idx}: {risk_data.get('RiskTitle', 'Unknown')[:50]}...")
...
def upload_and_process_risk_document(request):
    ...
    print(f"[INFO] Upload request for risk document")
    print(f"[INFO] Request data: {request.POST}")
    print(f"[INFO] Request files: {request.FILES}")
    print(f"[INFO] User ID: {request.POST.get('user_id', 'unknown')}")
    ...
    try:
        ...
        if file_name.endswith('.gz'):
            ...
            print(f"[FILE] Detected compressed file: {file_name} → {original_filename}")
        ...
        try:
            ...
            print(f"[OK] STEP 1A COMPLETE: Extracted {len(raw_text)} characters from document")
            
            # Step 1B: Preprocess document (Phase 2 optimization)
            print(f"[DEBUG] STEP 1B: Preprocessing document (Phase 2 optimization)...")
            ...
            print(f"[INFO] Document hash: {document_hash[:16]}... (for caching)")
            
            print(f"[DOC] First 200 chars: {text[:200]}...")
            
            # Step 2: Check AI provider configuration
            print(f"[DEBUG] STEP 2: Checking AI provider configuration...")
            ...
            print(f"[OK] STEP 2 COMPLETE: {AI_PROVIDER.upper()} provider is configured")
            ...
            print(f"[AI] STEP 3: Calling {provider_info} to extract risks (Phase 2+3: cached + few-shot + RAG + routing)...")
            ...
            if len(text) > 10000:  # Large documents use queue
                print(f"[RISK] Large document detected, using Phase 3 queuing...")
            ...
            if is_rag_available():
                try:
                    ...
                    print(f"[OK] Phase 3 RAG: Document added to knowledge base")
                except Exception as e:
                    print(f"[WARNING]  Phase 3 RAG: Failed to add document: {e}")
            
            print(f"[OK] STEP 3 COMPLETE: AI extracted {len(risks)} risk(s) from document")
