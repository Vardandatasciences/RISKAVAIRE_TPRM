...
def me():
    ...
    try:
        ...
        if company_resp.status_code == 200:
            ...
            print("[EMOJI] Company info retrieved")
        else:
    ...
    try:
        ...
        if directory_resp.status_code == 200:
            ...
            print("[EMOJI] Employee directory retrieved")
        else:
    ...
    if data_collected:
        ...
        <ul>
        {''.join([f'<li><strong>{key.replace("_", " ").title()}:</strong> Retrieved [EMOJI]</li>' for key in data_collected.keys()])}
        </ul>
        <hr>
        <h3>Raw Data:</h3>
