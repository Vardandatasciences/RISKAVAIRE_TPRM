...
if USE_LOCAL:
    ...
    print("[GLOBAL] Running in LOCAL development mode")
else:
    ...
    FRONTEND_URL = os.environ.get("FRONTEND_URL", "http://localhost:8080")
    print("[GLOBAL] Running in PRODUCTION mode")
...
def login():
    ...
    print(f"[LINK] Redirecting to BambooHR OAuth: {oauth_url}")
...
def oauth_callback():
    ...
    try:
        ...
        if not code:
            ...
            print(f"[ERROR] OAuth error: {error_description}")
        ...
        print(f"[EMOJI] Exchanging code for token with {subdomain}.bamboohr.com")
        ...
        try:
            ...
            if company_resp.status_code == 200:
                ...
                print(f"[OK] Token verified with company: {company_info}")
            else:
                print(f"[WARNING] Could not verify token with company API: {company_resp.status_code}")
        except Exception as e:
            print(f"[WARNING] Error verifying token: {e}")
        ...
        try:
            ...
            print(f"[EMOJI] Notifying Django backend: {callback_url}")
            django_resp = requests.post(callback_url, json=callback_data, timeout=30)
            
            if django_resp.status_code == 200:
                print("[OK] Django backend notified successfully")
            else:
                print(f"[WARNING] Django backend notification failed: {django_resp.status_code}")
        
        except Exception as e:
            print(f"[WARNING] Error notifying Django backend: {e}")
        ...
        print(f"[LINK] Redirecting to frontend: {frontend_callback_url}")
...
if __name__ == "__main__":
    ...
    print(f"[INFO] Starting BambooHR OAuth Server on port {port}")
    print(f"[PROC] Debug mode: {debug}")
    print(f"[GLOBAL] Environment: {'LOCAL' if USE_LOCAL else 'PRODUCTION'}")
    print(f"[API] Django Backend: {DJANGO_BACKEND_URL}")
    print(f"[API] Frontend URL: {FRONTEND_URL}")
    print(f"[LINK] Redirect URI: {REDIRECT_URI}")
    print(f"[KEY] Client ID configured: {'Yes' if CLIENT_ID else 'No'}")
    print(f"[SECURE] Client Secret configured: {'Yes' if CLIENT_SECRET else 'No'}")
