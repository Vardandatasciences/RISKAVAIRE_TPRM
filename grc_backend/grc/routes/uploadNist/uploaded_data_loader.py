...
def get_media_root() -> Path:
    ...
    try:
        ...
        if hasattr(settings, 'MEDIA_ROOT') and settings.MEDIA_ROOT and settings.MEDIA_ROOT != '.':
            ...
            logger.info(f"[EMOJI] Using MEDIA_ROOT from settings: {media_root}")
    ...
    logger.info(f"[EMOJI] Using default MEDIA_ROOT: {media_root}")
    
    if not media_root.exists():
        logger.error(f"[EMOJI] MEDIA_ROOT does not exist: {media_root}")
        ...
        for item in upload_folder.iterdir():
            if item.is_dir() and item.name.startswith('sections_'):
                ...
                if sections_subfolder.exists() and sections_subfolder.is_dir():
                    ...
                    logger.info(f"[EMOJI] Found sections subfolder: {sections_folder}")
                    break
                else:
                    # Use the main folder if no subfolder
                    sections_folder = item
                    logger.info(f"[EMOJI] Found sections folder: {sections_folder}")
                    break
    except Exception as e:
        logger.error(f"Error searching for sections folder: {str(e)}")
    
    if not sections_folder:
        logger.error(f"[EMOJI] No sections folder found in {upload_folder}")
    ...
    try:
        for item in upload_folder.iterdir():
            if item.is_dir() and item.name.startswith('policies_'):
                ...
                logger.info(f"[EMOJI] Found policies folder: {policies_folder}")
                break
    except Exception as e:
        logger.error(f"Error searching for policies folder: {str(e)}")
    
    if not policies_folder:
        logger.error(f"[EMOJI] No policies folder found in {upload_folder}")
        return None
...
def get_sections_index_json(user_id: str) -> Optional[Dict]:
        ...
        for item in upload_folder.iterdir():
            if item.is_dir() and item.name.startswith('sections_'):
                ...
                if potential_index.exists():
                    ...
                    logger.info(f"[EMOJI] Found sections_index.json: {sections_index_path}")
                    break
    except Exception as e:
    ...
    try:
        with open(sections_index_path, 'r', encoding='utf-8') as f:
            ...
            logger.info(f"[EMOJI] Successfully loaded sections_index.json for user {user_id}")
...
def get_pdf_index_json(user_id: str) -> Optional[Dict]:
    ...
    try:
        for item in upload_folder.iterdir():
            if item.is_file() and item.name.endswith('_index.json'):
                ...
                logger.info(f"[EMOJI] Found index JSON file: {index_json_path}")
                break
    except Exception as e:
    ...
    try:
        with open(index_json_path, 'r', encoding='utf-8') as f:
            ...
            logger.info(f"[EMOJI] Successfully loaded index JSON for user {user_id}")
            return index_data
    except Exception as e:
...
def build_sections_from_folders(sections_dir: Path) -> List[Dict]:
    ...
    logger.info(f"[EMOJI] Built {len(sections)} sections from folders")
    return sections
...
def build_sections_from_index(index_data: Dict) -> List[Dict]:
    ...
    logger.info(f"[EMOJI] Built {len(sections)} sections from index")
    return sections
...
def _get_policies_for_section_internal(policies_data: List[Dict], section_folder: str) -> List[Dict]:
    ...
    try:
        ...
        for policy_entry in policies_data:
            ...
            if is_match:
                logger.info(f"[MATCH] [EMOJI] Found match: '{entry_folder}' matches '{section_folder}'")
                ...
                for policy in entry_policies:
                    ...
                    logger.info(f"[MATCH] [EMOJI] Added policy: '{policy_title}' with {len(subpolicies)} subpolicies")
    ...
    logger.info(f"[MATCH] [EMOJI] FINAL: Found {len(policies)} total policies for section '{section_folder}'")
    return policies
...
def build_complete_structure(user_id: str) -> List[Dict]:
    ...
    if not sections:
        logger.error("[BUILD] [EMOJI] No sections found!")
        return []
    
    logger.info(f"[BUILD] [EMOJI] Built {len(sections)} sections")
    ...
    if policies_data:
        ...
        for idx, section in enumerate(sections):
            ...
            logger.info(f"[BUILD] [EMOJI] '{section_title}': {total_policies} policies, {total_subpolicies} subpolicies")
    else:
        logger.error("[BUILD] [EMOJI] No policies data available!")
    
    logger.info(f"=" * 80)
    logger.info(f"[BUILD] [EMOJI] Complete structure built with {len(sections)} sections")
    logger.info(f"=" * 80)
    return sections
...
def get_uploaded_data_sections(request, user_id):
    ...
    try:
        ...
        logger.info(f"API: [EMOJI] Returning {len(sections)} sections, {total_policies} policies, {total_subpolicies} subpolicies")
