...
print("\n[AI] Incident Import AI Provider Configuration:")
...
def parse_incidents_from_text(text: str, document_hash: str = None) -> list[dict]:
    ...
    """
    print(f"[STATS] parse_incidents_from_text() called with {len(text)} chars of text")
    ...
    if is_rag_available():
        try:
            ...
            if retrieved:
                rag_context = retrieved
                print(f"   [DATA] Phase 3 RAG: Retrieved {len(retrieved)} relevant document chunks for incident extraction")
        except Exception as e:
            print(f"   [WARNING]  RAG retrieval failed: {e}")
...
Begin analysis now and return the JSON object:"""
    ...
    print(f"[INFO] Generated prompt for AI (length: {len(prompt)} chars)")
    
    try:
        provider_info = f"{AI_PROVIDER.upper()} ({OPENAI_MODEL if AI_PROVIDER == 'openai' else OLLAMA_MODEL_DEFAULT})"
        print(f"[INFO] Calling {provider_info} to extract incidents (Phase 2+3: cached + few-shot + RAG + routing)...")
        print(f"[STATS] Processing document with {len(text)} characters")
        ...
        print(f"[OK] OpenAI returned {len(incidents)} incident(s)")

        cleaned = []
        for idx, inc in enumerate(incidents, 1):
            print(f"[LIST] Processing incident {idx}/{len(incidents)}")
            ...
            print(f"[DEBUG] Checking missing fields for incident: {item.get('IncidentTitle', 'Untitled')}")
            missing_fields = []
            ...
            if missing_fields:
                print(f"   [INFO] Missing fields to predict: {missing_fields}")
                ...
                print(f"   [OK] All fields already populated")
            ...
            if "_meta" in item and "per_field" in item["_meta"]:
                ...
                if ai_fields:
                    print(f"   [AI] AI Generated fields: {ai_fields}")
                else:
                    print(f"   [DOC] No AI generated fields for this incident")
            else:
                print(f"   [DOC] No metadata available for this incident")
...
def upload_and_process_incident_document(request):
    ...
    """
    print(f"[UPLOAD] Upload request for incident document")
    ...
    try:
        ...
        try:
            ...
            provider_info = f"{AI_PROVIDER.upper()} ({OPENAI_MODEL if AI_PROVIDER == 'openai' else OLLAMA_MODEL_DEFAULT})"
            print(f"[AI] STEP 3: Calling {provider_info} to extract incidents (Phase 2+3: cached + few-shot + RAG + routing)...")
            ...
            if len(text) > 10000:  # Large documents use queue
                print(f"[LIST] Large document detected, using Phase 3 queuing...")
            ...
            print(f"[OK] STEP 3 COMPLETE: AI extracted {len(incidents)} incident(s) from document")
...
def save_extracted_incidents(request):
    ...
    # MULTI-TENANCY: Extract tenant_id from request
    tenant_id = get_tenant_id_from_request(request)
    print(f"[SAVE] Save incidents request received")
    ...
    try:
        ...
        try:
            ...
            print(f"[WARNING]  Error reading request.body: {body_error}")
            # Fallback to request.data if available
            if hasattr(request, 'data') and request.data:
                data = request.data
                print(f"[OK] Using request.data as fallback")
            else:
                raise body_error
        ...
        print(f"[SAVE] Processing {len(incidents_data)} incident(s) for user {user_id}")
        ...
        for idx, inc in enumerate(incidents_data):
            try:
                ...
                print(f"[OK] Saved incident {idx+1}: {incident.IncidentTitle} (ID: {incident.IncidentId})")
                saved.append({'incident_id': incident.IncidentId, 'incident_title': incident.IncidentTitle})
            except Exception as ex:
                print(f"[ERROR] Error saving incident {idx+1}: {str(ex)}")
                errors.append({'incident_index': idx, 'title': inc.get('IncidentTitle'), 'error': str(ex)})

        print(f"[SAVE] Save complete: {len(saved)} saved, {len(errors)} errors")
