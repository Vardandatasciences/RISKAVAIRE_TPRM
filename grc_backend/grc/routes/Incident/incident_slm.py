...
print("\n[AI] Incident SLM AI Provider Configuration:")
    ...
    def __init__(self, api_key=None):
        ...
        if self.provider == 'ollama':
            if not OLLAMA_BASE_URL:
                ...
                print("[OK] Ollama integration initialized for incident analysis")
        ...
        if not api_key or api_key == 'your-openai-api-key-here' or str(api_key).startswith('YOUR_OPE'):
            ...
            self.is_available = True
            print("[OK] OpenAI integration initialized successfully for incident analysis")
    ...
    def generate_response(self, prompt, model=None, max_tokens=2000, temperature=0.3, document_hash: str | None = None):
        ...
        if self.provider == 'ollama':
            try:
                ...
                print(f"[ERROR] Ollama JSON call error (incident_slm): {type(e).__name__}: {e}")
                traceback.print_exc()
                return None
        ...
        try:
            ...
            error_message = str(e)
            print(f"[ERROR] AI JSON call error (incident_slm): {error_type}: {error_message}")
            traceback.print_exc()
            return None
...
def analyze_incident_comprehensive(incident_title, incident_description):
    ...
    try:
        ...
        print("[EMOJI] Using AI for incident analysis (with Phase 2+3 optimizations)")
        ...
        print(f"[INFO] Incident (comprehensive) document hash: {document_hash[:16]}...")
        ...
        if is_rag_available():
            try:
                ...
                if rag_context:
                    print(f"   [DATA] Phase 3 RAG (incident_slm): Retrieved {len(rag_context)} relevant chunks")
            except Exception as e:
                print(f"   [WARNING]  RAG retrieval failed in incident_slm: {e}")
...
Provide ONLY the JSON response, no additional text."""
        ...
        print(f"[STATS] Analyzing incident: {incident_title}")
        ...
        def _do_analysis():
            ...
            print(f"⏱[EMOJI] Incident SLM processing_time={processing_time:.2f}s")
        ...
        try:
            ...
            for field in list_fields:
                if not isinstance(incident_analysis.get(field), list):
                    ...
                    if isinstance(incident_analysis.get(field), str):
                        ...
                        print(f"[WARNING] Field {field} is not a list, falling back")
                        return generate_comprehensive_fallback_analysis(incident_title, incident_description)
            
            print(f"[OK] Successfully parsed comprehensive banking GRC incident analysis")
            ...
            if is_rag_available():
                try:
                    ...
                    print("[OK] Phase 3 RAG (incident_slm): Incident analysis added to knowledge base")
                except Exception as e:
                    print(f"[WARNING]  Phase 3 RAG (incident_slm): Failed to add document: {e}")
