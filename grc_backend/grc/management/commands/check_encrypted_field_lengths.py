...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
                    ...
                    try:
                        ...
                        if hasattr(field, 'max_length'):
                            ...
                            if max_length < 500:
                                ...
                                self.stdout.write(
                                    f"   [WARNING]  {field_name}: max_length={max_length} (TOO SMALL for encryption!)"
                                )
                                self.stdout.write(
                                ...
                                self.stdout.write(
                                    f"   [OK] {field_name}: max_length={max_length} (OK)"
                                )
                        elif hasattr(field, 'max_length') and field.max_length is None:
                            # TextField - no length limit, OK
                            self.stdout.write(f"   [OK] {field_name}: TextField (no limit, OK)")
                        else:
                            # Not a CharField, might be TextField
                            self.stdout.write(f"   ℹ[EMOJI]  {field_name}: {type(field).__name__} (check manually)")
                            
                    except Exception as e:
                        self.stdout.write(f"   [ERROR] {field_name}: Error - {str(e)}")
                        
            except Exception as e:
                self.stdout.write(f"\n[ERROR] Model {model_name}: Error - {str(e)}")
        ...
        if issues:
            self.stdout.write(f"\n[WARNING]  Found {len(issues)} fields that need length increases:\n")
            ...
            self.stdout.write("\n[OK] All encrypted CharField fields have sufficient length!")
