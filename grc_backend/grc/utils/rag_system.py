...
try:
    ...
    logger.warning(f"[WARNING]  ChromaDB not available. RAG features will be disabled. Error: {e}")
...
try:
    ...
    logger.warning("[WARNING]  sentence-transformers not installed. Will use ChromaDB default embeddings.")
...
def get_chroma_client():
    ...
    if _chroma_client is None:
        try:
            ...
            logger.info(f"[OK] ChromaDB client initialized at: {chroma_db_path}")
    ...
    if _chroma_collection is None:
        try:
            ...
            try:
                ...
                logger.info(f"[OK] ChromaDB collection '{collection_name}' retrieved")
            except:
            ...
            if "no such column" in error_msg.lower() or "schema" in error_msg.lower():
                logger.warning(f"[WARNING]  ChromaDB schema mismatch detected: {e}")
                logger.info("[INFO] Attempting to delete entire database and recreate...")
                try:
                    ...
                    if os.path.exists(chroma_db_path):
                        ...
                        logger.info(f"[EMOJI]  Deleted old ChromaDB database at: {chroma_db_path}")
                        os.makedirs(chroma_db_path, exist_ok=True)
                        ...
                        if client:
                            ...
                            logger.info(f"[OK] ChromaDB collection '{collection_name}' recreated with correct schema")
                        else:
                            logger.error(f"[ERROR] Failed to recreate ChromaDB client")
                            _chroma_collection = None
                    else:
                        logger.error(f"[ERROR] ChromaDB path not found: {chroma_db_path}")
                        _chroma_collection = None
                except Exception as e2:
                    logger.error(f"[ERROR] Failed to recreate database: {e2}")
                    _chroma_collection = None
            else:
                logger.error(f"[ERROR] Failed to get/create collection: {e}")
                _chroma_collection = None
...
def get_embedding_model():
    ...
    if _embedding_model is None:
        try:
            ...
            logger.warning(f"[WARNING]  Failed to load embedding model: {e}")
            _embedding_model = None
...
def add_document_to_rag(
    ...
    """
    if not CHROMADB_AVAILABLE:
        logger.warning("[WARNING]  ChromaDB not available, skipping RAG storage")
        return False
    
    collection = get_chroma_collection()
    if not collection:
        logger.warning("[WARNING]  ChromaDB collection not available, skipping RAG storage")
        return False
    ...
    try:
        ...
        if not chunks:
            logger.warning(f"[WARNING]  No chunks created for document {document_id}")
            return False
        ...
        logger.info(f"[OK] Added document {document_id} to RAG ({len(chunks)} chunks)")
        return True
        
    except Exception as e:
        logger.error(f"[ERROR] Failed to add document to RAG: {e}")
        return False
...
def retrieve_relevant_context(
    ...
    """
    if not CHROMADB_AVAILABLE:
        logger.warning("[WARNING]  ChromaDB not available, returning empty context")
        return []
    
    collection = get_chroma_collection()
    if not collection:
        logger.warning("[WARNING]  ChromaDB collection not available, returning empty context")
        return []
    ...
    try:
        ...
        logger.info(f"[OK] Retrieved {len(retrieved_chunks)} relevant chunks for query")
        return retrieved_chunks
        
    except Exception as e:
        logger.error(f"[ERROR] Failed to retrieve context: {e}")
        return []
