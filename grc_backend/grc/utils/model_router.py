...
def route_model(
    ...
    if system_load > 0.8:
        logger.info(f"[EMOJI] High system load ({system_load:.2f}), routing to fast model")
        return ollama_models["fast"]
    
    # Critical accuracy required -> use complex model
    if accuracy_required == "critical":
        logger.info(f"[TARGET] Critical accuracy required, routing to complex model")
    ...
    if accuracy_required == "high" and system_load < 0.7:
        logger.info(f"[TARGET] High accuracy required, routing to complex model")
        return ollama_models["complex"]
    
    # Simple task -> use fast model
    if complexity == "simple" and document_size < 2000:
        logger.info(f"[FAST] Simple task, routing to fast model")
        return ollama_models["fast"]
    
    # Complex task -> use complex model (if not overloaded)
    if complexity == "complex" and system_load < 0.7:
        logger.info(f"[EMOJI] Complex task, routing to complex model")
        return ollama_models["complex"]
    
    # Default -> balanced model
    logger.info(f"[EMOJI]  Balanced routing, using default model")
