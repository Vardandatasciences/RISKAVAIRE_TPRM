...
def decompress_if_needed(file_path: str) -> Tuple[str, bool, Optional[Dict]]:
    ...
    try:
        ...
        print(f"[WARNING]  Error reading compressed file {file_path}: {e}")
        return file_path, False, None
    ...
    try:
        ...
        print(f"[WARNING]  File has .gz extension but is not gzipped: {file_path}")
        return file_path, False, None
    except Exception as e:
        print(f"[WARNING]  Error decompressing file {file_path}: {e}")
        return file_path, False, None
    ...
    try:
        ...
        print(f"[WARNING]  Error writing decompressed file {decompressed_path}: {e}")
        return file_path, False, None
    ...
    try:
        os.remove(file_path)
        print(f"[OK] Decompressed upload: {compression_stats['ratio']}% reduction")
        print(f"   Bandwidth saved: {compression_stats['bandwidth_saved_kb']} KB")
    except Exception as e:
        print(f"[WARNING]  Warning: Could not remove compressed file {file_path}: {e}")
