...
class Command(BaseCommand):
    ...
    def handle_health(self, s3_service):
        ...
        self.stdout.write('[EMOJI] Checking S3 service health...')
        
        result = s3_service.test_connection()
        
        if result.get('overall_success'):
            self.stdout.write(self.style.SUCCESS('[EMOJI] S3 service is healthy'))
            ...
            self.stdout.write(self.style.ERROR('[EMOJI] S3 service is unhealthy'))
    ...
    def handle_upload(self, s3_service, options):
        ...
        self.stdout.write(f'[EMOJI] Uploading {file_path}...')
        ...
        if result.get('success'):
            self.stdout.write(self.style.SUCCESS('[EMOJI] Upload successful'))
            ...
            self.stdout.write(self.style.ERROR(f'[EMOJI] Upload failed: {result.get("error")}'))
    ...
    def handle_download(self, s3_service, options):
        ...
        self.stdout.write(f'[EMOJI]  Downloading {file_name} from S3...')
        ...
        if result.get('success'):
            self.stdout.write(self.style.SUCCESS('[EMOJI] Download successful'))
            self.stdout.write(f'   File Storage ID: {result.get("file_storage_id")}')
            self.stdout.write(f'   Local Path: {result.get("file_path")}')
            self.stdout.write(f'   File Size: {result.get("file_size")} bytes')
        else:
            self.stdout.write(self.style.ERROR(f'[EMOJI] Download failed: {result.get("error")}'))
    ...
    def handle_export(self, s3_service, options):
        ...
        self.stdout.write(f'[EMOJI] Exporting data as {export_format.upper()}...')
        ...
        if result.get('success'):
            self.stdout.write(self.style.SUCCESS('[EMOJI] Export successful'))
            self.stdout.write(f'   File Storage ID: {result.get("file_storage_id")}')
            self.stdout.write(f'   S3 URL: {result.get("s3_url")}')
            self.stdout.write(f'   S3 Key: {result.get("s3_key")}')
            self.stdout.write(f'   Stored Name: {result.get("stored_name")}')
            ...
            self.stdout.write(self.style.ERROR(f'[EMOJI] Export failed: {result.get("error")}'))
    ...
    def handle_export_rfp(self, s3_service, options):
        ...
        self.stdout.write(f'[EMOJI] Exporting RFP {rfp_id} as {export_format.upper()}...')
        ...
        if result.get('success'):
            self.stdout.write(self.style.SUCCESS('[EMOJI] RFP export successful'))
            self.stdout.write(f'   File Storage ID: {result.get("file_storage_id")}')
            self.stdout.write(f'   S3 URL: {result.get("s3_url")}')
            self.stdout.write(f'   S3 Key: {result.get("s3_key")}')
            self.stdout.write(f'   Stored Name: {result.get("stored_name")}')
            ...
            self.stdout.write(self.style.ERROR(f'[EMOJI] RFP export failed: {result.get("error")}'))
    ...
    def handle_history(self, s3_service, options):
        ...
        self.stdout.write('[EMOJI] File operation history:')
        ...
        for i, record in enumerate(history, 1):
            status_emoji = "[EMOJI]" if record['status'] == 'completed' else "[EMOJI]" if record['status'] == 'failed' else "⏳"
    ...
    def handle_stats(self, s3_service):
        ...
        self.stdout.write('[EMOJI] File operation statistics:')
    ...
    def handle_test(self, s3_service):
        ...
        self.stdout.write('[EMOJI] Testing S3 service connection...')
        
        result = s3_service.test_connection()
        
        if result.get('overall_success'):
            self.stdout.write(self.style.SUCCESS('[EMOJI] All systems operational'))
            ...
            self.stdout.write(self.style.ERROR('[EMOJI] Some systems need attention'))
            if result.get('direct_error'):
                self.stdout.write(f'   Direct Error: {result["direct_error"]}')
            if result.get('mysql_error'):
                self.stdout.write(f'   MySQL Error: {result["mysql_error"]}')
