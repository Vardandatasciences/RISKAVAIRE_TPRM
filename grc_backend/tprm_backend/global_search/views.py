...
class GlobalSearchViewSet(viewsets.ViewSet):
    ...
    def _track_search_analytics(self, query, results_count, response_time, filters_used):
        ...
        try:
            ...
            print(f"[EMOJI] Search analytics tracked: '{query[:50]}...' ({results_count} results, {response_time}ms)")
            
        except IntegrityError as e:
            print(f"[EMOJI] IntegrityError tracking search analytics: {e}")
            print(f"   Query: '{query[:100]}...'")
            print(f"   Results: {results_count}, Time: {response_time}")
        except DataError as e:
            print(f"[EMOJI] DataError tracking search analytics: {e}")
            print(f"   Query: '{query[:100]}...'")
            print(f"   Results: {results_count}, Time: {response_time}")
        except ValidationError as e:
            print(f"[EMOJI] ValidationError tracking search analytics: {e}")
            print(f"   Query: '{query[:100]}...'")
            print(f"   Results: {results_count}, Time: {response_time}")
        except Exception as e:
            print(f"[EMOJI] Unexpected error tracking search analytics: {type(e).__name__}: {e}")
            print(f"   Query: '{query[:100]}...'")
            print(f"   Results: {results_count}, Time: {response_time}")
            import traceback
            traceback.print_exc()
