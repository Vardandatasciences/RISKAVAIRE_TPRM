...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write('[EMOJI] Checking Search Index Status...\n')
        ...
        if total_count == 0:
            self.stdout.write(self.style.WARNING('[EMOJI]  No records found in search index!'))
            self.stdout.write('Run: python manage.py populate_search_index to index your data')
            return
        
        # Get counts by entity type
        self.stdout.write('\n[EMOJI] Records by Entity Type:')
        ...
        self.stdout.write('\n[EMOJI] Index Health Check:')
        
        # Check for empty titles
        empty_titles = SearchIndex.objects.filter(title='').count()
        if empty_titles > 0:
            self.stdout.write(self.style.WARNING(f'  [EMOJI]  {empty_titles} records with empty titles'))
        else:
            self.stdout.write('  [EMOJI] All records have titles')
        ...
        if duplicates:
            self.stdout.write(self.style.WARNING(f'  [EMOJI]  {len(duplicates)} duplicate entity entries found'))
        else:
            self.stdout.write('  [EMOJI] No duplicate entries found')
        
        self.stdout.write('\n[EMOJI] Search index status check completed!')
