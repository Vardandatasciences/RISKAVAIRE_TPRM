"""
Django Management Command: Encrypt Existing TPRM Data
Encrypts all existing plain text data in the TPRM database for configured fields.

Usage:
    python manage.py encrypt_tprm_data
    python manage.py encrypt_tprm_data --model Vendor --field company_name
    python manage.py encrypt_tprm_data --dry-run  # Preview changes without saving
"""
...
from tprm_backend.utils.data_encryption import encrypt_data, is_encrypted_data

logger = logging.getLogger(__name__)


class Command(BaseCommand):
    help = 'Encrypt existing plain text data in the TPRM database'

    def add_arguments(self, parser):
        parser.add_argument(
            '--model',
            type=str,
            help='Specific model to encrypt (e.g., Vendor, Contract)',
        )
        parser.add_argument(
            '--field',
            type=str,
        ...
        parser.add_argument(
            '--app',
            type=str,
            default='tprm_backend',
            help='Django app name (default: tprm_backend)',
        )
    ...
    def handle(self, *args, **options):
        ...
        app_name = options.get('app', 'tprm_backend')
        ...
        if model_name:
            try:
                ...
                model = None
                for app in ['vendors', 'contracts', 'slas', 'rfp', 'core', 'users']:
                    try:
        ...
        # Summary
        self.stdout.write('\n' + '=' * 60)
        self.stdout.write('TPRM ENCRYPTION SUMMARY')
