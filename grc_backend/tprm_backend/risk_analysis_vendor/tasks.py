...
def generate_entity_risks_task(entity, table, row_id):
    ...
    Args:
        entity: Entity name (e.g., 'vendor_module')
        table: Table name (e.g., 'vendor_profiles', 'vendor_contracts')
...
@shared_task
def generate_vendor_risks_task(approval_id):
    """
    Background task to generate vendor risks after response approval
    
    Args:
        approval_id: The approval request ID that was just approved
        
    Returns:
        dict: Task result with vendor risk generation status
    """
    try:
        logger.info(f"Starting background vendor risk generation for approval_id: {approval_id}")
        
        # Import here to avoid circular imports
        from .services import RiskAnalysisService
        
        # Generate vendor risks
        risk_service = RiskAnalysisService()
        result = risk_service.generate_vendor_risks(approval_id)
        
        if result['status'] == 'success':
            logger.info(f"Background task successfully created {result['risks_created']} vendor risks for approval {approval_id}")
        else:
            logger.error(f"Background vendor risk generation failed for approval {approval_id}: {result.get('error', 'Unknown error')}")
        
        return result
        
    except Exception as e:
        logger.error(f"Error in background vendor risk generation for approval {approval_id}: {str(e)}")
        return {
            'status': 'error',
