...
class SimpleAuthenticatedPermission(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        # More lenient check - allow if user exists and is authenticated, even without userid
        if request.user and getattr(request.user, 'is_authenticated', False):
            return True
        # Also allow if there's an Authorization header (authentication will be checked in view)
...
class JWTAuthentication(BaseAuthentication):
    """Custom JWT authentication class for DRF - supports both JWT and session tokens"""
    def authenticate(self, request):
        ...
        try:
            ...
            # Import User model at the top to avoid NameError
            try:
            ...
            if user_id:
                # Try to get user from User model if available
                if User is not None:
                    try:
                        ...
                        logger.info(f"User {user_id} not found in database, creating mock user")
                        pass
                ...
                class MockUser:
                    def __init__(self, user_id):
                        self.userid = user_id
                        self.id = user_id  # Also set id for compatibility
                        self.username = f"user_{user_id}"
                        self.is_authenticated = True
                
                return (MockUser(user_id), token)
            else:
                logger.warning(f"No user_id found in token payload. Available keys: {list(payload.keys())}")
                return None
            ...
            import traceback
            logger.error(f"JWT authentication traceback: {traceback.format_exc()}")
            return None
...
@api_view(['GET'])
@authentication_classes([JWTAuthentication])
@permission_classes([permissions.AllowAny])  # Allow any request, we'll check auth manually
def check_contract_create_permission(request):
    """Check if user has permission to create contracts"""
    logger.info(f"[PERMISSION CHECK] Endpoint called - Method: {request.method}, Path: {request.path}")
    logger.info(f"[PERMISSION CHECK] Headers: Authorization={bool(request.headers.get('Authorization'))}")
    logger.info(f"[PERMISSION CHECK] User: {request.user}, is_authenticated: {getattr(request.user, 'is_authenticated', False)}")
    ...
    try:
        ...
        # Get user_id from request
        user_id = RBACTPRMUtils.get_user_id_from_request(request)
        logger.info(f"[PERMISSION CHECK] Extracted user_id: {user_id}")
        
        # If no user_id, try to authenticate manually
        if not user_id:
            logger.info("[PERMISSION CHECK] No user_id from RBACTPRMUtils, trying manual authentication")
            ...
            if auth_result:
                ...
                logger.info(f"[PERMISSION CHECK] Manual auth successful, user_id: {user_id}")
            else:
                logger.warning("[PERMISSION CHECK] Manual authentication also failed")
        
        if not user_id:
            logger.warning("[PERMISSION CHECK] No user_id found in request after all attempts")
            return Response({
                'success': False,
        ...
        # If no RBAC record exists, try to create one with CreateContract permission
        if not rbac_record:
            logger.info(f"[PERMISSION CHECK] No RBAC record found for user {user_id}, attempting to create one")
            try:
                ...
                try:
                    ...
                    # If user doesn't exist in User model, use username from request if available
                    if hasattr(request.user, 'username'):
                        username = request.user.username
                
                # Create RBAC record with CreateContract permission
                rbac_record = RBACTPRM.objects.create(
                    user_id=user_id,
                    username=username,
                    role='Contract Manager',  # Default role
                    create_contract=True,  # Grant CreateContract permission
                    list_contracts=True,  # Also grant ListContracts for basic access
                    is_active='Y'  # Set as active
                )
                logger.info(f"[PERMISSION CHECK] Created RBAC record for user {user_id} with CreateContract permission")
            except Exception as e:
                logger.error(f"[PERMISSION CHECK] Failed to create RBAC record: {str(e)}")
                return Response({
                    'success': False,
                    'has_permission': False,
                    'error': 'No RBAC record found',
                    'message': 'Your account does not have permission settings configured. Please contact your administrator to set up permissions.'
                }, status=403)
        
        # Check if user has CreateContract permission
        has_permission = RBACTPRMUtils.check_contract_permission(user_id, 'CreateContract')
        
        if has_permission:
            logger.info(f"[PERMISSION CHECK] User {user_id} has CreateContract permission")
            return Response({
                'success': True,
                'has_permission': True,
            ...
            logger.warning(f"[PERMISSION CHECK] User {user_id} does NOT have CreateContract permission")
            # Try to grant the permission if RBAC record exists
            if rbac_record:
                try:
                    rbac_record.create_contract = True
                    rbac_record.save()
                    logger.info(f"[PERMISSION CHECK] Granted CreateContract permission to user {user_id}")
                    return Response({
                        'success': True,
                        'has_permission': True,
                        'message': 'Permission granted. You can now create contracts.'
                    })
                except Exception as e:
                    logger.error(f"[PERMISSION CHECK] Failed to grant permission: {str(e)}")
            
            return Response({
                'success': False,
                'has_permission': False,
                'error': 'Permission denied',
                'message': 'You do not have permission to create contracts. Please contact your administrator to request this permission.'
            }, status=403)
        
    except Exception as e:
        logger.error(f"[PERMISSION CHECK] Exception occurred: {str(e)}")
        logger.error(f"[PERMISSION CHECK] Exception type: {type(e).__name__}")
        import traceback
        logger.error(f"[PERMISSION CHECK] Full traceback:\n{traceback.format_exc()}")
        return Response({
            'success': False,
            'has_permission': False,
            'error': 'Failed to check permissions',
            'message': f'Unable to verify permissions. Error: {str(e)}',
            'debug': {
                'error_type': type(e).__name__,
                'error_message': str(e)
            }
...
def contract_renewals_list(request):
    ...
    try:
        ...
        renewals_list = list(renewals[start:end])
        
        # Serialize data
        try:
            serializer = ContractRenewalSerializer(renewals_list, many=True)
            serialized_data = serializer.data
        except Exception as serialization_error:
            logger.error(f"Contract renewals serialization error: {str(serialization_error)}")
            import traceback
            logger.error(f"Contract renewals serialization traceback: {traceback.format_exc()}")
...
def save_questionnaires_for_term(term_id, questionnaires_data, user):
    ...
    logger.info(f"[EMOJI] Processing {len(questionnaires_data)} questionnaires for term_id: {term_id_str}")
    ...
    try:
        ...
        if term_obj:
            ...
            logger.info(f"[EMOJI] Found term_category '{term_category}' for term_id: {term_id_str}")
        else:
            logger.warning(f"[EMOJI] Term not found in database for term_id: {term_id_str}, cannot get term_category")
    except Exception as e:
        logger.warning(f"[EMOJI] Could not fetch term_category for term_id {term_id_str}: {e}")
    ...
    if questionnaires_data and len(questionnaires_data) > 0:
        ...
        if first_q.get('template_id'):
            ...
            logger.info(f"[EMOJI] Using provided template_id: {provided_template_id} for term_id: {term_id_str}")
            logger.info(f"[EMOJI] Received {len(questionnaires_data)} questions from selected template")
    ...
    logger.info(f"[EMOJI] Prepared {len(template_questions)} questions for template (from {len(questionnaires_data)} provided)")
    ...
    if provided_template_id and template_questions:
        logger.info(f"[EMOJI] Template ID provided ({provided_template_id}) - will ONLY save the {len(template_questions)} questions provided")
        logger.info(f"[EMOJI] These questions will REPLACE all questions in the template (removed questions will be excluded)")
    ...
    if template_questions:
        ...
        if provided_template_id:
            try:
                ...
                logger.info(f"[EMOJI] Updated provided template (ID: {template_id}) for term_id: {term_id_str}")
                logger.info(f"[EMOJI] Template now has {len(template_questions)} questions (replaced all questions, removed ones are now excluded)")
                ...
                logger.info(f"[EMOJI] Deleted {deleted_count} existing contract_static_questionnaires for term_id: {term_id_str} before creating new ones")
                
            except QuestionnaireTemplate.DoesNotExist:
                logger.warning(f"[EMOJI] Provided template_id {provided_template_id} not found, creating/updating template")
        ...
        if not template_id:
            ...
            if existing_template:
                ...
                existing_template.save()
                template_id = existing_template.template_id
                logger.info(f"[EMOJI] Updated existing questionnaire_template (ID: {template_id}) for term_id: {term_id_str}")
                ...
                logger.info(f"[EMOJI] Created new questionnaire_template (ID: {template_id}) for term_id: {term_id_str}")
    ...
    for idx, q_data in enumerate(questionnaires_data):
        ...
        questions_created += 1
        logger.info(f"[EMOJI] Created question {idx + 1} in contract_static_questionnaires for term_id: {term_id_str} with template_id: {template_id}")
    
    logger.info(f"[EMOJI] Successfully saved {questions_created} questionnaires for term_id: {term_id_str} with template_id: {template_id}")
    ...
    try:
        ...
        if isinstance(questionnaires_data, str):
            try:
                ...
                logger.warning(f"[EMOJI] Unable to parse questionnaires string, defaulting to empty list. Value: {questionnaires_data}")
                questionnaires_data = []
        ...
        def create_term():
            ...
            if requested_term_id:
                logger.info(f"[EMOJI] Creating term with provided term_id: {requested_term_id}")
            else:
                logger.info(f"[EMOJI] Creating term without term_id, will be auto-generated")
            ...
            if serializer.is_valid():
                ...
                logger.info(f"[EMOJI] Contract term created: {term.term_id} for contract {contract_id}")
                # Verify term_id was preserved
                if requested_term_id and term.term_id != requested_term_id:
                    logger.warning(f"[EMOJI] Term ID changed from {requested_term_id} to {term.term_id}")
                elif requested_term_id and term.term_id == requested_term_id:
                    logger.info(f"[EMOJI] Term ID preserved: {term.term_id}")
                return term
            else:
                logger.error(f"[EMOJI] Serializer validation errors: {serializer.errors}")
                raise ValidationError(serializer.errors)
        ...
        logger.info(f"[EMOJI] Received questionnaires data for term creation: {len(questionnaires_data) if isinstance(questionnaires_data, list) else 0} items")
        if questionnaires_data and isinstance(questionnaires_data, list) and len(questionnaires_data) > 0:
            logger.info(f"[EMOJI] Sample questionnaire: {questionnaires_data[0]}")
        ...
        if original_term_id_from_request and original_term_id_from_request != saved_term_id:
            logger.info(f"[EMOJI] Term ID changed from {original_term_id_from_request} to {saved_term_id}, updating questionnaires in view...")
            try:
                from audits_contract.models import ContractStaticQuestionnaire
                ...
                if questionnaires_to_update.exists():
                    ...
                    logger.info(f"[EMOJI] Updated {updated_count} questionnaires from term_id {original_term_id_from_request} to {saved_term_id} (view-level)")
                    ...
                    for i in range(2, len(parts)):
                        ...
                        if partial_id != saved_term_id:
                            ...
                            if matching.exists():
                                ...
                                logger.info(f"[EMOJI] Updated {count} questionnaires from partial term_id {partial_id} to {saved_term_id}")
            except Exception as e:
                logger.error(f"Error updating questionnaires in view: {str(e)}")
        
        # Save questionnaires for this term if provided in request
        if questionnaires_data and isinstance(questionnaires_data, list) and len(questionnaires_data) > 0:
            logger.info(f"[EMOJI] Saving {len(questionnaires_data)} questionnaires for term_id: {saved_term_id}")
            save_questionnaires_for_term(saved_term_id, questionnaires_data, request.user)
            logger.info(f"[EMOJI] Successfully saved questionnaires for term_id: {saved_term_id}")
...
def subcontract_create(request, parent_contract_id):
    ...
    try:
        ...
        def create_subcontract():
            ...
            logger.info(f"[EMOJI] Raw subcontract_data keys: {list(subcontract_data.keys())}")
            if 'insurance_requirements' in subcontract_data:
                logger.info(f"[EMOJI] insurance_requirements raw value: {subcontract_data['insurance_requirements']}")
                logger.info(f"[EMOJI] insurance_requirements raw type: {type(subcontract_data['insurance_requirements'])}")
            ...
            logger.info(f"[EMOJI] After conversion - insurance_requirements: {subcontract_data.get('insurance_requirements')}")
            logger.info(f"[EMOJI] After conversion - data_protection_clauses: {subcontract_data.get('data_protection_clauses')}")
            
            serializer = VendorContractCreateSerializer(data=subcontract_data)
            logger.info(f"[EMOJI] Serializer is_valid() result: {serializer.is_valid()}")
            if not serializer.is_valid():
                logger.error(f"[EMOJI] Serializer errors: {serializer.errors}")
            if serializer.is_valid():
    ...
    try:
        ...
        def update_contract_and_create_subcontract():
            ...
            for i, term_data in enumerate(terms_data):
                ...
                if missing_fields:
                    logger.error(f"[EMOJI] Term {i+1} missing required fields: {missing_fields}")
                    continue
                ...
                try:
                    ...
                    logger.info(f"[EMOJI] Created term for subcontract {subcontract.contract_id}: id={created_term.term_id} title={term_data.get('term_title', 'No title')}")
                    try:
                    ...
                    logger.error(f"[EMOJI] Error creating term for subcontract: {str(e)}")
        ...
        if 'terms' in request.data and request.data['terms']:
            ...
            for term_data in request.data['terms']:
                try:
                    ...
                    created_term = ContractTerm.objects.create(**term_data)
                    logger.info(f"[EMOJI] Created term for subcontract {subcontract.contract_id}: id={created_term.term_id}")
                    try:
                        if questionnaires_data and isinstance(questionnaires_data, list):
                            logger.info(f"Saving {len(questionnaires_data)} questionnaires for subcontract term {created_term.term_id}")
                            save_questionnaires_for_term(created_term.term_id, questionnaires_data, request.user)
        ...
        if 'clauses' in request.data and request.data['clauses']:
            ...
            for clause_data in request.data['clauses']:
                try:
                    ...
                    logger.info(f"[EMOJI] Created clause for subcontract {subcontract.contract_id}: id={created_clause.clause_id}")
                except Exception as e:
                    logger.error(f"[EMOJI] Error creating clause for subcontract: {str(e)}")
