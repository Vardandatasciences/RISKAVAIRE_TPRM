        ...
        if created:
            ...
            try:
                ...
                if original_term_id and original_term_id != saved_term_id:
                    logger.info(f"[EMOJI] Term ID changed from {original_term_id} to {saved_term_id}, updating questionnaires...")
                    ...
                    if questionnaires_to_update.exists():
                        ...
                        logger.info(f"[EMOJI] Updated {updated_count} questionnaires from term_id {original_term_id} to {saved_term_id}")
                        ...
                        for variation in term_id_variations:
                            if variation != saved_term_id:
                                ...
                                if matching_questionnaires.exists():
                                    ...
                                    total_updated += updated_count
                                    logger.info(f"[EMOJI] Updated {updated_count} questionnaires from term_id variation {variation} to {saved_term_id}")
                    
                    if total_updated > 0:
                        logger.info(f"[EMOJI] Total questionnaires updated: {total_updated}")
                    else:
                        logger.warning(f"[EMOJI] No questionnaires found with original term_id {original_term_id} or its variations")
                        ...
                        if orphaned_questionnaires.exists():
                            logger.info(f"[EMOJI] Checking {orphaned_questionnaires.count()} orphaned questionnaires for potential matches...")
                            for orphaned_q in orphaned_questionnaires[:10]:  # Limit to first 10 to avoid performance issues
                                ...
                                if (original_term_id in orphaned_term_id or 
                                    ...
                                    logger.info(f"[EMOJI] Potential match found: orphaned questionnaire {orphaned_q.question_id} with term_id {orphaned_term_id}")
                                    orphaned_q.term_id = saved_term_id
                                    orphaned_q.save()
                                    total_updated += 1
                                    logger.info(f"[EMOJI] Updated orphaned questionnaire {orphaned_q.question_id} from {orphaned_term_id} to {saved_term_id}")
                            
                            if total_updated > 0:
                                logger.info(f"[EMOJI] Total questionnaires updated (including orphaned): {total_updated}")
                else:
                    ...
                    if matching_questionnaires.exists():
                        logger.info(f"[EMOJI] Found {matching_questionnaires.count()} questionnaires matching term_id {saved_term_id}")
                    else:
                        ...
                        if orphaned_questionnaires.exists():
                            logger.info(f"ℹ[EMOJI] Found {orphaned_questionnaires.count()} orphaned questionnaires (no matching term)")
                                ...
                                if '_' in saved_term_id and '_' in orphaned_term_id:
                                    ...
                                    if len(saved_parts) >= 2 and len(orphaned_parts) >= 2:
                                        if saved_parts[:2] == orphaned_parts[:2]:
                                            logger.info(f"[EMOJI] Potential match: updating orphaned questionnaire {orphaned_q.question_id} from {orphaned_term_id} to {saved_term_id}")
                                            orphaned_q.term_id = saved_term_id
                                            orphaned_q.save()
                                            logger.info(f"[EMOJI] Updated orphaned questionnaire {orphaned_q.question_id}")
