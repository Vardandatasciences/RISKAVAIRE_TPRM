...
class RenderS3Client:
    ...
    def _setup_mysql_database(self, mysql_config: Dict):
        ...
        try:
            ...
            print("[OK] MySQL connection pool initialized successfully")
    ...
    def _create_table_if_not_exists(self):
        ...
        try:
            ...
            conn.commit()
            print("[OK] Database table verified/created successfully")
    ...
    def test_connection(self) -> Dict:
        ...
        try:
            ...
            print("[OK] Direct microservice: Connected")
        ...
        try:
            ...
            if self.db_pool:
                ...
                print("[EMOJI]  MySQL database: Not configured")
    ...
    def upload(self, file_path: str, user_id: str = "default-user", 
        ...
        try:
            ...
            print(f"[EMOJI] Uploading {file_name} ({file_size} bytes) via Direct...")
            ...
            with open(file_path, 'rb') as file:
                ...
                print(f"[EMOJI] File details: name={file_name}, size={file_size}, type={mimetypes.guess_type(file_path)[0]}")
                
                try:
                    response = requests.post(url, files=files, timeout=300)
                    print(f"[EMOJI] Response status: {response.status_code}")
                    ...
                    if not isinstance(result, dict):
                        ...
                        print(f"[ERROR] Response value: {result}")
            ...
            if result.get('success'):
                ...
                print(f"[OK] Upload successful! File: {file_info['storedName']}")
    ...
    def download(self, s3_key: str, file_name: str, 
        ...
        try:
            ...
            print(f"[OK] Download successful! Saved to: {local_file_path}")
    ...
    def export(self, data: Union[List[Dict], Dict], export_format: str, 
        ...
        try:
            ...
            print(f"[EMOJI] Exporting {record_count} records as {export_format.upper()} via Direct...")
            ...
            print(f"[EMOJI] Export URL: {url}")
            print(f"[EMOJI] Payload size: {len(str(payload))} characters")
            print(f"[EMOJI] Using AWS credentials: {aws_credentials['awsAccessKey'][:10]}...")
            ...
            if result.get('success'):
                ...
                print(f"[OK] Export successful! File: {export_info['storedName']}")
...
def create_direct_mysql_client(mysql_config: Optional[Dict] = None) -> RenderS3Client:
    ...
    try:
        ...
        print(f"[EMOJI] Creating S3 client with MySQL config: {mysql_config['host']}:{mysql_config['port']}/{mysql_config['database']}")
        ...
        return client
            ...
            try:
                ...
                print("[EMOJI]  S3 client created without MySQL (fallback mode)")
                return client
...
def quick_test():
    """Quick test function"""
    print("[EMOJI] Quick Test: Direct S3 Client with Local MySQL")
    print("=" * 60)
...
def test_all_export_formats():
    ...
    print("[EMOJI] Testing All Export Formats")
    print("[EMOJI] Direct URL: http://15.207.1.40:3000")
    print("[EMOJI] Testing: JSON, CSV, XML, TXT, PDF")
    print("=" * 60)
    ...
    print(f"[EMOJI] Data: {len(sample_data)} employee records")
    ...
    for i, export_format in enumerate(export_formats, 1):
        ...
        try:
            ...
            print(f"[EMOJI] Exporting as {export_format.upper()}...")
            export_result = client.export(sample_data, export_format, file_name, user_id)
            
            if export_result['success']:
                print(f"[OK] {export_format.upper()} Export: SUCCESS")
                print(f"   [EMOJI] File: {export_result['export_info']['storedName']}")
                print(f"   [SAVE] Size: {export_result['export_info']['size']} bytes")
                print(f"   [EMOJI] URL: {export_result['export_info']['url']}")
    ...
    # Summary report
    print("\n" + "=" * 60)
    print("[EMOJI] EXPORT TEST SUMMARY")
    print("=" * 60)
    ...
    if failed_formats:
        print(f"   [EMOJI]  Failed formats: {', '.join(failed_formats)}")
    
    # Show operation history
    print(f"\n[EMOJI] Recent operation history:")
    history = client.get_operation_history('export_test_user', 10)
    
    if history:
        for i, op in enumerate(history, 1):
            status_emoji = "[OK]" if op['status'] == 'completed' else "[ERROR]" if op['status'] == 'failed' else "[PENDING]"
...
def main():
    ...
    print("[EMOJI] Direct S3 Microservice Client with Local MySQL")
    print("[EMOJI] Direct URL: http://15.207.1.40:3000")
    print("[EMOJI]  Database: Local MySQL")
    print("[EMOJI] AWS Credentials: Handled by microservice")
    print("=" * 60)
