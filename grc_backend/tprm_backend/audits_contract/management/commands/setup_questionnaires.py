        ...
        try:
            with connections['default'].cursor() as cursor:
                ...
                if not table_exists:
                    ...
                    self.stdout.write('[EMOJI] Table created successfully')
                ...
                self.stdout.write(f'[EMOJI] Inserted {len(sample_data)} questionnaires')
                
                # Verify insertion
                cursor.execute("SELECT COUNT(*) FROM contract_static_questionnaires")
                count = cursor.fetchone()[0]
                self.stdout.write(f'[EMOJI] Total questionnaires in database: {count}')
