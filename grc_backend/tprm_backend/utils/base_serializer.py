"""
Auto-Decrypting Base Serializer for TPRM Models

This serializer automatically decrypts encrypted fields when serializing model data.
All TPRM serializers should inherit from AutoDecryptingModelSerializer instead of
...
class AutoDecryptingSerializer(serializers.Serializer):
    """
    Base Serializer (non-Model) that can handle decryption for explicit fields.
    
    Use this for serializers that don't inherit from ModelSerializer.
    You'll need to manually specify fields, but decryption will be automatic.
    
    Example:
        class MySerializer(AutoDecryptingSerializer):
    ...
    def to_representation(self, instance):
        """
        Override to_representation to automatically decrypt configured fields.
        """
        ret = super().to_representation(instance)
        
        try:
            # Check if Meta class specifies encrypted_fields
            if hasattr(self.Meta, 'encrypted_fields'):
                encrypted_fields = self.Meta.encrypted_fields
            ...
            logger.debug(f"No encrypted_fields defined or error: {str(e)}")
