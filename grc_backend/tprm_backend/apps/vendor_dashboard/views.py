...
class DashboardMetricsAPIView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            error_message = str(e)
            if 'permission' in error_message.lower() or 'authenticated' in error_message.lower():
                return Response(
                    {
                        'error': 'Authentication or permission error',
                        'message': 'You do not have permission to access this resource. Please ensure you are logged in with proper credentials.',
                        'details': error_message
                    },
                    status=status.HTTP_403_FORBIDDEN
                )
            
            # Check if it's a database/model error
            if 'does not exist' in error_message.lower() or 'no such table' in error_message.lower():
                return Response(
                    {
                        'error': 'Database error',
                        'message': 'Required database tables or records not found. Please contact support.',
                        'details': error_message
                    },
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
    ...
    def get(self, request):
        ...
        try:
            with connections['default'].cursor() as cursor:
                ...
                # 1. Initial Validation KPIs
                cursor.execute("SELECT COUNT(*) FROM temp_vendor WHERE status IN ('APPROVED', 'PENDING_REVIEW')")
                total_validated = cursor.fetchone()[0] or 0
                
                cursor.execute("SELECT COUNT(*) FROM temp_vendor WHERE status = 'APPROVED'")
                approved_vendors = cursor.fetchone()[0] or 0
                ...
                # 2. Due Diligence KPIs
                cursor.execute("SELECT COUNT(*) FROM questionnaire_assignments")
                total_questionnaires = cursor.fetchone()[0] or 0
                
                cursor.execute("SELECT COUNT(*) FROM questionnaire_assignments WHERE status = 'RESPONDED'")
                completed_questionnaires = cursor.fetchone()[0] or 0
                ...
                # 3. Risk Scoring KPIs
                cursor.execute("SELECT COUNT(*) FROM temp_vendor WHERE risk_level IS NOT NULL AND risk_level != ''")
                total_risk_assessed = cursor.fetchone()[0] or 0
                
                cursor.execute("SELECT COUNT(*) FROM temp_vendor WHERE risk_level IN ('LOW', 'MEDIUM')")
                low_medium_risk = cursor.fetchone()[0] or 0
                ...
                # 4. External Verification KPIs
                cursor.execute("""
                    SELECT COUNT(DISTINCT v.id) FROM temp_vendor v
                    JOIN external_screening_results esr ON v.id = esr.vendor_id
                """)
                total_screened = cursor.fetchone()[0] or 0
                
                cursor.execute("""
                    SELECT COUNT(DISTINCT v.id) FROM temp_vendor v
                    JOIN external_screening_results esr ON v.id = esr.vendor_id
                    JOIN screening_matches sm ON esr.screening_id = sm.screening_id
                    WHERE sm.is_false_positive = 0 AND sm.resolution_status IN ('PENDING', 'ESCALATED', 'BLOCKED')
                """)
                flagged_vendors = cursor.fetchone()[0] or 0
