...
def create_approval_version(approval_id, version_type, version_label, json_payload, changes_summary, 
    ...
    try:
        ...
        with connections['default'].cursor() as cursor:
            ...
            print(f"[EMOJI] Version created and committed: {version_id} (v{next_version_number})")
    ...
    try:
        ...
        with connections['default'].cursor() as cursor:
            ...
            cursor.execute(
                ...
                SELECT DISTINCT assigned_user_id AS id, assigned_user_name AS name

                FROM approval_stages

                WHERE assigned_user_id IS NOT NULL

                  AND CAST(assigned_user_id AS CHAR) <> ''
                            ...
                            if current_request_data:
                                ...
                                if vendor_id:
                                    ...
                                    if approval_type == 'questionnaire_approval':
                                        ...
                                        try:
                                            ...
                                            if ques_approval_stage_id and ques_response_stage_id:
                                                ...
                                                if approval_entry:
                                                    ...
                                                    approval_entry.save()
                                                    print(f"[EMOJI] Ended Questionnaire Approval stage for vendor {vendor_id}")
                                                ...
                                                print(f"[EMOJI] Started Questionnaire Response stage for vendor {vendor_id}")
                                        except Exception as e:
                                        ...
                                        print(f"[EMOJI] Response Approval lifecycle stage completed for vendor {vendor_id}")
                                        ...
                                        print(f"[EMOJI] Vendor Approval lifecycle stage completed for vendor {vendor_id}")
                                    else:
                    ...
                    if approval_type == 'response_approval':
                        ...
                        if questionnaire_assignment_id:
                            ...
                            try:
                                ...
                                print(f"[EMOJI] Scores successfully written to questionnaire_response_submissions for assignment {questionnaire_assignment_id}")
                        ...
                        if vendor_id:
                            ...
                            if vendor_id:
                                if approval_type == 'questionnaire_approval':
                                    ...
                                    print(f"[EMOJI] Lifecycle Stage 3 (Questionnaire Approval) completed for vendor {vendor_id}")
                                elif approval_type == 'response_approval':
                                    # End Response Approval and start Vendor Approval
...
def admin_handle_rejection(request, approval_id):
    ...
    try:
        ...
        if hasattr(request, 'user') and request.user.is_authenticated:
            ...
            # Default to GRC Administrator (ID: 60) when no authentication
            admin_id = 60
            admin_name = 'GRC Administrator'
        
        print(f"[EMOJI] Admin user info: ID={admin_id}, Name={admin_name}")
        ...
        with connections['default'].cursor() as cursor:
            ...
            if action == 'RESTART_FROM_REJECTED':
                ...
                print(f"\n{'='*80}")
                print(f"[EMOJI] CREATING VERSION for Admin Action: RESTART_FROM_REJECTED")
                print(f"{'='*80}")
                print(f"[EMOJI] Approval ID: {approval_id}")
                print(f"[EMOJI] Stage: {rejected_stage_name}")
                print(f"[EMOJI] Admin: {admin_name} (ID: {admin_id})")
                print(f"[EMOJI] Calling create_approval_version()...")
                ...
                if version_id:
                    print(f"[EMOJI] SUCCESS! Version created: {version_id}")
                    print(f"[EMOJI] Admin action 'RESTART_FROM_REJECTED' recorded in version table")
                else:
                    print(f"[EMOJI] FAILURE - create_approval_version returned None")
                    print(f"[EMOJI] Admin action was NOT recorded in version table!")
                print(f"{'='*80}\n")
                ...
                # Create version record
                print(f"\n{'='*80}")
                print(f"[EMOJI] CREATING VERSION for Admin Action: RESTART_FROM_STAGE")
                print(f"{'='*80}")
                print(f"[EMOJI] Approval ID: {approval_id}")
                print(f"[EMOJI] Stage: {target_stage_name} (Order: {target_stage_order})")
                print(f"[EMOJI] Admin: {admin_name} (ID: {admin_id})")
                print(f"[EMOJI] Calling create_approval_version()...")
                ...
                if version_id:
                    print(f"[EMOJI] SUCCESS! Version created: {version_id}")
                    print(f"[EMOJI] Admin action 'RESTART_FROM_STAGE' recorded in version table")
                else:
                    print(f"[EMOJI] FAILURE - create_approval_version returned None")
                    print(f"[EMOJI] Admin action was NOT recorded in version table!")
                print(f"{'='*80}\n")
                ...
                print(f"\n{'='*80}")
                print(f"[EMOJI] CREATING VERSION for Admin Action: FINAL_REJECT")
                print(f"{'='*80}")
                print(f"[EMOJI] Approval ID: {approval_id}")
                print(f"[EMOJI] Admin: {admin_name} (ID: {admin_id})")
                print(f"[EMOJI] Comments: {admin_comments}")
                print(f"[EMOJI] Calling create_approval_version()...")
                ...
                if version_id:
                    print(f"[EMOJI] SUCCESS! Version created: {version_id}")
                    print(f"[EMOJI] Admin action 'FINAL_REJECT' recorded in version table")
                else:
                    print(f"[EMOJI] FAILURE - create_approval_version returned None")
                    print(f"[EMOJI] Admin action was NOT recorded in version table!")
                print(f"{'='*80}\n")
        ...
        try:
            ...
            with connections['default'].cursor() as cursor:
                ...
                """, [
                    ...
                    int(data.get('created_by', 60)) if str(data.get('created_by', '60')).isdigit() else 60,
        ...
        try:
            ...
            with connections['default'].cursor() as cursor:
                ...
                """, [
                    ...
                    int(request_data.get('requester_id', 60)) if str(request_data.get('requester_id', '60')).isdigit() else 60,
                        ...
                        try:
                            ...
                            if isinstance(request_data, dict):
                                ...
                                if business_object_id and isinstance(business_object_id, (int, str)) and str(business_object_id).strip():
                                    ...
                                    print(f"Debug - [EMOJI] Found vendor_id in request_data.business_object_id: {vendor_id}")
                            ...
                            if not vendor_id and isinstance(request_data, dict):
                                ...
                                if direct_vendor_id and str(direct_vendor_id).strip():
                                    ...
                                    print(f"Debug - [EMOJI] Found vendor_id in request_data.vendor_id: {vendor_id}")
                            ...
                            if not vendor_id and isinstance(request_data, dict):
                                ...
                                if nested_request_data and isinstance(nested_request_data, dict):
                                    ...
                                    if nested_vendor_id and str(nested_vendor_id).strip():
                                        ...
                                        print(f"Debug - [EMOJI] Found vendor_id in nested request_data: {vendor_id}")
                                    ...
                                    if not vendor_id:
                                        ...
                                        if nested_business_object_id and str(nested_business_object_id).strip():
                                            ...
                                            print(f"Debug - [EMOJI] Found vendor_id in nested business_object_id: {vendor_id}")
                            ...
                            if not vendor_id and hasattr(request, 'data'):
                                ...
                                if main_business_object_id and str(main_business_object_id).strip():
                                    ...
                                    print(f"Debug - [EMOJI] Found vendor_id in request.data.request.business_object_id: {vendor_id}")
                            ...
                            if not vendor_id and hasattr(request, 'data'):
                                ...
                                if isinstance(request_body, dict):
                                    ...
                                    if main_vendor_id and str(main_vendor_id).strip():
                                        ...
                                        print(f"Debug - [EMOJI] Found vendor_id in main request body: {vendor_id}")
                                    ...
                                    if not vendor_id:
                                        ...
                                        if main_business_object_id and str(main_business_object_id).strip():
                                            vendor_id = main_business_object_id
                                            print(f"Debug - [EMOJI] Found vendor_id in main request body business_object_id: {vendor_id}")
                            ...
                            if not vendor_id:
                                ...
                                if request_vendor_id and str(request_vendor_id).strip():
                                    ...
                                    print(f"Debug - [EMOJI] Found vendor_id in request URL parameters: {vendor_id}")
                            ...
                            if not vendor_id:
                                ...
                                if questionnaire_id:
                                    ...
                                    try:
                                        ...
                                        if questionnaire and questionnaire.vendor_id:
                                            vendor_id = questionnaire.vendor_id
                                            print(f"[EMOJI] Successfully extracted vendor_id {vendor_id} from questionnaire_id {questionnaire_id}")
                                        else:
                                    ...
                                    if ques_app_stage:
                                        ...
                                        with transaction.atomic():
                                            ...
                                            if lifecycle_stage_id:
                                                print(f"[EMOJI] Successfully ensured Lifecycle Stage 3 (Questionnaire Approval) for vendor {vendor_id} with lifecycle_stage_id {lifecycle_stage_id}")
                                            else:
                    ...
                    if vendor_id and request_data:
                        ...
                        if isinstance(rd, dict):
                            ...
                            if approval_type == 'questionnaire_approval' and workflow_data.get('workflow_type') == 'MULTI_PERSON':
                                print(f"[EMOJI] Detected MULTI_PERSON questionnaire_approval workflow for vendor {vendor_id}")
                                print(f"[EMOJI] Starting Lifecycle Stage 3 (Questionnaire Approval) for vendor {vendor_id}")
                                    ...
                                    if ques_approval_stage_id:
                                        ...
                                        if not existing_entry:
                                            ...
                                            try:
                                                ...
                                                temp_vendor.save()
                                                print(f"[EMOJI] Successfully started Lifecycle Stage 3 (Questionnaire Approval) for vendor {vendor_id}")
                                            except TempVendor.DoesNotExist:
                                                print(f"Warning: TempVendor with ID {vendor_id} not found")
                                        else:
                                            print(f"[EMOJI] Lifecycle Stage 3 (Questionnaire Approval) already active for vendor {vendor_id}")
                                    else:
...
def dashboard_stats(request):
    ...
    try:
        ...
        total_requests = ApprovalRequests.objects.count()

        pending_requests = ApprovalRequests.objects.filter(overall_status='PENDING').count()

        in_progress_requests = ApprovalRequests.objects.filter(overall_status='IN_PROGRESS').count()

        approved_requests = ApprovalRequests.objects.filter(overall_status='APPROVED').count()

        rejected_requests = ApprovalRequests.objects.filter(overall_status='REJECTED').count()
        ...
        recent_activity = ApprovalRequests.objects.filter(
...
def save_reviewer_scores(request):
    ...
    try:
        ...
        reviewer_id = data.get('reviewer_id', 60)
...
def load_stage_draft(request, stage_id):
    ...
    try:
        ...
        user_id = request.query_params.get('user_id', 60)
...
def save_final_assignee_scores(request):
    ...
    try:
        ...
        assignee_id = data.get('assignee_id', 60)
        ...
        with transaction.atomic():
            ...
            if assignee_decision == 'APPROVE' and assignment.temp_vendor:
                try:
                    ...
                    _end_response_approval_start_vendor_approval(vendor_id)
                    print(f"[EMOJI] Response Approval lifecycle stage completed for vendor {vendor_id} after assignee approval")
                except Exception as e:
            ...
            try:
                ...
                with connections['default'].cursor() as cursor:
                    ...
                    if not result:
                        ...
                        print(f"[EMOJI] ERROR - No approval found for assignment {assignment_id} using JSON_EXTRACT")

                        print(f"[EMOJI] Trying alternative query method (LIKE)...")
                        ...
                        if not result:
                            ...
                            print(f"[EMOJI] LIKE query failed. Trying third method via questionnaire_responses...")
                            ...
                            if not result:
                                print(f"[EMOJI] CRITICAL - All 3 methods failed. No approval found for assignment {assignment_id}.")
                            else:
                                print(f"[EMOJI] Found approval via questionnaire_responses join!")
                    ...
                    if result:
                        ...
                        print(f"[EMOJI] Found approval_id: {approval_id} for assignment {assignment_id}")
                    ...
                    if approval_id_for_version and assignee_decision in ['APPROVE', 'REJECT']:
                        print(f"\n{'='*80}")
                        print(f"[EMOJI] CREATING VERSION for Assignee Decision: {assignee_decision}")
                        print(f"{'='*80}")
                        try:
                            ...
                            try:
                                with connections['default'].cursor() as user_cursor:
                                    ...
                                    if user_row:
                                        ...
                                        print(f"[EMOJI] Got assignee name from database: {assignee_name}")
                            except Exception as e:
                                print(f"[EMOJI]  Could not fetch assignee name: {str(e)}")
                            ...
                            if version_id:
                                print(f"\n[EMOJI] SUCCESS! Version created: {version_id}")
                                print(f"[EMOJI] Assignee decision '{assignee_decision}' recorded in version table")
                                print(f"[EMOJI] Version number has been incremented for approval_id: {approval_id_for_version}")
                            else:
                                print(f"\n[EMOJI] FAILURE - create_approval_version returned None")
                                print(f"[EMOJI] Version was NOT created for assignee decision!")
                            
                            print(f"{'='*80}\n")
                                
                        except Exception as version_error:
                            print(f"\n[EMOJI] EXCEPTION creating version for assignee decision [EMOJI]")
                            print(f"Error: {str(version_error)}")
                            import traceback
                            traceback.print_exc()
                            print(f"[EMOJI] Version creation FAILED but operation will continue")
                            print(f"{'='*80}\n")
                        ...
                        print(f"\n[EMOJI] CANNOT CREATE VERSION - approval_id not found for assignment {assignment_id} [EMOJI]")
                        print(f"[EMOJI] Assignee decision '{assignee_decision}' will NOT be recorded in version table!")
                        print(f"[EMOJI] This is a critical issue - check approval_requests table for this assignment")
                    else:
                        print(f"ℹ[EMOJI]  No version created - Decision is '{assignee_decision}' (only APPROVE/REJECT create versions)")
...
def _end_response_approval_start_vendor_approval(vendor_id):
    ...
    try:
        ...
        if response_approval_entry:
            ...
            print(f"[EMOJI] Successfully ended Response Approval stage for vendor {vendor_id} at {current_time}")
        else:
        ...
        print(f"[EMOJI] Successfully created Vendor Approval stage entry (ID: {new_entry.id}) for vendor {vendor_id}")
        ...
        try:
            ...
            temp_vendor.save()
            print(f"[EMOJI] Successfully updated temp vendor {vendor_id} lifecycle stage from {old_stage} to {vendor_approval_stage_id}")
        except TempVendor.DoesNotExist:
            print(f"ERROR - Temp vendor {vendor_id} not found")
        
        print(f"[EMOJI] COMPLETED - Successfully transitioned vendor {vendor_id} from Response Approval to Vendor Approval stage")
    ...
    try:
        ...
        if existing_entry:
            ...
            if existing_entry.ended_at:
                ...
                existing_entry.save()
                print(f"[EMOJI] Reactivated {stage_code} stage for vendor {vendor_id}")
        else:
            ...
            print(f"[EMOJI] Started {stage_code} stage for vendor {vendor_id}")
    ...
    try:
        ...
        if not questionnaire_response_entry:
            ...
            if questionnaire_response_entry.ended_at is not None:
                ...
                print(f"[EMOJI] Reactivated Questionnaire Response stage for vendor {vendor_id}")
            else:
                print(f"[EMOJI] Questionnaire Response stage already active for vendor {vendor_id}")
        ...
        try:
            ...
            temp_vendor.save()
            print(f"[EMOJI] Updated temp vendor lifecycle stage to Questionnaire Response for vendor {vendor_id}")
        except TempVendor.DoesNotExist:
            print(f"WARNING - TempVendor with ID {vendor_id} not found")
