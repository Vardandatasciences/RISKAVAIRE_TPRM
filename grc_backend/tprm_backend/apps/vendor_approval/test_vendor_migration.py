...
def test_vendor_migration():
    ...
    try:
        ...
        if result['success']:
            print("[EMOJI] Migration successful!")
            ...
            with connection.cursor() as cursor:
                ...
                if vendor_data:
                    print("[EMOJI] Vendor data verified in main vendors table")
                else:
                    print("[EMOJI] Vendor data not found in main vendors table")
                
                # Check vendor_contacts table
                cursor.execute("SELECT COUNT(*) FROM vendor_contacts WHERE vendor_id = %s", [result['vendor_id']])
                contact_count = cursor.fetchone()[0]
                if contact_count > 0:
                    print(f"[EMOJI] {contact_count} contacts verified in vendor_contacts table")
                else:
                    print("[EMOJI] No contacts found in vendor_contacts table")
                
                # Check vendor_documents table
                cursor.execute("SELECT COUNT(*) FROM vendor_documents WHERE vendor_id = %s", [result['vendor_id']])
                doc_count = cursor.fetchone()[0]
                if doc_count > 0:
                    print(f"[EMOJI] {doc_count} documents verified in vendor_documents table")
                else:
                    print("[EMOJI] No documents found in vendor_documents table")
            
            return True
        else:
            print(f"[EMOJI] Migration failed: {result['error']}")
            return False
            
    except Exception as e:
        print(f"[EMOJI] Test failed with error: {str(e)}")
        import traceback
        traceback.print_exc()
        return False
...
def cleanup_test_data():
    ...
    try:
        with connection.cursor() as cursor:
            ...
            print("[EMOJI] Test data cleaned up successfully")
    except Exception as e:
