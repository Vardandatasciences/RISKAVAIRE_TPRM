...
try:
    ...
    print(f"[EMOJI] Found user: {user.username} (ID: {user.userid})")
    
    # Get RBAC record
    try:
        rbac = RBACTPRM.objects.get(user_id=user.userid, is_active='Y')
        print(f"[EMOJI] Found active RBAC record")
        ...
        for field_name, display_name in permissions_to_check.items():
            ...
            status = "[EMOJI] GRANTED" if has_permission else "[EMOJI] DENIED"
            color = "" if has_permission else "[EMOJI] "
        ...
        print("\n" + "-"*60)
        if all_granted:
            print("[EMOJI] SUCCESS: All contract permissions are granted!")
            ...
            print(f"[EMOJI]  WARNING: {len(missing_permissions)} permissions are DENIED")
            print("\nMissing permissions:")
        ...
        print(f"[EMOJI] No RBAC record found for user_id {user.userid}")
...
INSERT INTO rbac_tprm (UserId, UserName, Role, 
    ...
    print(f"[EMOJI] User 'testuser1' not found in database")
