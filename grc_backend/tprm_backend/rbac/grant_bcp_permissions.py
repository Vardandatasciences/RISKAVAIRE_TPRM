...
def grant_all_bcp_drp_permissions(username='testuser1'):
    ...
    try:
        ...
        try:
            ...
            print(f"[EMOJI] User '{username}' not found in User table")
            return False
        ...
        if created:
            print(f"[EMOJI] Created new RBAC record for {username}")
        else:
            print(f"[EMOJI] Found existing RBAC record for {username} (Role: {rbac_record.role})")
        ...
        print(f"\n[EMOJI] SUCCESS! Granted {granted_count} BCP/DRP permissions to {username}")
        print(f"[EMOJI] RBAC Record ID: {rbac_record.rbac_id}")
        print(f"[EMOJI] User ID: {rbac_record.user_id}")
        print(f"[EMOJI] Role: {rbac_record.role}")
        print(f"[EMOJI] Active: {rbac_record.is_active}")
        
        # Verify key permissions
        print("\n[EMOJI] Key BCP/DRP Permissions Verified:")
        key_permissions = [
            'create_bcp_drp_strategy_and_plans',
            'view_plans_and_documents',
            'assign_plans_for_evaluation',
        ...
        for perm in key_permissions:
            ...
            status = "[EMOJI]" if value else "[EMOJI]"
...
if __name__ == '__main__':
    ...
    if success:
        print(f"\n{'='*60}")
        print(f"  [EMOJI] ALL PERMISSIONS GRANTED SUCCESSFULLY!")
        print(f"{'='*60}\n")
    else:
        print(f"\n{'='*60}")
        print(f"  [EMOJI] FAILED TO GRANT PERMISSIONS")
        print(f"{'='*60}\n")
        sys.exit(1)
