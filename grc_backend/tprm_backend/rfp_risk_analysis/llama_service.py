...
class LlamaService:
    ...
    def __init__(self):
        try:
            ...
            self.model_name = getattr(settings, 'LLAMA_MODEL_NAME', 'llama3.2:3b')
...
Generate specific, actionable risks based on the actual record data:"""
    ...
    def _build_rfp_row_prompt(self, table_name: str, row_data: dict, today: str) -> str:
        ...
        if 'responses' in table_name.lower():
            return self._build_rfp_response_prompt(row_data, today)
        elif 'rfps' in table_name.lower():
            return self._build_rfp_request_prompt(row_data, today)
        else:
            # Default RFP prompt for other tables
            return self._build_rfp_generic_prompt(table_name, row_data, today)
    
    def _build_rfp_request_prompt(self, row_data: dict, today: str) -> str:
        """Build specific prompt for RFP requests"""
        prompt = f"""Analyze this RFP (Request for Proposal) record and identify 3-5 specific risks. Today is {today}.

Apply RFP Request heuristics:
- Timeline risks: Submission deadline < 30 days from creation = High pressure risk
- Budget risks: Unclear or missing budget estimates = Planning risk
- Scope risks: Vague descriptions or missing requirements = Ambiguity risk
- Evaluation risks: Poorly defined criteria = Unfair evaluation risk
- Criticality risks: High criticality without proper oversight = Control risk
- Approval risks: Missing or delayed approvals = Process risk

RFP Request Record Data:
{json.dumps(row_data, indent=2)}

Focus on these RFP request-specific risk areas:
1. Requirements clarity and completeness
2. Timeline feasibility and pressure points
3. Budget estimation accuracy
4. Evaluation criteria fairness and clarity
5. Approval and governance controls
6. Vendor selection process integrity

Format each risk EXACTLY like this:

RISK 1:
TITLE: [Specific RFP request-related risk title]
DESCRIPTION: The RFP analysis reveals specific control gaps. Owner: RFP Manager/Procurement Lead. Evidence needed: Review RFP documentation and approval workflow. Review due: 2025-12-17.
LIKELIHOOD: [1-5]
IMPACT: [1-5]
EXPLANATION: RFP analysis shows [specific findings from the RFP data such as tight deadlines, unclear requirements, missing approvals].
MITIGATIONS:
- [RFP-specific mitigation like "Extend submission deadline"]
- [Process mitigation like "Clarify evaluation criteria"]
- [Governance mitigation like "Complete approval workflow"]

Generate specific, actionable risks based on the actual RFP data:"""
        
        return prompt
    
    def _build_rfp_response_prompt(self, row_data: dict, today: str) -> str:
        """Build specific prompt for RFP responses/vendor proposals"""
        prompt = f"""Analyze this RFP Response (Vendor Proposal) record and identify 3-5 specific risks. Today is {today}.

Apply RFP Response heuristics:
- Vendor qualification: Unknown or unverified vendors = Credibility risk
- Pricing risks: Proposed value significantly below/above estimate = Viability/Value risk
- Compliance risks: Missing required documents or certifications = Non-compliance risk
- Technical risks: Insufficient technical details or capability proof = Delivery risk
- Commercial risks: Unclear payment terms or warranties = Contract risk
- Timeline risks: Unrealistic delivery schedules = Execution risk
- Evaluation risks: Low scores in critical areas = Performance risk

RFP Response Record Data:
{json.dumps(row_data, indent=2)}

Focus on these vendor response-specific risk areas:
1. Vendor credibility and track record
2. Pricing reasonableness and competitiveness
3. Technical capability and solution quality
4. Compliance with RFP requirements
5. Commercial terms and conditions
6. Delivery timeline feasibility
7. Post-award performance risks

Format each risk EXACTLY like this:

RISK 1:
TITLE: [Specific vendor response-related risk title]
DESCRIPTION: The vendor proposal analysis reveals specific concerns. Owner: Evaluation Committee/Procurement Manager. Evidence needed: Verify vendor credentials and references. Review due: 2025-12-17.
LIKELIHOOD: [1-5]
IMPACT: [1-5]
EXPLANATION: Vendor response analysis shows [specific findings from the response data such as pricing anomalies, missing documentation, capability gaps].
MITIGATIONS:
- [Vendor-specific mitigation like "Request additional references"]
- [Technical mitigation like "Conduct capability assessment"]
- [Commercial mitigation like "Negotiate contract terms"]

Generate specific, actionable risks based on the actual vendor response data:"""
        
        return prompt
    
    def _build_rfp_generic_prompt(self, table_name: str, row_data: dict, today: str) -> str:
        """Build generic RFP prompt for other table types"""
        prompt = f"""Analyze this RFP {table_name} record and identify 2-3 specific risks. Today is {today}.

Apply general RFP heuristics:
- Process compliance: Non-standard procedures = Compliance risk
- Timeline adherence: Overdue items = Operational risk
- Documentation: Missing critical information = Information risk
- Stakeholder engagement: Poor communication = Coordination risk
...
Generate specific risks based on the actual data provided:"""
    ...
    def _create_risk_from_block_simple(self, block: str, entity: str, table_name: str, row_data: dict, risk_number: int) -> Risk:
        ...
        try:
            ...
            # Extract the appropriate row identifier based on entity/table
            row_id = 'unknown'
            if entity == 'RFP':
                if 'responses' in table_name.lower():
                    row_id = str(row_data.get('response_id', row_data.get('id', 'unknown')))
                elif 'rfps' in table_name.lower():
                    row_id = str(row_data.get('rfp_id', row_data.get('id', 'unknown')))
            else:
                row_id = str(row_data.get('id') or row_data.get('plan_id') or row_data.get('evaluation_id') or 'unknown')
