...
urlpatterns = [
    ...
    # Unmatched vendors endpoints - MOVED BEFORE ROUTER (CRITICAL for 404 fix)
    # Using re_path with explicit regex to ensure these match before router
    re_path(r'rfps/(?P<rfp_id>\d+)/unmatched-vendors/$', views.get_unmatched_vendors, name='get_unmatched_vendors'),
    re_path(r'rfps/(?P<rfp_id>\d+)/unmatched-vendors/create/$', views.create_unmatched_vendor, name='create_unmatched_vendor'),
    re_path(r'rfps/(?P<rfp_id>\d+)/unmatched-vendors/bulk-upload/$', views.unmatched_vendor_bulk_upload, name='unmatched_vendor_bulk_upload'),
   
    # Approved vendors endpoint - MOVED BEFORE ROUTER
    path('rfps/<int:rfp_id>/approved-vendors/', views.get_approved_vendors, name='get_approved_vendors'),
   
    # RFP document management endpoints - MOVED BEFORE ROUTER
    ...
    # RFP Versioning endpoints - MOVED BEFORE ROUTER
    ...
    # Vendor endpoints (non-rfps specific) - can stay after router
...
# Standalone merge endpoint (works without RFP ID)
    ...
    # RFP Versioning endpoints (non-rfps specific can stay after router)
