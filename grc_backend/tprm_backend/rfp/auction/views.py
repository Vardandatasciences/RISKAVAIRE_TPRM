...
class AuctionViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            ...
            # If Auction doesn't exist, create it instead (upsert behavior)
            # This handles the case where frontend has stale ID in localStorage
            import logging
            logger = logging.getLogger(__name__)
            logger.warning(f"[Auction Update] Auction with ID {pk} not found. Creating new Auction instead (upsert).")
        ...
        try:
            ...
            import traceback
            print(f"Exception during Auction update: {str(e)}")
            print(traceback.format_exc())
            return Response({"error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
class AuctionEvaluationCriteriaViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            import logging
            logger = logging.getLogger(__name__)
            logger.error(f"Exception during Auction evaluation criteria creation: {str(e)}")
