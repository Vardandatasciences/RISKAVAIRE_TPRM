...
class DirectProcurement(TPRMEncryptedFieldsMixin, models.Model):
    """Model for Direct Procurement data"""
    ...
    direct_id = models.BigAutoField(primary_key=True, auto_created=True)
    tenant = models.ForeignKey('core.Tenant', on_delete=models.CASCADE, db_column='TenantId', 
                               related_name='direct_procurements', null=True, blank=True)
    direct_number = models.CharField(max_length=50, unique=True, null=True, blank=True)
    direct_title = models.CharField(max_length=255)
    description = models.TextField()
    direct_type = models.TextField()
    ...
    direct_justification = models.TextField(null=True, blank=True)
    vendor_id = models.BigIntegerField(null=True, blank=True)
    
    def __str__(self):
        return f"{self.direct_title} ({self.direct_number})"
    
    def save(self, *args, **kwargs):
        if isinstance(self.direct_number, str):
            self.direct_number = self.direct_number.strip()
            if not self.direct_number:
                self.direct_number = None
        
        if not self.direct_number and not self.pk:
            today = timezone.now()
            prefix = f"DIRECT-{today.year}-{today.month:02d}-"
            last_direct = DirectProcurement.objects.filter(direct_number__startswith=prefix).order_by('-direct_number').first()
            if last_direct:
                try:
                    last_number = int(last_direct.direct_number.split('-')[-1])
                    new_number = last_number + 1
                except (ValueError, IndexError):
                    new_number = 1
            else:
                new_number = 1
            self.direct_number = f"{prefix}{new_number:04d}"
    ...
    class Meta:
        db_table = 'direct_procurements'
        ordering = ['-created_at']
        indexes = [
            models.Index(fields=['direct_number']),
...
class DirectEvaluationCriteria(models.Model):
    """Model for Direct Procurement evaluation criteria"""
    ...
    tenant = models.ForeignKey('core.Tenant', on_delete=models.CASCADE, db_column='TenantId', 
                               related_name='direct_evaluation_criteria', null=True, blank=True)
    direct = models.ForeignKey(DirectProcurement, on_delete=models.CASCADE, related_name='evaluation_criteria', db_column='direct_id')
    ...
    class Meta:
        db_table = 'direct_evaluation_criteria'
        ordering = ['display_order', 'criteria_id']
        indexes = [
            models.Index(fields=['direct']),
