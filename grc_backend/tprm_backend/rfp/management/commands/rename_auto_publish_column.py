    ...
    def handle(self, *args, **options):
        with connection.cursor() as cursor:
            try:
                ...
                self.stdout.write(f'[EMOJI] Database: {db_name}')
                ...
                self.stdout.write(f'\n[EMOJI] auto_publish exists: {auto_publish_exists}')
                self.stdout.write(f'[EMOJI] auto_approved exists: {auto_approved_exists}')
                self.stdout.write(f'[EMOJI] auto_approve exists: {auto_approve_exists}\n')
                ...
                if auto_approved_exists and not auto_approve_exists:
                    ...
                    self.stdout.write(self.style.SUCCESS('[EMOJI] Successfully renamed auto_approved to auto_approve'))
                    ...
                    self.stdout.write(self.style.SUCCESS('[EMOJI] Successfully renamed auto_publish to auto_approve'))
                elif not auto_publish_exists and not auto_approved_exists and auto_approve_exists:
                    self.stdout.write(self.style.WARNING('[EMOJI]  Column already renamed to auto_approve'))
                    ...
                    self.stdout.write(self.style.SUCCESS('[EMOJI] Removed duplicate auto_publish column'))
                    ...
                    self.stdout.write(self.style.SUCCESS('[EMOJI] Removed duplicate auto_approved column'))
                else:
                    self.stdout.write(self.style.ERROR('[EMOJI] Column not found. Adding auto_approve column...'))
                    cursor.execute("ALTER TABLE rfps ADD COLUMN auto_approve BOOLEAN DEFAULT FALSE")
                    self.stdout.write(self.style.SUCCESS('[EMOJI] Added auto_approve column'))
                    
            except Exception as e:
                self.stdout.write(self.style.ERROR(f'[EMOJI] Error: {e}'))
                import traceback
