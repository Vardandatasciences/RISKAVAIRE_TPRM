...
const shareRFP = async (rfp) => {
  ...
  try {
    ...
    // Use current window origin (works for both dev and production)
    const baseUrl = typeof window !== 'undefined' ? window.location.origin : 'https://grc-tprm.vardaands.com'
    ...
    try {
      ...
      const apiBase = getTprmApiBaseUrl()
      const response = await fetch(`${apiBase}/rfp/generate-open-invitation/`, {
        method: 'POST',
...
const viewRFP = async (rfp) => {
  ...
  try {
    const { getAuthHeaders } = useRfpApi()
    const apiBase = getTprmApiBaseUrl()
    const response = await fetch(`${apiBase}/rfp/rfps/${rfp.id}/get_full_details/`, {
      method: 'GET',
      headers: getAuthHeaders(),
    })
...
const editRFP = async (rfp) => {
  ...
  try {
    const { getAuthHeaders } = useRfpApi()
    const apiBase = getTprmApiBaseUrl()
    const response = await fetch(`${apiBase}/rfp/rfp-change-requests/${rfp.id}/`, {
      method: 'GET',
      headers: getAuthHeaders()
    })
    if (response.ok) {
...
const downloadRFPFromList = async (rfp, format) => {
  ...
  try {
    ...
    const endpoint = format === 'pdf' 
      ? `${apiBase}/rfp/rfps/${rfpId}/download/pdf/`
      : `${apiBase}/rfp/rfps/${rfpId}/download/word/`
...
const testBackendConnection = async () => {
  try {
    const { getAuthHeaders } = useRfpApi()
    const apiBase = getTprmApiBaseUrl()
    const response = await fetch(`${apiBase}/rfp/rfps/`, {
      method: 'GET',
      headers: getAuthHeaders(),
    })
