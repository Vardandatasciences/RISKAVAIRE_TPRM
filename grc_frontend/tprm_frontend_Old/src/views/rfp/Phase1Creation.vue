...
const handleSaveDraft = async () => {
  ...
  try {
    ...
    if (changeRequestMode && changeRequestId && approvalId && stageId) {
      const respondUrl = 'https://grc-tprm.vardaands.com/api/tprm/rfp-approval/change-requests/respond/'
      const payload = {
  ...
  try {
    ...
    try {
      ...
      if (isResubmission && changeRequests.length > 0) {
        ...
        const versionHistoryResponse = await axios.get(`https://grc-tprm.vardaands.com/api/tprm/rfp-approval/approval-request-versions/${rfpId}/`, {
          headers: getAuthHeaders()
        })
        ...
        const versionResponse = await axios.post(`https://grc-tprm.vardaands.com/api/tprm/rfp-approval/approval-request-versions/`, versionData, {
          headers: getAuthHeaders()
        })
