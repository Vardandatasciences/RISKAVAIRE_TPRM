...
const addCommitteeEvaluationStages = async () => {
  try {
    ...
    const response = await fetch(`http://localhost:8000/api/v1/rfp/${rfpId}/committee/get/`, {
      method: 'GET',
...
const fetchRFPFromDatabase = async (rfpId: string) => {
  try {
    ...
    const response = await axios.get(`http://localhost:8000/api/v1/rfps/${rfpId}/`, {
      headers: getAuthHeaders()
    })
    ...
    if (response.data) {
      ...
      rfpRequestData.value = {
        ...
        autoApproved: response.data.auto_approved, // Add frontend field name
...
const fetchEvaluationCriteria = async (rfpNumber: string) => {
  try {
    ...
    const response = await axios.get(`http://localhost:8000/api/v1/rfp/${rfpNumber}/evaluation-criteria/`, {
      headers: getAuthHeaders()
    })
      ...
      try {
        ...
        if (response.data.status !== 'APPROVED' && response.data.auto_approve) {
          ...
          try {
            ...
            await axios.post(`http://localhost:8000/api/v1/rfps/${rfpId}/approve/`, {}, {
              headers: getAuthHeaders()
            })
            
            // Verify it was approved
            const verifyResponse = await axios.get(`http://localhost:8000/api/v1/rfps/${rfpId}/`, {
              headers: getAuthHeaders()
            })
