          ...
          <div class="flex items-center justify-between">
            <div class="flex-1">
              <div class="flex items-center space-x-3">
                ...
                <span :class="getStatusBadgeClass(approval.stage_status)">
                  {{ formatStatusText(approval.stage_status) }}
                </span>
              </div>
      ...
      <div class="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto">
        ...
        <div v-if="selectedApproval" class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              <label class="text-sm font-medium text-gray-700">Status:</label>
              <span :class="getStatusBadgeClass(selectedApproval.stage_status)">
                {{ formatStatusText(selectedApproval.stage_status) }}
              </span>
            </div>
            <div>
...
onMounted(async () => {
  ...
  try {
    ...
    const testUrl = 'http://localhost:8000/api/rfp-approval/users/'
