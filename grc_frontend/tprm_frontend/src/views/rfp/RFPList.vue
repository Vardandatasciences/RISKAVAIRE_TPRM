  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      ...
      <div class="flex gap-2">
        <button class="button button--refresh" @click="refreshRFPs" :disabled="loading">
    ...
    <!-- Filters -->
    <Card class="phase-card filters-card">
      <div class="p-6">
        <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
          ...
          <div class="flex gap-2 filters-dropdown-container">
      ...
      <Card v-for="rfp in filteredRFPs" :key="rfp.id" class="phase-card">
        <div class="p-6 overflow-hidden">
          <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <!-- RFP Info -->
            <div class="flex-1 min-w-0 space-y-3 pr-4">
              <div class="flex items-center gap-3 flex-wrap">
                <h3 class="text-lg font-semibold truncate flex-1 min-w-0">{{ rfp.title }}</h3>
                <span :class="getStatusColor(rfp.status)" class="flex-shrink-0">
                  {{ formatStatus(rfp.status) }}
                </span>
                <Badge variant="outline" class="flex-shrink-0">{{ rfp.type }}</Badge>
              </div>
              
              <div class="flex items-center gap-4 text-sm text-muted-foreground flex-wrap">
                <span class="font-medium truncate max-w-full">{{ rfp.rfp_number }}</span>
                ...
                <span v-if="rfp.budgetMin || rfp.budgetMax" class="flex items-center gap-1 whitespace-nowrap">
          ...
          <div class="flex-shrink-0 border-b border-gray-200 bg-gray-50 px-6 py-4">
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center gap-3">
                <span :class="getStatusColor(selectedRFP?.status)" class="text-sm">
              ...
              <div class="bg-white rounded-xl border border-gray-200 p-8">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div class="space-y-4">
                    ...
                    <div class="space-y-3">
                      <div>
                        <Label class="text-xs font-medium text-gray-500 uppercase tracking-wide">Auto Approved</Label>
                        <Badge :class="rfpFullDetails.auto_approved ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'" class="mt-1">
                          {{ rfpFullDetails.auto_approved ? 'Enabled' : 'Disabled' }}
                        </Badge>
                      </div>
                      <div>
...
const statusOptions = [
  ...
  { value: 'submission_open', label: 'Submission Open' },
...
const getStatusColor = (status) => {
  ...
  if (statusLower === 'approved' || statusLower === 'published' || statusLower === 'awarded') {
    return 'badge-approved'
  }
  if (statusLower === 'draft') {
    return 'badge-draft'
  }
  if (statusLower === 'in_review' || statusLower === 'submission_open' || 
      statusLower === 'evaluation') {
    return 'badge-in-review'
  }
  
  // For other statuses (cancelled, archived), return empty string (no special styling)
  return ''
}
...
/* Fix dropdown overlap in filters card */
.filters-card {
...
.filters-dropdown-container :deep(.dropdown) {
  position: relative;
  z-index: 1001;
}

.filters-dropdown-container :deep(.dropdown__menu) {
