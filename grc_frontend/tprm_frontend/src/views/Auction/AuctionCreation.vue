        ...
        <div class="flex flex-col gap-4">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-3">
              <div class="h-10 w-10 rounded-lg bg-gradient-to-br from-amber-600 to-orange-600 flex items-center justify-center">
                <Icons name="gavel" class="h-6 w-6 text-white" />
              </div>
              <div>
                <h1 class="text-2xl sm:text-3xl font-bold tracking-tight text-gray-900">Auction Creation & Setup</h1>
            ...
            <div class="flex flex-wrap items-center gap-2">
              ...
              <button 
                ...
                class="inline-flex items-center px-5 h-10 rounded-lg bg-amber-600 text-white hover:bg-amber-700 hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed text-sm font-semibold shadow-md"
              >
          ...
          <p class="text-gray-600 text-sm mt-2">
            Create an auction to allow vendors to bid competitively on your procurement requirements.
          </p>
        </div>
      </div>
      ...
      <div class="space-y-6">
            ...
            <nav class="flex overflow-x-auto" aria-label="Tabs">
              <div class="flex w-full px-2 tab-container">
                <button
                  ...
                  :class="[
                    'group relative flex items-center gap-2 px-2 sm:px-3 py-2.5 sm:py-3 text-sm font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 tab-button',
                    activeTab === tab.value
                      ? 'text-amber-700 border-b-2 border-amber-600 bg-white shadow-sm'
        ...
        >
          <CardHeader class="bg-gradient-to-r from-gray-50 to-white border-b border-gray-100 rounded-t-xl">
            <div class="flex items-center gap-3">
              <div class="h-10 w-10 rounded-lg bg-gradient-to-br from-amber-500 to-amber-600 flex items-center justify-center shadow-sm">
                <Icons name="gavel" class="h-5 w-5 text-white" />
              </div>
              <div>
                ...
                <CardDescription class="text-sm text-gray-600 mt-0.5">
                  Define the core details of your auction
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent class="p-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="space-y-2">
                <Label html-for="auctionNumber">
                  <span>Auction Number *</span>
                </Label>
                <Input
                  id="auctionNumber"
                  placeholder="e.g., AUCTION-2024-001"
                  v-model="formData.auctionNumber"
                />
              </div>
              <div class="space-y-2">
                ...
                <Input
                  id="title"
                  placeholder="e.g., IT Equipment Auction"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="space-y-2">
                ...
                <Select v-model="formData.type" :disabled="loadingAuctionTypes">
                  <option value="" disabled>Select type</option>
                  <option v-for="auctionType in auctionTypes" :key="auctionType" :value="auctionType">
                    {{ auctionType }}
                  </option>
                </Select>
                <p v-if="loadingAuctionTypes" class="text-xs text-muted-foreground">Loading auction types...</p>
              </div>
              <div class="space-y-2">
                <Label html-for="auctionFormat">
                  <span>Auction Format *</span>
                </Label>
                <Select v-model="formData.auctionFormat">
            ...
            <div class="space-y-2">
              ...
              <Textarea
                id="description"
                placeholder="Provide detailed description of items/services being auctioned..."
                :rows="4"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="space-y-2">
                <Label html-for="auctionStartTime">
                  <span>Auction Start Time *</span>
                </Label>
                <input
                  id="auctionStartTime"
                  type="datetime-local"
                  v-model="formData.auctionStartTime"
                  class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                />
              </div>
              <div class="space-y-2">
                <Label html-for="auctionEndTime">
        ...
        >
          <CardHeader>
            <div class="flex items-center gap-2">
              ...
              <CardTitle>Budget & Auction Pricing</CardTitle>
            </div>
            <CardDescription>
              Set budget parameters and auction pricing rules
            </CardDescription>
          </CardHeader>
          <CardContent class="p-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div v-if="!hiddenFields.startingBid" class="space-y-2">
                <Label html-for="startingBid">Starting Bid *</Label>
                <Input
                  id="startingBid"
                  type="number"
                  placeholder="50,000"
                  v-model="formData.startingBid"
                />
              </div>
              <div v-if="!hiddenFields.reservePrice" class="space-y-2">
                <Label html-for="reservePrice">Reserve Price</Label>
                <Input
                  id="reservePrice"
                  type="number"
                  placeholder="40,000"
                ...
                />
                <p class="text-xs text-gray-500">Minimum acceptable bid</p>
              </div>
              <div v-if="!hiddenFields.bidIncrement" class="space-y-2">
                <Label html-for="bidIncrement">Bid Increment</Label>
                <Input
                  id="bidIncrement"
                  type="number"
                  placeholder="1,000"
                  v-model="formData.bidIncrement"
                />
                <p class="text-xs text-gray-500">Minimum bid increase</p>
              </div>
            </div>
        ...
        >
          <CardHeader>
            ...
            <CardDescription>
              Upload supporting documents for your auction
            </CardDescription>
          </CardHeader>
          <CardContent class="p-6 space-y-6">
            <div class="space-y-4">
...
const formTabs = [
  { value: 'basic', label: 'Basic Setup', description: 'Core details & auction settings' },
  { value: 'documents', label: 'Documents', description: 'Upload files' },
  { value: 'budget', label: 'Budget & Pricing', description: 'Financials & bid rules' },
...
// Default Auction types
const defaultAuctionTypes = [
  'Equipment',
  'Assets',
  'Surplus Inventory',
  'IT Equipment',
...
const auctionTypes = ref<string[]>([...defaultAuctionTypes])
const loadingAuctionTypes = ref(false)
...
const hasExistingDraft = computed(() => {
  ...
  return auctionId && auctionId !== 'null' && auctionId !== ''
})
...
const isFormValid = computed(() => {
  return !!(formData.value.title && formData.value.description && formData.value.type && formData.value.auctionNumber && formData.value.deadline && formData.value.issueDate && formData.value.auctionFormat && formData.value.startingBid && formData.value.auctionStartTime && formData.value.auctionEndTime)
})

const fetchAuctionTypes = async () => {
  try {
    loadingAuctionTypes.value = true
    const response = await axios.get(`${API_BASE_URL}/auction-types/types/`, {
      headers: getAuthHeaders()
    })
    if (response.data && response.data.success && response.data.auction_types && response.data.auction_types.length > 0) {
      // Merge backend types with defaults, removing duplicates
      const backendTypes = response.data.auction_types
      const merged = [...new Set([...defaultAuctionTypes, ...backendTypes])]
      auctionTypes.value = merged.sort()
    } else {
      // Use defaults if backend doesn't return types
      auctionTypes.value = [...defaultAuctionTypes]
    }
  } catch (err) {
    ...
    loadingAuctionTypes.value = false
  }
}
...
const clearDraftAndStartFresh = () => {
  ...
  localStorage.removeItem('auction_draft_current')
  resetFormData()
...
const loadSampleData = () => {
  ...
  const auctionStart = new Date(today.getTime() + 7 * 24 * 60 * 60 * 1000)
  const auctionEnd = new Date(today.getTime() + 21 * 24 * 60 * 60 * 1000)
  ...
  formData.value = {
    auctionNumber: `AUCTION-${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-001`,
    title: 'IT Equipment Auction - Laptops and Servers',
    description: 'Auction for surplus IT equipment including laptops, servers, and networking hardware. All equipment is in good working condition and includes original documentation.',
    type: auctionTypes.value[0] || 'Equipment',
    ...
    auctionStartTime: formatDateTimeLocal(auctionStart),
    auctionEndTime: formatDateTimeLocal(auctionEnd),
    evaluationPeriodEnd: formatDate(inThreeWeeks),
    awardDate: formatDate(inOneMonth),
    criticalityLevel: 'medium',
    evaluationMethod: 'lowest_price',
    geographicalScope: 'United States',
    complianceRequirements: 'Equipment Certification, Warranty Requirements, Data Wiping Standards',
    retentionExpiry: formatDate(new Date(today.getTime() + 365 * 24 * 60 * 60 * 1000)),
    allowLateSubmissions: false,
    autoApprove: false,
    customFields: {
      'equipment_condition': 'Good',
      'inspection_available': 'Yes'
    }
  }
  ...
  criteria.value = [
    {
      ...
      name: 'Bid Amount',
      description: 'Primary evaluation based on the bid amount submitted',
      weight: 60,
      isVeto: false
    },
    {
      ...
      description: 'Assessment of equipment condition and remaining warranty',
      weight: 25,
      isVeto: false
    },
    {
      ...
      name: 'Delivery Terms',
      description: 'Evaluation of delivery timeline and shipping arrangements',
      weight: 15,
      isVeto: false
    }
  ]
  ...
  uploadedDocuments.value = [
    {
      ...
      fileName: 'equipment-inventory.pdf',
      fileSize: 153600,
      uploaded: false
    },
    {
      name: 'Auction Terms and Conditions',
      fileName: 'auction-terms.docx',
      fileSize: 81920,
      uploaded: false
    }
  ]
...
const handleSaveDraft = async () => {
  try {
    ...
    const auctionData = {
      ...
      auction_type: formData.value.type || 'EQUIPMENT',
      auction_format: formData.value.auctionFormat,
      starting_bid: formData.value.startingBid ? Number(formData.value.startingBid) : null,
      reserve_price: formData.value.reservePrice ? Number(formData.value.reservePrice) : null,
      bid_increment: formData.value.bidIncrement ? Number(formData.value.bidIncrement) : null,
      ...
      auction_start_time: formData.value.auctionStartTime ? new Date(formData.value.auctionStartTime).toISOString() : null,
      auction_end_time: formData.value.auctionEndTime ? new Date(formData.value.auctionEndTime).toISOString() : null,
    ...
    const isUpdate = existingAuctionId && existingAuctionId !== 'null' && existingAuctionId !== ''
    
    let response
    if (isUpdate) {
      response = await axios.patch(`${API_BASE_URL}/auctions/${existingAuctionId}/`, auctionData, {
        headers: { ...getAuthHeaders(), 'Content-Type': 'application/json' }
      })
    } else {
      response = await axios.post(`${API_BASE_URL}/auctions/`, auctionData, {
        headers: { ...getAuthHeaders(), 'Content-Type': 'application/json' }
      })
    }
    
    const savedAuctionId = response.data.auction_id || existingAuctionId
    localStorage.setItem('current_auction_id', savedAuctionId)
    
    if (response.data.auction_number) {
      formData.value.auctionNumber = response.data.auction_number
    }
    
    // Save evaluation criteria if any exist
    if (criteria.value.length > 0 && savedAuctionId) {
      try {
        // Delete existing criteria first (if updating)
        if (isUpdate) {
          await axios.delete(`${API_BASE_URL}/auction-evaluation-criteria/`, {
            headers: getAuthHeaders(),
            params: { auction_id: savedAuctionId }
        ...
        for (const criterion of criteria.value) {
          if (criterion.name && criterion.description) {
            await axios.post(`${API_BASE_URL}/auction-evaluation-criteria/`, {
              auction_id: savedAuctionId,
    ...
    success(isUpdate ? 'Draft Updated' : 'Draft Saved', `Your Auction has been ${isUpdate ? 'updated' : 'saved'} as a draft.`)
    lastSaved.value = new Date()
    
  } catch (err) {
    console.error('Error saving Auction:', err)
    if (err.response && err.response.data) {
      error('Error', `Failed to save Auction: ${JSON.stringify(err.response.data)}`)
    } else {
      error('Error', 'Failed to save Auction. Please try again.')
    }
  } finally {
...
onMounted(async () => {
  ...
  autoSaveInterval = setInterval(() => {
    if (formData.value.title || formData.value.description) {
      ...
      localStorage.setItem('auction_draft_current', JSON.stringify(formData.value))
      lastSaved.value = new Date()
