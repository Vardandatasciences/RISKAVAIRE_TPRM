<template>
  <div class="space-y-6 plan-library-page">
    ...
    <div v-if="showPlanDetail" class="plan-detail-view">
      <div class="plan-detail-header">
        <button type="button" @click="goBackToLibrary" class="button button--back">
              ...
              <div class="card-content">
                <div class="space-y-3">
                  ...
                  <div>
                    ...
                    <span :class="getStatusBadgeClass(planDetailData.plan_info.status)">
                      {{ formatStatusText(planDetailData.plan_info.status) }}
                    </span>
                  </div>
                  <div>
                  ...
                  >
                    <div class="evaluation-header">
                      ...
                      <div class="evaluation-status">
                        <span :class="getStatusBadgeClass(evaluation.status)">
                          {{ formatStatusText(evaluation.status) }}
                        </span>
                      </div>
                    </div>
                    ...
                    <div class="strategy-title-row">
                      ...
                      <div class="strategy-status-badges">
                        <span 
                          ...
                          :key="status"
                          :class="getStatusBadgeClass(String(status))"
                        >
                          {{ formatStatusText(String(status)) }}: {{ count }}
                        </span>
                      </div>
                    </div>
                ...
                <div class="plans-grid">
                  ...
                  >
                    ...
                    <div class="plan-actions">
                      <button class="button button--view" @click="handleViewPlan(plan)" title="View Plan">
                        View
                      </button>
    ...
    <!-- Table View (Original) -->
    <div v-else-if="!showPlanDetail" class="plan-table-section">
      <div class="plan-table-header">
        <h3 class="plan-table-title flex items-center gap-2">
          <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="plan-table-content">
        ...
        <!-- Data Table -->
        <div v-else class="table-container overflow-x-hidden">
          <table class="table table-fixed w-full">
            <thead>
              <tr>
                <th style="width: 8%;">Plan ID</th>
                <th style="width: 18%;">Plan Name</th>
            ...
            <tbody>
              <tr v-for="plan in filteredData" :key="plan.plan_id">
                <td class="font-medium break-words">{{ plan.plan_id }}</td>
                <td class="break-words">{{ plan.plan_name }}</td>
                <td>
                  <span :class="plan.plan_type === 'BCP' ? 'badge badge--default' : 'badge badge--secondary'">
                    {{ plan.plan_type }}
                  </span>
                ...
                <td class="break-words">{{ plan.plan_scope }}</td>
                <td class="break-words">{{ plan.vendor_name }}</td>
                <td>
                  <span :class="['badge', getCriticalityColor(plan.criticality)]">
                    {{ plan.criticality }}
                  </span>
                </td>
                <td class="break-words">{{ plan.strategy_name || 'N/A' }}</td>
                <td class="break-words">{{ formatDate(plan.submitted_at) }}</td>
                <td>
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <span :class="getStatusBadgeClass(selectedPlan.status)">
                {{ formatStatusText(selectedPlan.status) }}
              </span>
            </div>
            <div>
              <label class="text-sm font-medium">Vendor</label>
...
const statusFilterOptions = [
  ...
  { value: "OCR_IN_PROGRESS", label: "OCR In Progress" },
  { value: "OCR_COMPLETED", label: "OCR Completed" },
  { value: "ASSIGNED_FOR_EVALUATION", label: "Assigned for Evaluation" },
...
const vendorFilterOptions = [
  { value: "all", label: "Any" },
  { value: "Mau", label: "Mau" },
  { value: "CoreNet", label: "CoreNet" },
  { value: "FinEdge", label: "FinEdge" }
]
...
const scopeFilterOptions = [
  ...
  { value: "physical_device", label: "Physical Device" },
...
const fetchStrategies = async () => {
  ...
  try {
    ...
    const url = queryString ? `/api/tprm/v1/bcpdrp/strategies/?${queryString}` : '/api/tprm/v1/bcpdrp/strategies/'
...
const fetchPlans = async () => {
  ...
  try {
    ...
    const queryString = params.toString()
    const url = queryString ? `/api/tprm/v1/bcpdrp/plans/?${queryString}` : '/api/tprm/v1/bcpdrp/plans/'
...
const getStatusBadgeClass = (status: string) => {
  ...
  } else if (statusUpper === 'ASSIGNED_FOR_EVALUATION' || statusUpper === 'ASSIGNED FOR EVALUATION') {
    return 'badge-pending-assignment' // Orange
  } else if (statusUpper === 'UNDER_EVALUATION' || statusUpper === 'UNDER EVALUATION') {
    return 'badge-in-review' // Orange
...
const fetchPlanDetails = async (planId: number) => {
  ...
  try {
    const response = await http.get(`/api/tprm/v1/bcpdrp/plans/${planId}/comprehensive/`)
