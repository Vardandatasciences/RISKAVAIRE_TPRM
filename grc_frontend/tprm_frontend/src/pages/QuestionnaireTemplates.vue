<template>
  <div class="qt-container">
    ...
    <form class="global-form-container" @submit.prevent="handleSubmit">
      <div class="global-form-box">
        <div class="global-form-row">
          <div class="global-form-group">
            <label for="template_name" class="global-form-label">Template Name<span class="global-form-label-required">*</span></label>
            <input id="template_name" v-model.trim="form.template_name" type="text" class="global-form-input" required placeholder="e.g., Vendor Security Assessment" />
          </div>
          <div class="global-form-group">
            <label for="template_version" class="global-form-label">Version</label>
            <input id="template_version" v-model.trim="form.template_version" type="text" class="global-form-input" placeholder="1.0" />
          </div>
          <div class="global-form-group">
            <label for="template_type" class="global-form-label">Template Type</label>
            <select id="template_type" v-model="form.template_type" class="global-form-select">
              <option v-for="opt in TEMPLATE_TYPE_CHOICES" :key="opt" :value="opt">{{ opt }}</option>
            </select>
          </div>

          <div class="global-form-group">
            <label for="module_type" class="global-form-label">Module Type</label>
            <select id="module_type" v-model="form.module_type" class="global-form-select">
              <option v-for="opt in MODULE_TYPE_CHOICES" :key="opt" :value="opt">{{ opt }}</option>
            </select>
          </div>

          <div class="global-form-group">
            <label for="module_subtype" class="global-form-label">Module Subtype</label>
            <input id="module_subtype" v-model.trim="form.module_subtype" type="text" class="global-form-input" placeholder="Optional subtype" />
          </div>

          <div class="global-form-group">
            <label for="status" class="global-form-label">Status</label>
            <select id="status" v-model="form.status" class="global-form-select">
          ...
          <div class="global-form-group global-form-group-full-width">
            <div class="global-form-checkbox-group">
              <label class="global-form-checkbox-option">
                <input type="checkbox" v-model="form.approval_required" />
                <span class="global-form-checkbox-label">
                  <span class="global-form-checkbox-label-text">Approval Required</span>
                </span>
              </label>
              <label class="global-form-checkbox-option">
                <input type="checkbox" v-model="form.is_active" />
                <span class="global-form-checkbox-label">
                  <span class="global-form-checkbox-label-text">Active</span>
                </span>
              </label>
            </div>
          </div>
        ...
        <div class="global-form-group">
          <label for="template_description" class="global-form-label">Description</label>
          <textarea id="template_description" v-model.trim="form.template_description" class="global-form-textarea" rows="4" placeholder="Describe the purpose of this template"></textarea>
        </div>
      </div>
      ...
      <div class="global-form-box">
        <div class="global-form-section">
          <div class="qt-label-row">
            <label class="global-form-section-title">Questions</label>
            <div class="qt-question-controls">
              <button type="button" class="button button--add" @click="addQuestion">Add question</button>
              <button
          ...
          <div v-for="(q, idx) in questions" :key="q._key" class="qt-question">
            ...
            <div class="global-form-group">
              <label class="global-form-label">question_text<span class="global-form-label-required">*</span></label>
              <textarea v-model.trim="q.question_text" class="global-form-textarea" rows="2" placeholder="Enter the question text"></textarea>
            </div>

            <div class="global-form-group">
              <label class="global-form-label">help_text</label>
              <textarea v-model.trim="q.help_text" class="global-form-textarea" rows="2" placeholder="Optional help for respondent"></textarea>
            </div>

            <!-- Then the attribute grid -->
            <div class="global-form-row">
              <div class="global-form-group" v-if="form.module_type === 'VENDOR'">
                <label class="global-form-label">question_category</label>
                <input type="text" v-model.trim="q.question_category" class="global-form-input" placeholder="e.g., Security" />
              </div>
              <div class="global-form-group">
                <label class="global-form-label">answer_type</label>
                <select v-model="q.answer_type" class="global-form-select">
                  <option v-for="opt in ANSWER_TYPE_CHOICES" :key="opt" :value="opt">{{ opt }}</option>
                </select>
              </div>
              <div class="global-form-group">
                <label class="global-form-label">is_required</label>
                <select v-model="q.is_required" class="global-form-select">
                  <option :value="true">true</option>
                  <option :value="false">false</option>
                </select>
              </div>
              <div class="global-form-group">
                <label class="global-form-label">weightage</label>
                <input type="number" step="0.01" v-model.number="q.weightage" class="global-form-input" placeholder="e.g., 10.0" />
              </div>
              <div class="global-form-group" v-if="form.module_type === 'SLA'">
                <label class="global-form-label">metric_name</label>
                <input type="text" v-model.trim="q.metric_name" class="global-form-input" placeholder="e.g., Security Score" />
              </div>
              <div class="global-form-group" v-if="form.module_type === 'CONTRACT'">
                <label class="global-form-label">term_id</label>
                <input type="text" v-model.trim="q.term_id" class="global-form-input" placeholder="e.g., TERM-001 or 42 (optional)" />
              </div>
              <div class="global-form-group">
                <label class="global-form-label">allow_document_upload</label>
                <select v-model="q.allow_document_upload" class="global-form-select">
                  <option :value="true">true</option>
                  <option :value="false">false</option>
                </select>
              </div>
              <div class="global-form-group" v-if="q.answer_type === 'MULTIPLE_CHOICE' || q.answer_type === 'CHECKBOX'">
                <label class="global-form-label">options (comma separated)</label>
                <input type="text" v-model.trim="q._optionsString" class="global-form-input" placeholder="Option 1, Option 2" />
              </div>
            </div>
      ...
      <div class="qt-actions">
        <button type="submit" class="button button--save" :disabled="submitting">{{ submitting ? 'Saving...' : 'Save Template' }}</button>
...
.qt-label-row .global-form-section-title {
  margin-bottom: 0;
  border-bottom: none;
  padding-bottom: 0;
}
