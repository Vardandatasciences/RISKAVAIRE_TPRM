    ...
    <div class="response-content">
        ...
        <div v-for="(q, idx) in responses" :key="q.id" class="question-card">
          ...
          <div class="question-content">
            ...
            ></textarea>
              ...
              <div v-else-if="q.question_type === 'MULTIPLE_CHOICE'" class="global-form-radio-group">
                <label v-for="option in getQuestionOptions(q)" :key="option" class="global-form-radio-option">
                  <input 
                    type="radio" 
                    :value="option" 
                    v-model="q.vendor_response" 
                  ...
                  <span class="global-form-radio-label">
                    <span class="global-form-radio-label-text">{{ option }}</span>
                  </span>
                </label>
              </div>

              <!-- CHECKBOX Response -->
              <div v-else-if="q.question_type === 'CHECKBOX'" class="global-form-checkbox-group">
                <label v-for="option in getQuestionOptions(q)" :key="option" class="global-form-checkbox-option">
                  <input 
                    type="checkbox" 
                    :value="option"
      ...
      <div v-if="responses.length > 0" class="submit-section">
        <div class="submit-content">
          ...
          <div class="submit-actions">
            <button @click="submitResponses" class="button button--submit" :disabled="saving || isLocked" v-if="!isLocked">
      ...
      <div v-if="vendorAssignments.length === 0" class="empty-state">
        ...
        <div v-if="!currentVendorId" class="empty-state-actions">
          <router-link to="/vendor-registration" class="button button--create">
...
const getCurrentVendorId = async () => {
  try {
    ...
    // First, try to get user from Vuex store
    try {
...
const getStatusBadgeClass = (status) => {
  ...
  switch (statusLower) {
    case 'assigned': return 'badge-assigned status-assigned assigned-text'
    case 'in-progress':
    case 'in_progress': return 'badge-in-review status-in-review in-review-text'
    case 'submitted': return 'badge-submitted submitted-text'
    case 'responded': return 'badge-completed completed-text'
    case 'completed': return 'badge-completed completed-text'
    case 'overdue': return 'badge-priority-high priority-high-text'
    default: return 'badge-draft status-draft draft-text'
  }
}
...
const saveResponses = async (showMessage = true) => {
  ...
  try {
    ...
    if (isConnectionError) {
      ...
      console.error('Connection error - backend server may not be running at http://localhost:8000')
...
const performSubmit = async () => {
  ...
  try {
    ...
    if (error.response && (error.response.status === 200 || error.response.status === 201)) {
      ...
      const errorMsg = 'Cannot connect to the server. Please check if the backend server is running at http://localhost:8000 and try again.'
...
// Lifecycle
onMounted(async () => {
  console.log('[VendorQuestionnaireResponse] Component mounted, checking permissions...')
  
  await loggingService.logPageView('Vendor', 'Vendor Questionnaire Response')
  
  // Check if user has permission to view/submit questionnaire responses
  try {
    ...
    if (isInIframe) {
      ...
      if (!user || (!user.id && !user.userid)) {
        console.log('[VendorQuestionnaireResponse] In iframe but no user found, waiting for GRC auth...')
        ...
        if (!retryUser || (!retryUser.id && !retryUser.userid)) {
          console.warn('[VendorQuestionnaireResponse] Still no user found after wait, allowing access in iframe mode')
          // In iframe mode, allow access if GRC is handling permissions
          // Proceed with loading questionnaire
        } else {
          console.log('[VendorQuestionnaireResponse] User found after wait:', retryUser)
        }
      }
    }
    
    // Check for submit permission
    const hasSubmitPermission = await permissionsService.checkVendorPermission('submit_questionnaire_responses')
    
    console.log('[VendorQuestionnaireResponse] Permission check result:', hasSubmitPermission)
    
    // In iframe mode, allow access even if permission check fails (GRC handles permissions)
    if (!hasSubmitPermission && !isInIframe) {
      console.warn('[VendorQuestionnaireResponse] User does not have permission to submit questionnaire responses')
      
      sessionStorage.setItem('access_denied_error', JSON.stringify({
        message: 'You do not have permission to submit questionnaire responses. Please contact your administrator.',
        code: '403',
    ...
    if (hasSubmitPermission || isInIframe) {
      console.log('[VendorQuestionnaireResponse] Permission granted, loading questionnaire...')
    }
  } catch (error) {
    console.error('[VendorQuestionnaireResponse] Error checking permissions:', error)
    ...
    if (!isInIframe) {
      ...
      sessionStorage.setItem('access_denied_error', JSON.stringify({
        message: 'Error checking permissions for questionnaire responses. Please try again or contact your administrator.',
        code: '500',
        path: window.location.pathname
      }))
      window.location.href = '/access-denied'
      ...
      console.log('[VendorQuestionnaireResponse] Error in iframe mode, allowing access (GRC handles permissions)')
    }
  }
