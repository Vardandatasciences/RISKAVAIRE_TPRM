<template>
  <div class="vendor_space-y-6">
    ...
    <div class="kpi-cards-grid">
      <div v-for="(vendor_kpi, vendor_index) in vendor_kpiData" :key="vendor_index" class="kpi-card">
        <div class="kpi-card-content">
          <div class="kpi-card-icon-wrapper" :class="vendor_getKpiIconColorClass(vendor_kpi)">
            <component :is="vendor_kpi.icon" />
          </div>
          <div class="kpi-card-text">
            <h3 class="kpi-card-title">{{ vendor_kpi.title }}</h3>
            <div class="kpi-card-value">{{ vendor_kpi.value }}</div>
        ...
        <CardContent class="vendor_activity-content">
          <div v-for="(vendor_vendor, vendor_index) in vendor_recentVendors" :key="vendor_index" class="vendor_vendor-activity-item">
            ...
            <div class="vendor_vendor-actions">
              <span :class="vendor_getStatusBadgeClass(vendor_vendor.status)">
                {{ vendor_vendor.status }}
              </span>
              <span :class="vendor_getRiskBadgeClass(vendor_vendor.riskLevel)">
...
const vendor_getKpiIconColorClass = (kpi) => {
  // Map KPI titles to icon color classes from main.css
  const title = kpi.title.toLowerCase()
  if (title.includes('high-risk')) return 'kpi-card-icon-red'
  if (title.includes('questionnaire') || title.includes('completion')) return 'kpi-card-icon-green'
  if (title.includes('risk score')) return 'kpi-card-icon-orange'
  if (title.includes('total vendors')) return 'kpi-card-icon-blue'
  return 'kpi-card-icon-blue'
}


// Map risk levels to badge classes from badge.css
const vendor_getRiskBadgeClass = (risk) => {
  switch (risk.toLowerCase()) {
    case 'low': return 'badge-priority-low priority-low-text'
    case 'medium': return 'badge-priority-medium priority-medium-text'
    case 'high': return 'badge-priority-high priority-high-text'
    case 'critical': return 'badge-critical critical-text'
    default: return 'badge-priority-low priority-low-text'
  }
}

// Map status to badge classes from badge.css
const vendor_getStatusBadgeClass = (status) => {
  const statusLower = status.toLowerCase().replace(/\s+/g, '-').replace(/_/g, '-')
  switch (statusLower) {
    case 'approved': return 'badge-approved status-approved approved-text'
    case 'active': return 'badge-active status-active active-text'
    case 'in-review':
    case 'in_review':
    case 'review': return 'badge-in-review status-in-review in-review-text'
    case 'draft': return 'badge-draft status-draft draft-text'
    case 'pending': return 'badge-pending-assignment status-pending-assignment pending-assignment-text'
    case 'submitted': return 'badge-submitted submitted-text'
    case 'completed': return 'badge-completed completed-text'
    case 'terminated': return 'badge-terminated terminated-text'
    case 'rejected': return 'badge-rejected rejected-text'
    case 'cancelled': return 'badge-cancelled cancelled-text'
    case 'compliant': return 'badge-compliant compliant-text'
    case 'non-compliant': return 'badge-non-compliant non-compliant-text'
