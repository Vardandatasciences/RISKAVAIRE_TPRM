<template>
  <div class="space-y-6 contract-dashboard-page">
    <!-- Loading State -->
      ...
      <div class="kpi-cards-grid">
        <div class="kpi-card">
          <div class="kpi-card-content">
            ...
            <div class="kpi-card-text">
              <div class="kpi-card-title">Total Contracts</div>
              <div class="kpi-card-value">{{ totalContracts }}</div>
              <div class="kpi-card-subheading">+2 from last month</div>
            </div>
          </div>
        </div>
        ...
        <div class="kpi-card">
          <div class="kpi-card-content">
            ...
            <div class="kpi-card-text">
              <div class="kpi-card-title">Active Contracts</div>
              <div class="kpi-card-value">{{ activeContracts }}</div>
              <div class="kpi-card-subheading">{{ Math.round((activeContracts / totalContracts) * 100) || 0 }}% of total</div>
            </div>
          </div>
        </div>
        ...
        <div class="kpi-card">
          <div class="kpi-card-content">
            ...
            <div class="kpi-card-text">
              <div class="kpi-card-title">Expiring Soon</div>
              <div class="kpi-card-value">{{ expiringContracts }}</div>
              <div class="kpi-card-subheading">Within 90 days</div>
            </div>
          </div>
        </div>
        ...
        <div class="kpi-card">
          <div class="kpi-card-content">
            ...
            <div class="kpi-card-text">
              <div class="kpi-card-title">Total Value</div>
              <div class="kpi-card-value">${{ (totalValue / 1000000).toFixed(1) }}M</div>
              <div class="kpi-card-subheading">Active contracts only</div>
            </div>
          </div>
        </div>
      </div>
            ...
            <div class="flex items-center space-x-4">
              ...
              <div class="flex items-center space-x-4">
                ...
                <div class="flex flex-col gap-2">
                  <span :class="getStatusBadgeClass(contract.status)" class="badge text-xs">
                    {{ formatStatusText(contract.status) }}
                  </span>
                  <span :class="getPriorityBadgeClass(contract.priority)" class="badge text-xs">
                    {{ formatPriorityText(contract.priority) }}
                  </span>
              </div>
              <Button
                variant="ghost"
...
const getStatusBadgeClass = (status) => {
  ...
  if (statusUpper === 'UNDER_REVIEW' || statusUpper === 'PENDING' || statusUpper === 'IN_REVIEW' || statusUpper === 'IN_PROGRESS') {
    return 'badge-in-review'
  }
  if (statusUpper === 'PENDING_ASSIGNMENT' || statusUpper === 'PENDING ASSIGNMENT') {
  ...
  if (statusUpper === 'TERMINATED' || statusUpper === 'EARLY_TERMINATION') {
  ...
  // For unknown statuses, return draft as default
  return 'badge-draft'
}
...
const getPriorityBadgeClass = (priority) => {
  ...
  // Map to badge.css classes
  if (priorityLower === 'high' || priorityLower === 'urgent' || priorityLower === 'critical') {
  ...
  // For unknown priority, return medium as default
  return 'badge-priority-medium'
}
...
const formatPriorityText = (priority) => {
  ...
  const priorityText = String(priority).toUpperCase()
...
.contract-dashboard-page .kpi-card:hover {
  background-color: #ffffff !important;
  background: #ffffff !important;
}

.dark-theme .contract-dashboard-page .kpi-card:hover {
  background-color: #374151 !important;
  background: #374151 !important;
}

.contract-dashboard-page .kpi-card:hover .kpi-card-content {
  background-color: transparent !important;
  background: transparent !important;
}

.dark-theme .contract-dashboard-page .kpi-card:hover .kpi-card-content {
  background-color: transparent !important;
  background: transparent !important;
}
</style>